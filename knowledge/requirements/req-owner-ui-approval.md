...
allow the exact hagency MCP coordination and task-control tools without
creating a recursive owner approval. Text-only hagency messaging MAY use
this path; any message that asks hagency to read and transmit a local file
...
[REQ-OWNER-UI-APPROVAL-AUTHORITY] Robrix2 MUST remain a rendering and event-emission client; hagency MUST remain the authorization authority.
...
Scenario: Owner approves once from the encrypted DM UI
  ...
  Then hagency atomically accepts one matching verdict and relays it through the supported runtime permission protocol

Scenario: Public project-room notice is read only
  Given an agent is waiting for approval
  When hagency publishes status to the project room
...
Scenario: Another developer cannot approve
  ...
  Then hagency rejects the event
...
Scenario: Two local agents share one project room without sharing authority
  ...
  Then hagency resolves the request through that exact agent's room-agent binding
...
Scenario: Expired or replayed action is rejected
  ...
  Then hagency rejects it without changing the terminal result
...
Scenario: Empty owner fails closed
  ...
  Then hagency denies the request and creates no generally-approvable fallback
...
Scenario: Codex reads its workflow inbox without recursive approval
  ...
  When Codex calls the exact hagency check_inbox MCP tool
