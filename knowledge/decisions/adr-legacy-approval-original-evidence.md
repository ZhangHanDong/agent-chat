...
The current room/readiness/join/encryption observations are authenticated bridge claims and must be refreshed before publication; this store/API unit performs no Matrix I/O. Plaintext uses only the existing ADR-003 explicit non-production plaintext-test opt-in and never downgrades a room observed as encrypted. The bridge owns encrypt-once preparation and exact stored ciphertext replay.
