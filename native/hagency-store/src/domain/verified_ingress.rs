...
impl DomainRepository {
    ...
    /// Offline host scheduling and activation proof only. A live adapter must
    /// add durable begin-send/uncertain custody and revalidate the current route
    /// immediately before I/O; a stable transaction ID cannot fence a delayed
    /// private send after promotion. Do not wire this claim directly to Matrix.
    pub fn claim_verified_task_notice(
        ...
        clock(now)?;
        if !(1..=60_000).contains(&lease_ms) || now > hagency_core::JSON_SAFE_MAX - lease_ms {
        ...
        let tx = self
            ...
            .transaction_with_behavior(TransactionBehavior::Immediate)?;
        matrix_routes::reconcile(&tx, now)?;
        tx.execute("UPDATE task_notices SET state='cancelled',claim_hash=NULL,claim_until=NULL,error_code='route_retired' WHERE verified_route IS NOT NULL AND state IN ('pending','claimed','failed') AND NOT EXISTS(SELECT 1 FROM current_matrix_routes r WHERE r.session_id=json_extract(task_notices.config,'$.session_id'))",[])?;
        let id:Option<String>=tx.query_row("SELECT id FROM task_notices WHERE verified_route IS NOT NULL AND not_before<=?1 AND (state='pending' OR (state='claimed' AND claim_until<=?1)) ORDER BY rowid LIMIT 1",[now],|r|r.get(0)).optional()?;
        let Some(id) = id else {
            tx.commit()?;
            return Ok(None);
        };
        let notice = task_intents::notice(&tx, &id)?;
        let route = matrix_routes::route(&tx, &notice.session_id)?;
        let (encoded, digest): (String, String) = tx.query_row(
            "SELECT verified_route,content_digest FROM task_notices WHERE id=?1",
            [&id],
            |r| Ok((r.get(0)?, r.get(1)?)),
        )?;
        if serialize(&route)? != encoded {
            return Err(Error::RunnerAuthority);
        }
        let root: u64 = tx.query_row(
            "SELECT root_sequence FROM task_intents WHERE task_id=?1",
            [&notice.task_id],
            |r| r.get(0),
        )?;
        let source = task_message(&tx, &notice.task_id, root)?;
        ...
        tx.execute(
            ...
            params![id, canonical::digest(&json!(token))?, now + lease_ms],
        )?;
        tx.commit()?;
    ...
    pub fn deliver_verified_task_notice(
        ...
        let notice = task_intents::notice(&tx, id)?;
        let route = matrix_routes::route(&tx, &notice.session_id)?;
        let (encoded,digest,state,hash,deadline,old):(String,String,String,Option<String>,Option<u64>,Option<String>)=tx.query_row("SELECT verified_route,content_digest,state,claim_hash,claim_until,delivery FROM task_notices WHERE id=?1",[id],|r|Ok((r.get(0)?,r.get(1)?,r.get(2)?,r.get(3)?,r.get(4)?,r.get(5)?)))?;
        if serialize(&route)? != encoded
            || input.transaction_id != notice.transaction_id
            || input.digest != digest
        ...
        if state == "delivered" {
            if old.as_deref() != Some(&observed) {
                return Err(Error::Conflict);
            }
            return task_intents::intent_result(&tx, &notice.task_id, true);
        }
        if state != "claimed"
            || deadline.is_none_or(|until| until <= now)
            || !execution::matches_secret(hash.as_deref().unwrap_or(""), token)?
        {
            return Err(Error::RunnerAuthority);
        }
        tx.execute("UPDATE task_notices SET state='delivered',delivery=?2,claim_hash=NULL,claim_until=NULL WHERE id=?1",params![id,observed])?;
        if notice.kind == "ack" {
            if tx.execute("UPDATE task_intents SET state='active',anchor_event_id=?2 WHERE task_id=?1 AND state='pending'",params![notice.task_id,input.event_id])?!=1{return Err(Error::State)}
