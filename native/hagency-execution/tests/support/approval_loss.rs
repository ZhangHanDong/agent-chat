...
/// The armed-frame window (design Q3): with the host held at the recheck
/// gate and the frame armed at zero accepted bytes, the probe puts one
/// buffered usage event on the wire BEFORE the gate releases. The old
/// `prepared_inner` returned `Controlled::Event` there — discarding the
/// write for that call, leaving `write: None` and no receipt. The fix writes
/// the armed frame first; the buffered event is delivered on the next call,
/// after the receipt, and nothing is lost.
#[tokio::test]
async fn native_owned_approval_armed_frame_precedes_buffered_event() {
    use std::sync::{Arc, atomic::Ordering};
    let root = tempfile::tempdir().unwrap();
    let work = root.path().join("work");
    hagency_store::private::directory(&work).unwrap();
    ...
    let (domain, cap) = fixture(root.path());
    let gate = Arc::new(crate::approval::Gate::default());
    let mut configured = host(&work, Fault::RecheckGate, "owned-approval-write-first");
    configured.approval_gate = Some(gate.clone());
    let mut op = Operation::start(
        domain.clone(),
        cap.clone(),
    ...
    let notice = tokio::time::timeout(Duration::from_secs(6), notices.recv())
        ...
        .unwrap();
    choose(&domain, notice.request_id).await;
    // The host is at the recheck gate, frame armed, zero bytes accepted.
    let end = tokio::time::Instant::now() + harness_wait();
    while !gate.entered.load(Ordering::Acquire) {
        assert!(
            tokio::time::Instant::now() < end,
    ...
    // The buffered event must be ON THE WIRE before the host is released, so
    // the armed frame's first write step faces ready input — the window.
    let buffered = work.join("owned-dispatch.approval-buffered");
    let end = tokio::time::Instant::now() + harness_wait();
    while !buffered.exists() {
        assert!(
            tokio::time::Instant::now() < end,
            "probe never emitted the buffered event; probe markers present: {}",
            markers_present(&work)
        );
        tokio::time::sleep(Duration::from_millis(5)).await;
    }
    ...
    // The frame was written first: exactly one response frame on the wire,
    // its receipt recorded, the acceptance row present.
    assert_eq!(host_response_frames(&work).len(), 1);
    let trace = hagency_execution::diagnostics::dispatch_trace(&cap.dispatch_id);
    assert!(
        trace.contains("write-accepted"),
        "receipt never recorded; trace: {trace}"
    );
    assert!(
        trace.contains("recorded"),
        "acceptance row never recorded; trace: {trace}"
    );
    let sql = rusqlite::Connection::open(root.path().join("state/domain.sqlite3")).unwrap();
    assert_eq!(
        sql.query_row(
    ...
    // Nothing lost: the buffered usage event is delivered after the receipt.
    let usage = report.usage_status();
