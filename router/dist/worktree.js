...
function writeBootstrapState(statePath, state) {
    writeFileSync(statePath, `${JSON.stringify(state)}\n`, { encoding: 'utf8', mode: 0o600 });
}
function ensureBootstrap(spec, worktreePath) {
    if (!spec.bootstrap || spec.bootstrap.length === 0)
        return;
    const executable = spec.bootstrap[0];
    if (!executable)
        throw new Error('worktree bootstrap executable is empty');
    ...
    if (state?.status === 'complete' && state.digest === bootstrapDigest)
        return;
...
export class WorktreeManager {
    ...
    ensureAsync(spec) {
        ...
        const preparation = new Promise((resolve, reject) => {
            ...
            worker.once('message', (message) => {
                ...
                if (message !== null && typeof message === 'object' && !Array.isArray(message)
                    ...
                    const error = message !== null && typeof message === 'object' && !Array.isArray(message)
                        ? String(message.error ?? 'worktree preparation failed')
            ...
            worker.once('exit', (code) => {
                if (!settled && code !== 0) {
                    ...
                    reject(new Error('worktree preparation worker exited without a result'));
            });
        ...
        if (repositoryRoot !== repositoryPath)
            throw new Error('repository_path must be the git worktree root');
        ...
        if (existingBranchPath) {
            ...
            if (!existsSync(resolved.target))
                throw new Error('registered worktree path is missing');
        ...
        try {
            git(repositoryPath, ['show-ref', '--verify', '--quiet', `refs/heads/${resolved.branch}`]);
        }
        catch {
            branchExists = false;
        }
        if (branchExists)
            git(repositoryPath, ['worktree', 'add', resolved.target, resolved.branch]);
        else
            git(repositoryPath, ['worktree', 'add', '-b', resolved.branch, resolved.target, 'HEAD']);
