...
export type DispatchState = 'queued' | 'leased' | 'started' | 'parked' | 'completed' | 'cancelled_before_start' | 'outcome_unknown';
export type RefusalCode = 'bad_request' | 'not_found' | 'idempotency_conflict' | 'missing_task_binding' | 'missing_task_credential' | 'task_not_active' | 'unsupported_framework' | 'remote_runner_unsupported' | 'resource_unavailable' | 'workspace_quarantined' | 'live_runner_cap' | 'parked_runner_cap' | 'invalid_capability' | 'capability_expired' | 'dispatch_not_current' | 'invalid_transition' | 'approval_mismatch' | 'inspection_required' | 'inspection_expired' | 'matrix_command_conflict';
