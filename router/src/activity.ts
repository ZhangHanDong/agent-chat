...
export type ActivityEvent = { phase: 'started' | 'heartbeat' | 'waiting' | 'resumed' | 'completed' | 'interrupted' }
  | { phase: 'tool_start' | 'tool_end'; kind: ToolKind; eventId: string };
type ActivityRow = {
  dispatch_id: string; phase: string; kind: ToolKind | null; tools: number; finished: number;
  started_at: number; updated_at: number; queued_at: number; revision: number; anchor: string | null;
};
...
export class ActivityStore {
  ...
  update(dispatchId: string, event: ActivityEvent, now: number): { revision: number; body: string } | null {
    const previous = this.read(dispatchId);
    if (previous && ['completed', 'interrupted'].includes(previous.phase)) return null;
    if (!previous && event.phase !== 'started') return null;
    if (previous && event.phase === 'started') return null;
    let row: ActivityRow = previous ?? {
    ...
    if (!previous) this.db.prepare(`INSERT INTO runner_activity
    ...
    } else if (event.phase !== 'heartbeat') row = { ...row, phase: event.phase, kind: null };
