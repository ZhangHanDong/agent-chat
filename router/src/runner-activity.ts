...
const CODEX_TOOLS: Record<string, ToolKind> = {
...
const CLAUDE_TOOLS: Record<string, ToolKind> = {
...
export function codexActivity(method: string, params: Readonly<Record<string, unknown>>, threadId: string, turnId: string): ActivityEvent | null {
  if (!threadId || !turnId || params.threadId !== threadId || params.turnId !== turnId) return null;
  if (method !== 'item/started' && method !== 'item/completed') return null;
  const item = params.item as Record<string, unknown> | undefined;
  if (!item || typeof item.id !== 'string' || typeof item.type !== 'string' || !Object.hasOwn(CODEX_TOOLS, item.type)) return null;
  return { phase: method === 'item/started' ? 'tool_start' : 'tool_end', kind: CODEX_TOOLS[item.type]!, eventId: key(item.id) };
}

export function claudeActivity(event: Readonly<Record<string, unknown>>, activeTools: Map<string, ToolKind>): ActivityEvent[] {
