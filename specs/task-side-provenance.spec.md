...
The operator selected model C: independent Hagency instances share one Palpo,
...
- Use deterministic Vitest fixtures with an isolated `HAGENCY_RUNTIME_DIR` set before bridge import, a disk-backed `TMPDIR` outside `/tmp`, disabled core dumps, and serial workers. Drive actual push, edge, and sync adapters through the real router and bridge ingress; observe typed downstream call counts and adapter transaction/ack/cursor outcomes. Predicate-only tests or direct ingress calls cannot substitute for these integration scenarios.
...
Scenario: Two instances on one Palpo receive only their own interested rooms
  ...
  Test:
    ...
    Targets: lib/appservice-listener.js, lib/appservice-puller.js, lib/appservice-sync.js, lib/appservice-receiver.js, bridge-matrix.js
  Given instances A and B have separate control planes and runtime directories, the same sideId palpo.test, different registration IDs and token pairs, representatives @hagency_a:palpo.test and @hagency_b:palpo.test recorded by whoami, and disjoint ac_a_ and ac_b_ agent namespaces
