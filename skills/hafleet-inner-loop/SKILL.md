---
name: hafleet-inner-loop
description: Use when an HAFleet-managed Claude, Codex or other middle agent delegates repository work through Herdr and octoloop to a lower execution agent, or must monitor and verify such a job.
---

# HAFleet inner execution

The HAFleet-managed agent owns decomposition, execution choice, verification
...
installed kind. This does not make octos a supported HAFleet thread runner.
...
HAFleet task state or an authorization boundary between same-user processes.
...
seconds. Do not end the HAFleet dispatch while an unowned watcher or inner
...
HAFleet marked its task done, or Matrix delivered a reply.

After review, use the current session's authenticated HAFleet task lifecycle
