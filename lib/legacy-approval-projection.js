...
export async function legacyOwnedJsonRequest(url, options, budget) {
  ...
  try {
    budget.check();
    const response = await fetch(url, { method: options.method || 'GET', headers: options.headers, redirect: 'error',
    ...
    controller.signal.throwIfAborted(); budget.check();
...
export function normalizeLegacyProofEvent(raw, clear, roomId, expectedId, role, approval) {
  ...
  const namespace = ['com.agentchat.approval', 'com.hafleet.approval']
    .find(key => content.msgtype === `${key}.${role}.v1`);
  const detail = namespace && content[namespace];
  if (!detail || detail.version !== 1 || detail.kind !== role
    || (Object.hasOwn(content, 'com.agentchat.approval') && Object.hasOwn(content, 'com.hafleet.approval'))) fail('legacy_invalid_role');
...
export async function publishLegacyApprovalProjection(row, io, options = {}) {
  ...
  try {
    ...
    await backend('POST', `${projectionPath}/receipt`, { ...identity, event_id: response.event_id });
