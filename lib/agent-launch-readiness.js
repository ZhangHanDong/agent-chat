 ...
 * before it creates tmux". Two launchers honoured that (`bin/hagency-up` and its remote twin) and a
 ...
 *
 * `bin/hagency-up`'s preflight ESTABLISHES readiness: it mints the agent token, installs Claude ask
 ...
 * sees. `bin/hagency-up` MINTS this when missing; here its absence means provisioning never ran
...
function checkAgentToken(stateDir) {
  ...
  if (!nonEmptyFile(file)) {
    return {
      ...
      detail: `no owner-approval token at ${file}; run \`hagency up\` for this agent to provision it`,
    };
  }
  return { ok: true };
}
 ...
 *
 * Mirrors what `bin/hagency-up`'s preflight checks for the same framework: the channel not disabled,
...
function checkClaude(agentPath, env) {
  if (String(env.HAGENCY_CLAUDE_PERMISSION_CHANNEL ?? 'true') === 'false') {
    return {
      ok: false,
      reason: 'claude_permission_channel_disabled',
      detail: 'HAGENCY_CLAUDE_PERMISSION_CHANNEL=false turns off the only channel by which this '
...
function checkCodex(agentDataDir) {
  ...
  if (!nonEmptyFile(file)) {
    return {
      ...
      detail: `no Codex permission-hook config at ${file}; run \`hagency up\` for this agent, `
 ...
 * requests through their own client-side path — `scripts/hagency-acp-agent.mjs` declines anything
 * that is not a hagency coordination tool — so they have no adapter for this to verify. Returning
