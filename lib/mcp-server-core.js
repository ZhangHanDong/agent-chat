...
server.tool(
  ...
  async ({ assignee, status, priority, label, limit, offset }) => {
    try {
      if (EPHEMERAL_RUNNER) {
        const tasks = await api('POST', '/api/router/session-task', { agent: AGENT_NAME, op: 'list' });
        const filtered = tasks.filter((task) => (!assignee || assignee === '*' || task.assignee === assignee)
          && (!status || status.split(',').includes(task.status)) && (!priority || task.priority === priority)
          && (!label || task.labels?.includes(label)));
        return text(filtered.slice(offset || 0, limit === undefined ? undefined : (offset || 0) + limit));
      }
      const params = new URLSearchParams();
...
server.tool(
  ...
  async ({ id }) => {
    try {
      if (EPHEMERAL_RUNNER) return text(await api('POST', '/api/router/session-task', { agent: AGENT_NAME, op: 'get', id }));
...
server.tool(
  ...
  async ({ id }) => {
    try {
      if (EPHEMERAL_RUNNER) return text(await api('POST', '/api/router/session-task', { agent: AGENT_NAME, op: 'transition', id, status: 'accepted' }));
...
server.tool(
  ...
  async ({ id, status, waiting_reason, waiting_until }) => {
    try {
      ...
      if (EPHEMERAL_RUNNER) return text(await api('POST', '/api/router/session-task', { agent: AGENT_NAME, op: 'transition', id, ...body }));
...
server.tool(
  ...
  async ({ id, text: commentText }) => {
    try {
      if (EPHEMERAL_RUNNER) return text(await api('POST', '/api/router/session-task', { agent: AGENT_NAME, op: 'comment', id, text: commentText }));
...
server.tool(
  ...
  async ({ id, heartbeat, waiting_reason, waiting_until }) => {
    try {
      ...
      if (waiting_reason !== undefined) body.waiting_reason = waiting_reason;
      if (waiting_until !== undefined) body.waiting_until = waiting_until;
      if (EPHEMERAL_RUNNER) return text(await api('POST', '/api/router/session-task', {
        agent: AGENT_NAME, op: 'execution', id, ...body,
      }));
