/**
 * Which tmux sessions is this Hagency install allowed to manage?
 *
 * Hagency talks to an agent by typing into its tmux pane. It has no way to tell
 ...
 *
 *   HAGENCY_SESSION_ALLOWLIST   only these sessions are managed
 *   HAGENCY_SESSION_DENYLIST    these sessions are never managed
 *
 ...
 *
 * @param {object} config
 * @param {string|undefined} config.allowlist raw HAGENCY_SESSION_ALLOWLIST
 * @param {string|undefined} config.denylist  raw HAGENCY_SESSION_DENYLIST
 */
export function createSessionPolicy({ allowlist, denylist } = {}) {
  ...
  const warnings = [];
  if (allowBroken) warnings.push('HAGENCY_SESSION_ALLOWLIST is set but lists no session; refusing all sessions');
  if (denyBroken) warnings.push('HAGENCY_SESSION_DENYLIST is set but lists no session; refusing all sessions');
...
export function sessionPolicyFromEnv(env = process.env) {
  return createSessionPolicy({
    allowlist: env.HAGENCY_SESSION_ALLOWLIST,
    denylist: env.HAGENCY_SESSION_DENYLIST,
  });
}
