...
import { verifyRuntimeApprovalReadiness } from './agent-launch-readiness.js';
...
/**
 * Where an agent's data lives, matching the one convention the rest of the system uses.
 *
 * `bin/hagency-up` sets `DATA_DIR="$RUNTIME_DIR/data/agents"` and the backend's storage adapter
 * composes `agentDataPath(name)` the same way. Recomputed here rather than imported because the
 * adapter is constructed with a `dataDir` this module does not receive — and getting it wrong is
 * visible: the readiness check would report a missing Codex hook config for an agent that has one.
 */
function agentDataDirFor(agentName) {
  const runtimeDir = String(process.env.HAGENCY_RUNTIME_DIR || '').trim();
  const root = runtimeDir || process.cwd();
  return path.join(root, 'data', 'agents', agentName);
}
...
/*
 * `exec` replaces the pane's shell, so when the runtime exits the pane closes.
 *
 * ADR-005 ¶6: the managed launch wrapper "replaces the tmux pane shell, so exiting a runtime closes
 * the pane". `bin/hagency-up` does this — `tmux send-keys "exec <script>"`, asserted by an exact
 * count in tests/mcp-permission-channel.test.js — and this launcher did not, so a supervisor whose
 * runtime exited left a live pane sitting at an interactive shell. That is worse than untidy: the
 * session still EXISTS, so `tmuxSessionExists` reports it and `reconcile` returns `kept`, meaning a
 * supervisor whose runtime died is indistinguishable from one that is running and is never
 * restarted.
 */
function startTmuxSession(sessionName, workspaceDir, command) {
  ...
  execFileSync('tmux', ['send-keys', '-t', `${sessionName}:0.0`, `exec ${command}`, 'C-m'], {
    timeout: 5000, stdio: ['ignore', 'ignore', 'pipe'],
  });
}
...
function buildLaunchSelection(runtimeProfile) {
  ...
  const envFramework = normalizeFramework(process.env.HAGENCY_SUPERVISOR_FRAMEWORK);
  ...
  const reasoning = normalizeOptionalText(selected?.reasoning, 64)
    || normalizeOptionalText(process.env.HAGENCY_SUPERVISOR_REASONING_PROFILE, 64)
    || null;
  const extraArgs = normalizeOptionalText(selected?.extraArgs, 4000)
    || normalizeOptionalText(process.env.HAGENCY_SUPERVISOR_EXTRA_ARGS, 4000)
    || null;
...
function buildLaunchCommand(targetAgent, workspaceDir, selection, runtimeProfile) {
  ...
  const permissionMetadata = selection.framework === 'claude'
    ? [`HAGENCY_AGENT_PERMISSION_MODE=${shellQuote('auto')}`]
    : (selection.framework === 'codex' ? [
      `HAGENCY_AGENT_PERMISSION_LEVEL=${shellQuote('2')}`,
      `HAGENCY_AGENT_SANDBOX_MODE=${shellQuote('workspace-write')}`,
      `HAGENCY_AGENT_APPROVAL_POLICY=${shellQuote('on-request')}`,
    ] : []);
  /*
   * ADR-005 ¶5: an ambient ANTHROPIC_API_KEY must not silently override the authenticated account.
   *
   * Both hagency-up launchers do this — pinned by `launchers_clear_ambient_anthropic_key_without_
   * explicit_profile` — and this one did not, so a key sitting in the operator's shell was inherited
   * by the tmux server and used INSTEAD of the subscription the contributor configured. Silent, and
   * it bills the wrong credential: the seat accounting in ADR-013 assumes an agent draws on the
   * credential home its profile names.
   *
   * `unset` rather than empty: an exported empty string is a configured-but-blank key to some
   * clients, which is a third state nobody wants. Only when the profile supplies no key of its
   * own — an explicit per-agent key is a deliberate choice and is left alone.
   */
  const primaryApiKey = typeof runtimeProfile?.primary?.apiKey === 'string'
    ? runtimeProfile.primary.apiKey.trim()
    : '';
  const anthropicHygiene = (selection.framework === 'claude' && !primaryApiKey)
    ? ['env', '-u', 'ANTHROPIC_API_KEY']
    : [];
  const envPrefix = [
    ...anthropicHygiene,
    `PATH=${shellQuote(process.env.PATH || '')}`,
    `HAGENCY_AGENT_SANDBOX=${shellQuote('1')}`,
    ...permissionMetadata,
    `HAGENCY_API=${shellQuote(process.env.HAGENCY_API || '')}`,
    `HAGENCY_BACKEND_PORT=${shellQuote(process.env.HAGENCY_BACKEND_PORT || '')}`,
    `HAGENCY_SUPERVISED_AGENT_NAME=${shellQuote(targetAgent)}`,
    `HAGENCY_SUPERVISOR_WORKSPACE=${shellQuote(workspaceDir)}`,
    `HAGENCY_SUPERVISOR_PROFILE_SOURCE=${shellQuote(selection.profileSource)}`,
    `HAGENCY_SUPERVISOR_FRAMEWORK=${shellQuote(selection.framework)}`,
    `SUPERVISOR_LLM_PROVIDER=${shellQuote(selection.provider || '')}`,
    `SUPERVISOR_LLM_MODEL=${shellQuote(selection.model || '')}`,
    `HAGENCY_SUPERVISOR_REASONING_PROFILE=${shellQuote(selection.reasoning || '')}`,
    `HAGENCY_SUPERVISOR_EXTRA_ARGS=${shellQuote(selection.extraArgs || '')}`,
    `HAGENCY_RUNTIME_PROFILE_PRIMARY_JSON=${shellQuote(JSON.stringify(runtimeProfile?.primary || null))}`,
    `HAGENCY_RUNTIME_PROFILE_SUPERVISOR_JSON=${shellQuote(JSON.stringify(runtimeProfile?.supervisor || null))}`,
  ].join(' ');
...
export function createSupervisorLifecycleManager({
  ...
  function reconcile(targetAgent, now = Date.now()) {
    ...
    try {
      ...
      /*
       * Can this runtime's permission requests reach a human? Checked HERE, immediately before the
       * session is created, because ADR-005's wording is "verifies every required adapter before it
       * creates tmux" — and a supervisor that is already running healthily is not creating one. An
       * earlier draft put this among the §12 checks, which made a missing artifact report `failed`
       * for a session that was up and working; the test suite caught it by failing the `kept` case.
       *
       * Two launchers honoured this precondition and this one did not, so a supervisor Claude agent
       * ran `--permission-mode auto` in a detached pane with no relay — auto mode then hard-denies
       * or opens a prompt in a pane nobody watches, the state REQ-OWNER-UI-APPROVAL-BACKGROUND
       * forbids by name.
       *
       * Verify-only and shared rather than reimplemented: `bin/hagency-up` ESTABLISHES readiness and
       * needs a TTY to confirm Codex hook trust, which a sweep loop inside the backend can never
       * have. It does not need to — the supervisor launches into an already-provisioned home, so the
       * artifacts either exist or the agent was never provisioned. See lib/agent-launch-readiness.js.
       *
       * Refusing is the point: launching anyway is the forbidden state, and the reason names what to
       * run to fix it.
       */
      const readiness = verifyRuntimeApprovalReadiness({
        framework: selection.framework,
        agentPath: manifest.homeDir,
        stateDir: manifest.stateDir,
        dataDir: agentDataDirFor(supervisorName),
      });
      if (!readiness.ok) {
        return {
          target: targetAgent,
          action: 'failed',
          reason: 'approval-adapter-unready',
          error: `${readiness.reason}: ${readiness.detail}`,
        };
      }
