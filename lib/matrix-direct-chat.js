...
function afterAdmission(event, binding) {
  return (binding?.sinceTs || 0) === 0 || Number.isSafeInteger(event.origin_server_ts) && event.origin_server_ts >= binding.sinceTs;
}
...
export async function ensureDirectDevice({ sender, directory, fetchImpl = fetch }) {
  ...
  const response = await fetchImpl(`${sender.side.apiBaseUrl.replace(/\/+$/, '')}/_matrix/client/v3/login`, {
    ...
    body: JSON.stringify({ type: 'm.login.application_service', identifier: { type: 'm.id.user', user: sender.agentUserId },
      initial_device_display_name: 'Hagency agent private chat' }),
  });
...
export class MatrixDirectChats {
  ...
  async start(sender) {
    ...
    client.agentChatSyncHandler = async (type, roomId, event, error) => {
      ...
      if (type === 'room.failed_decryption' && entry.rooms[roomId]) {
        if (!afterAdmission(event, entry.rooms[roomId])) return;
    ...
    client.onSyncSuccess = async () => {
      ...
      for (const item of entry.pending.list()) {
        if (!entry.rooms[item.roomId] || !afterAdmission(item.event, entry.rooms[item.roomId])) {
          entry.pending.remove(item.eventId); continue;
        }
        try {
  ...
  async readHistory(entry, roomId) {
    ...
    const events = []; const cursors = new Set(); let from = null, truncated = false;
    ...
    for (let page = 0; page < HISTORY_PAGE_LIMIT; page++) {
      ...
      events.push(...(history.chunk || []).filter(event => afterAdmission(event, binding)));
      if (history.chunk?.some(event => Number.isSafeInteger(event.origin_server_ts) && event.origin_server_ts < (binding.sinceTs || 0))) break;
      ...
      truncated = page === HISTORY_PAGE_LIMIT - 1;
    }
    if (truncated) this.warning(`Conversation history in ${roomId} is limited to the latest ${HISTORY_PAGE_LIMIT * 100} events since admission.`);
    const encrypted = [];
    for (const message of events.reverse()) {
      if (message.type === 'm.room.message') await this.message(entry, roomId, message, true);
      else if (message.type === 'm.room.encrypted') encrypted.push({ roomId, event: message });
    }
    if (encrypted.length) entry.pending.putMany(encrypted);
    entry.rooms[roomId].historyPending = false;
    entry.rooms[roomId].historyTruncated = truncated;
  ...
  async verify(entry, binding, senderMxid = null) {
    ...
    if (mode === 'direct') assertPrivateMembers(state, entry.sender.agentUserId, binding.humanMxid);
  ...
  async message(entry, roomId, event, fromHistory = false) {
    ...
    if (!binding || !afterAdmission(event, binding)) return;
  ...
  async send(roomId, content, transactionId, agentName = null, origin = {}) {
    ...
    this.assertReplyScope(entry, roomId, content, origin);
  ...
  assertReplyScope(entry, roomId, content, { sourceCreatedAt } = {}) {
    const binding = entry.rooms[roomId];
    if (binding?.mode !== 'group') return;
    // Relations alone do not prove public provenance: a front-desk session may
    // have no root, and files/activity can also outlive the private conversation.
    const privateRoot = binding.privateRootEventId &&
      [content['m.relates_to']?.event_id, content['m.new_content']?.['m.relates_to']?.event_id]
        .includes(binding.privateRootEventId);
    if (privateRoot || !Number.isSafeInteger(sourceCreatedAt) || sourceCreatedAt < (binding.sinceTs || 0)) {
      const error = new Error('private reply cannot be delivered after room promotion');
      error.code = 'private_reply_scope_changed'; error.permanent = true;
      throw error;
    }
  }
}
