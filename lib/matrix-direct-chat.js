...
import { EncryptedRoomEvent, RustSdkCryptoStorageProvider, SimpleFsStorageProvider } from 'matrix-bot-sdk';
...
export async function ensureDirectDevice({ sender, directory, fetchImpl = fetch }) {
  ...
  if (cached) {
    if (cached.user_id !== sender.agentUserId || cached.baseUrl !== sender.side.apiBaseUrl) throw new Error('direct device identity mismatch');
    return cached;
  }
...
export class MatrixDirectChats {
  constructor({ directory, Client, backend, onMessage, onBinding, warning }) {
  ...
  async start(sender) {
    ...
    const client = new this.Client(sender.side.apiBaseUrl, session.access_token,
      new SimpleFsStorageProvider(path.join(directory, 'sync.json')),
      new RustSdkCryptoStorageProvider(path.join(directory, 'crypto'), 0));
    ...
    client.onSyncSuccess = async () => {
      ...
      for (const item of entry.pending.list()) {
        try {
          const decrypted = await client.crypto.decryptRoomEvent(new EncryptedRoomEvent(item.event), item.roomId);
  ...
  async message(entry, roomId, event, fromHistory = false) {
    ...
    if (!binding || event.sender === entry.sender.agentUserId) return;
