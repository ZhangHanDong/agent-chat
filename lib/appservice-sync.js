...
export async function appserviceSyncOnce({ baseUrl, accessToken, since, timeoutMs = SYNC_TIMEOUT_MS, fetchImpl = fetch }) {
  ...
  for (const [roomId, room] of Object.entries(joins)) {
    ...
    if (room?.timeline?.limited === true) roomsNeedingReconcile.push(roomId);
  }
  const leaves = [];
  ...
  return { nextBatch: body?.next_batch ?? null, timelineEvents, inviteEvents, stateEvents, leaves, roomsNeedingReconcile, initial: !since };
}
  ...
  const loop = (async () => {
    while (!stopped && shouldContinue()) {
      try {
        ...
        /*
         * SUCCESS-CURSOR-ADVANCE RESET (5-r6, R6 first-class). "8 attempts" means EIGHT
         * CONSECUTIVE refusals of ONE batch — it must not accumulate across a successful
         * cursor advance. Every branch below that advances the cursor on success calls
         * resetBatchRetry(), so a NEW batch always starts from attempts 0 and cannot
         * circuit-break early off a previous batch's refusals. The helper also clears
        ...
        if (deliverable.length) {
          ...
          if (handled?.status === 200) {
            ...
            // F07: gap-flagged rooms ride the batch; reconcile is the caller's job
            if (result.roomsNeedingReconcile?.length) {
              try { onRoomsNeedingReconcile?.(side, [...result.roomsNeedingReconcile]); } catch { /* hook must not break delivery */ }
            }
          } else {
            /*
          ...
          // F07: an empty batch can still carry gap flags — reconcile regardless
          if (result.roomsNeedingReconcile?.length) {
            try { onRoomsNeedingReconcile?.(side, [...result.roomsNeedingReconcile]); } catch { /* hook must not break delivery */ }
          }
        } else if (result.nextBatch && skipTimeline) {
          ...
          // F07: even a swallowed initial timeline can flag gaps — reconcile anyway
          if (result.roomsNeedingReconcile?.length) {
            try { onRoomsNeedingReconcile?.(side, [...result.roomsNeedingReconcile]); } catch { /* hook must not break delivery */ }
          }
        }
