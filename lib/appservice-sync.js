 ...
 *
 * WHY THIS EXISTS. The listener needs Hagency to be reachable FROM the homeserver; the edge link
...
export function resolveAppserviceSyncConfig(env = process.env) {
  let side = String(env.HAGENCY_APPSERVICE_SYNC_SIDE ?? '').trim();
  const baseUrl = String(env.HAGENCY_APPSERVICE_SYNC_URL ?? '').trim().replace(/\/+$/, '');
  if (!side && !baseUrl) {
    return { enabled: false, reason: 'HAGENCY_APPSERVICE_SYNC_SIDE is not set, so sync intake is not expected' };
  }
  if (side) {
    /*
    ...
    if (/\/\s*$/.test(side) || /^https?:\/\//i.test(side)) {
      return { enabled: false, reason: `HAGENCY_APPSERVICE_SYNC_SIDE must be a side id, not a URL or slash-suffixed value, got ${side}` };
    }
    side = side.toLowerCase();
  }
  if (!side) return { enabled: false, reason: 'HAGENCY_APPSERVICE_SYNC_URL is set but HAGENCY_APPSERVICE_SYNC_SIDE names no project side' };
  if (!baseUrl) return { enabled: false, reason: 'HAGENCY_APPSERVICE_SYNC_SIDE is set but HAGENCY_APPSERVICE_SYNC_URL names no homeserver' };
  if (!/^https?:\/\//i.test(baseUrl)) {
    return { enabled: false, reason: `HAGENCY_APPSERVICE_SYNC_URL must be an absolute http(s) URL, got ${baseUrl}` };
  }
