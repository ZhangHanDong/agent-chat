...
import { holdsAllocation as holdsCommitment, usesResourcePool } from './resource-allocation-budget.js';

const ROOM_ID_RE = /^![^:\s]+:[^\s]+$/;
const MXID_RE = /^@[^:\s]+:[^\s]+$/;
const STATES = ['pending', 'active', 'ended'];
const ROUTES = ['autoJoin', 'notWhitelisted', 'overOffer', 'overCeiling', 'crossFamilyUnavailable'];
 ...
 *
 * See the call site for why the plain form was not enough. The order is deliberate: what is
 * left, what the limit was, then WHICH of the two competing draws is binding, then the two
 * things that change the answer (raise the preset, or wait for the period).
 */
export function overCommitMessage(e, alloc, remainingTokens, ctx) {
  const head = `allocating ${compactTokens(alloc)} would exceed the ${compactTokens(remainingTokens)} left on ${e.agent}`;
  if (!ctx) return head;

  const parts = [];
  if (Number.isFinite(ctx.ceilingTokens)) parts.push(`its ceiling is ${compactTokens(ctx.ceilingTokens)} per ${ctx.period ?? 'period'}`);
  const committed = Number.isFinite(ctx.reserved) ? ctx.reserved : null;
  const measured = Number.isFinite(ctx.spent) ? ctx.spent : null;
  if (committed !== null && measured !== null) {
    // Naming the binding one removes the guess. `remainingFor` takes the larger of the two.
    const binding = measured > committed ? 'measured spend' : 'committed allocations';
    parts.push(`${compactTokens(committed)} is committed to active engagements and `
      + `${compactTokens(measured)} of fresh tokens was measured this ${ctx.period ?? 'period'}`
      + `${ctx.spendPeriodKey ? ` (${ctx.spendPeriodKey})` : ''}, so ${binding} is what is binding`);
  } else if (committed !== null) {
    parts.push(`${compactTokens(committed)} is committed to active engagements and nothing has been measured`);
  }
  /*
   * The console shows a LARGER consumption figure than the one enforced here, because cache
   * reads are measured and reported but never draw the ceiling. Left unexplained, that
   * discrepancy looks like one of the two numbers being wrong.
   */
  if (Number.isFinite(ctx.consumed) && Number.isFinite(ctx.spent) && ctx.consumed > ctx.spent) {
    parts.push(`total consumption reads ${compactTokens(ctx.consumed)} because cache reads are `
      + 'measured and shown but do not draw against a ceiling');
  }
  const remedy = ctx.presetName
    ? `raise the ceiling on preset "${ctx.presetName}" or wait for the ${ctx.period ?? 'period'} to roll over`
    : `raise the agent's declared ceiling or wait for the ${ctx.period ?? 'period'} to roll over`;
  return `${head} — ${parts.join('; ')}. To proceed, ${remedy}.`;
}
 ...
 *
 * PRD A-R0-1: "repeating the same `request_id` and digest produces the same
 * assignment; a different digest is rejected as a conflict." The digest is the half
 * that matters. Without it a `request_id` becomes an overwrite handle: a caller could
 * reuse an id it had already used and quietly change the amount, and the store would
 * hand back the old engagement as though nothing had been asked.
 *
 * Covers exactly the fields that define the ask. `project` is deliberately excluded —
 * it is the room's display label, it can be renamed, and a rename is not a different
 * request. `agent` is excluded too: which agent would serve the role is the provider's
 * answer, not part of the question.
 *
 * JSON.stringify of an array, for the same reason seat-store uses it: canonical for
 * these values and impossible to collide by choosing a separator that appears inside
 * one of them.
 */
function requestDigest({ projectRoomId, role, requester, requestedTokens, ratePerDay, requestContext }) {
  return createHash('sha256').update(JSON.stringify([
    projectRoomId, role, requester, requestedTokens,
    ratePerDay === null || ratePerDay === undefined ? null : ratePerDay,
 ...
 */
export function routeRequest({ request, whitelisted, offer, remainingTokens, activeForRole = 0, crossFamilyOk = true }) {
  if (!whitelisted) return { route: 'notWhitelisted', autoJoin: false };
  if (!crossFamilyOk) return { route: 'crossFamilyUnavailable', autoJoin: false };
  /*
...
export function createEngagementStore({ load, persist, now = () => Date.now() } = {}) {
  ...
  /**
   * The active engagements whose room belongs to this project side — the single definition behind both
   * `committedForProjectSide` (the total) and `commitmentsForProjectSide` (the breakdown).
   *
   * A local function rather than a method, so neither caller depends on `this`: these are read off a
   * destructured store in places, and a `this`-bound helper would break there in a way tests that call
   * `store.x()` would never see.
   */
  function commitmentsFor(serverNameValue) {
    const target = String(serverNameValue || '').trim().toLowerCase();
    if (!target) return [];
    return Object.values(engagements).filter((e) => {
      if (!holdsCommitment(e)) return false;
      const room = typeof e.projectRoomId === 'string' ? e.projectRoomId : '';
      const at = room.indexOf(':');
      return at > 0 && room.slice(at + 1).toLowerCase() === target;
    });
  }

  function replayRequest(input) {
    if (input.requestId === null || input.requestId === undefined) return null;
    const key = text(input.requestId, 'requestId', 256);
    const prior = Object.values(engagements).find((e) => e.requestId === key);
    if (!prior) return null;
    const digest = requestDigest({ ...input, projectRoomId: text(input.projectRoomId, 'projectRoomId'),
      role: String(input.role ?? '').toLowerCase(), ratePerDay: input.ratePerDay ?? null });
    if (prior.requestDigest !== digest) throw new EngagementError('conflict', `requestId ${key} was already used for a different request`);
    return prior;
  }

  return {
    replayRequest,
    selectAgent(engagementId, agent, allocationChoice, by) {
      const e = engagements[engagementId];
      if (!e || e.state !== 'pending' || e.fulfillment) throw new EngagementError('conflict', 'Assignment is already reserved');
      const snapshot = { ...e };
      e.agent = text(agent, 'agent');
      e.allocationChoice = allocationChoice;
      const unrecord = record('engagement.agent_selected', { engagementId, agent: e.agent, allocationChoice, by });
      commit(() => { engagements[engagementId] = snapshot; unrecord(); }, 'Agent selection');
      return e;
    },
    planFulfillment(engagementId, plan) {
      const e = engagements[engagementId];
      if (!e || e.state !== 'pending') throw new EngagementError('conflict', 'engagement is not pending');
      if (e.fulfillment && e.fulfillment.phase !== 'failed') return e;
      const snapshot = { ...e };
      e.agent = text(plan.agent, 'agent');
      e.allocatedTokens = posInt(plan.allocatedTokens, 'allocatedTokens');
      e.fulfillment = { ...plan, phase: 'reserved', error: null, updatedAt: now() };
      const unrecord = record('engagement.fulfillment_reserved', { engagementId, agent: e.agent });
      commit(() => { engagements[engagementId] = snapshot; unrecord(); }, 'fulfillment reservation');
      return e;
    },
    setFulfillmentPhase(engagementId, phase, error = null) {
      const e = engagements[engagementId];
      if (!e?.fulfillment) throw new EngagementError('not_found', 'fulfillment plan not found');
      if (!['reserved', 'provisioned', 'identified', 'bound', 'joined', 'starting', 'complete', 'failed'].includes(phase)) {
        throw new EngagementError('bad_request', 'unknown fulfillment phase');
      }
      const snapshot = { ...e };
      e.fulfillment = { ...e.fulfillment, phase, error: error ? String(error).slice(0, 400) : null, updatedAt: now() };
      const unrecord = record(`engagement.fulfillment_${phase}`, { engagementId, agent: e.agent });
      commit(() => { engagements[engagementId] = snapshot; unrecord(); }, 'fulfillment phase');
      return e;
    },
    // ── whitelist ──────────────────────────────────────────────────────
    /**
     ...
     */
    createRequest({
      project, projectRoomId, role, requester, requestedTokens, ratePerDay, agent, offer,
      remainingTokens, requestId, allowAutoJoin = true, deferActivation = false, crossFamilyOk = true,
      requestContext = null,
    }) {
      const room = text(projectRoomId, 'projectRoomId');
      if (!ROOM_ID_RE.test(room)) throw new EngagementError('bad_request', 'projectRoomId must be a Matrix room id');
      ...
      /*
       * IDEMPOTENCY, PER PRD A-R0-1 — AND THE SHARED IDENTIFIER IS THE MATRIX EVENT ID.
       *
       ...
       * `!request coding 400000 20000`, and a human will not type a UUID, so the store
       * had no key and simply allowed the duplicate.
       *
       * But the two sides already share a stable identifier: the Matrix event id. The
       * bridge passes it as `requestId`, so the human path gets idempotency without
       * anyone typing anything, and a programmatic caller supplies its own.
       *
       * `requestId` is OPTIONAL and its absence is recorded rather than papered over.
       * Requiring it would break every existing caller, and a store that silently
       * invents a key gives no idempotency while appearing to have some — the
       * engagement carries `idempotent: false` so a reader can tell which it is.
       */
      const key = requestId === null || requestId === undefined ? null : text(requestId, 'requestId', 256);
      ...
      if (key) {
        const prior = Object.values(engagements).find((e) => e.requestId === key);
        if (prior) {
          // Same ask, already accepted: hand back what exists. Creating a second one
          // is the defect this exists to prevent.
          if (prior.requestDigest === digest) return prior;
          /*
           * Same id, different ask. Refused rather than merged or overwritten: merging
           * would answer a question nobody asked, and overwriting would let a caller
           * reuse an id to quietly change an amount already under review.
           */
          throw new EngagementError('conflict',
            `requestId ${key} was already used for a different request`);
        }
      }
      ...
      const req = {
        ...
        ...(requestContext ? { requestContext: structuredClone(requestContext) } : {}),
        // Stated, not implied: without a key this request could not have been deduped.
      ...
      const decision = routeRequest({
        request: req,
        whitelisted: allowAutoJoin && Boolean(whitelist[room]),
        ...
        activeForRole: Object.values(engagements)
          .filter((e) => e.role === req.role && holdsCommitment(e)).length,
      });
      const engagement = {
        ...req,
        route: decision.route,
        autoJoined: decision.autoJoin && !deferActivation,
        state: decision.autoJoin && !deferActivation ? 'active' : 'pending',
        ...
        allocatedTokens: decision.autoJoin && !deferActivation ? req.requestedTokens : null,
        decidedAt: decision.autoJoin && !deferActivation ? now() : null,
        decidedBy: decision.autoJoin && !deferActivation ? 'auto' : null,
        endedAt: null,
        endedReason: null,
      };
      engagements[engagement.id] = engagement;
      const unrecord = record(engagement.autoJoined ? 'engagement.auto_joined' : 'engagement.pending', {
     ...
     */
    decide({ engagementId, approve, allocatedTokens, remainingTokens, by, reason, spendContext = null, autoJoin = false, notify = false }) {
      ...
      if (alloc > remainingTokens) {
        /*
         * A REFUSAL MUST NAME WHAT DREW THE CEILING DOWN.
         *
         * "allocating 50000 would exceed the 0 left on BigLittle" is true and useless: on an
         * agent with one small engagement it reads as a bug in the arithmetic. It became the
         * whole story the moment metering started working, because measured spend began
         * drawing ceilings down for the first time and an operator's only clue was a zero.
         *
         * Two numbers compete for the ceiling — what is committed to active engagements, and
         * what was measured this period — and `remainingFor` takes the LARGER. Which one is
         * binding is the first thing anyone needs to know, and it is not inferable from the
         * result. So the message states both, names the preset to raise, and says the period
         * the measurement belongs to.
         *
         * `spendContext` is optional: a caller that has no ledger still gets the plain form
         * rather than a crash or a fabricated breakdown.
         */
        throw new EngagementError('over_commit', overCommitMessage(e, alloc, remainingTokens, spendContext));
      }
      e.state = 'active';
      e.autoJoined = autoJoin;
      if (e.fulfillment) e.fulfillment = { ...e.fulfillment, phase: 'complete', error: null, updatedAt: now() };
      e.allocatedTokens = alloc;
      e.decidedAt = now();
      e.decidedBy = by ?? 'operator';
      if (notify && !autoJoin) e.approvalNotice = { state: 'pending' };
    ...
    // An explicit verdict replay can recover a pre-notification approval. Do not
    // backfill every historical engagement when the service is upgraded.
    queueApprovalNotice(engagementId) {
      const e = engagements[engagementId];
      if (!e || e.state !== 'active' || !e.bound || e.approvalNotice) return e;
      const snapshot = { ...e };
      e.approvalNotice = { state: 'pending' };
      commit(() => { engagements[engagementId] = snapshot; }, 'approval notice');
      return e;
    },

    settleApprovalNotice(engagementId, { eventId = null, code = null }) {
      const e = engagements[engagementId];
      if (!e || e.approvalNotice?.state !== 'pending') return e;
      const snapshot = { ...e };
      e.approvalNotice = eventId
        ? { state: 'delivered', eventId, deliveredAt: now() }
        : { state: 'failed', code: code || 'delivery_unavailable' };
      commit(() => { engagements[engagementId] = snapshot; }, 'approval notice outcome');
      return e;
    },
    ...
    beginWithdrawal(engagementId, scope = 'room') {
      const e = engagements[engagementId];
      if (!e || e.state !== 'ended' || !(e.allocatedTokens > 0)) return e;
      const snapshot = { ...e };
      e.withdrawal = { ...e.withdrawal, state: 'pending', scope };
      commit(() => { engagements[engagementId] = snapshot; }, 'withdrawal started');
      return e;
    },

    setWithdrawalOutcome(engagementId, { binding, roomWithdrawal, retirement }) {
      const e = engagements[engagementId];
      if (!e || e.state !== 'ended' || !e.withdrawal) return e;
      const snapshot = { ...e };
      const failed = binding?.reason && !binding.keptBound || roomWithdrawal?.left === false || retirement?.state === 'failed';
      e.withdrawal = {
        scope: retirement ? 'agent' : e.withdrawal.scope ?? 'room',
        state: failed ? 'failed' : binding?.keptBound ? 'retained' : 'complete',
        reason: failed ? String(retirement?.error || binding?.reason || roomWithdrawal?.reason || 'Room departure failed').slice(0, 256) : null,
        roomWithdrawal: roomWithdrawal ?? null,
        ...(retirement ? { retirement } : {}),
        checkedAt: now(),
      };
      if (!binding?.reason || binding.keptBound) e.bound = binding?.keptBound === true;
      commit(() => { engagements[engagementId] = snapshot; }, 'withdrawal outcome');
      return e;
    },
    ...
    committedFor(agentName) {
      return Object.values(engagements)
        .filter((e) => e.agent === agentName && holdsCommitment(e))
        .reduce((n, e) => n + (e.allocatedTokens ?? 0), 0);
    },
    /**
     * What a PROJECT SIDE has committed — the same sum, grouped by homeserver instead of by agent.
     *
     * ADR-016's settled question 2: the operator chose a real per-side allocation over a slice of the
     * deployment total, because a slice cannot distinguish "this project exceeded its allocation" from
     * "the deployment is out of budget", and those two sentences have different remedies.
     *
     * THE SIDE IS DERIVED, NOT STORED. A room id is `!opaque:origin-server`, so the server that owns
     * the room is already on every engagement — adding a `projectSide` field would be a second copy of
     * a fact that can disagree with the first.
     *
     * COMMITMENT ONLY, NOT MEASURED SPEND, and that is a limitation rather than a simplification.
     * `remainingFor(agent)` uses `max(committed, measured)` because metering attributes consumption
     * per agent; attributing it per SIDE would need each agent to record which side it serves, which
     * ADR-016 decision 4 introduces and this does not. So a side's figure is what has been promised,
     * which is exactly what admission control needs — and it will understate real consumption until
     * minted agents carry their side.
     */
    committedForProjectSide(serverNameValue, { legacyOnly = false } = {}) {
      return commitmentsFor(serverNameValue).filter(e => !legacyOnly || !usesResourcePool(e))
        .reduce((n, e) => n + (e.allocatedTokens ?? 0), 0);
    },
    /**
     * WHICH engagements make up that number, so a side's commitment can be attributed rather than only
     * totalled.
     *
     * IT EXISTS BECAUSE THE TOTAL ALONE IS UNANSWERABLE. An operator looking at `已承诺 200k` beside a
     * project with nobody assigned had no way to ask what the 200k was: finding out meant fetching the
     * engagement list, fetching the agent list, and cross-referencing them by hand. Three of those four
     * commitments turned out to belong to agents that had been deleted hours earlier.
     *
     * ONE DEFINITION, DELIBERATELY. `committedForProjectSide` sums exactly this list rather than repeating
     * the filter, because a breakdown that can disagree with its own total is worse than no breakdown —
     * it makes the number look explained when it is not.
     */
    commitmentsForProjectSide(serverNameValue) {
      return commitmentsFor(serverNameValue).map((e) => ({
        id: e.id,
        agent: e.agent ?? null,
        role: e.role ?? null,
        project: e.project ?? null,
        allocatedTokens: e.allocatedTokens ?? 0,
        projectRoomId: e.projectRoomId ?? null,
      }));
    },
    listAudit({ limit = 200 } = {}) {
