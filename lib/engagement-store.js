 ...
 * one of them.
 */
function requestDigest({ projectRoomId, role, requester, requestedTokens, ratePerDay }) {
...
export function createEngagementStore({ load, persist, now = () => Date.now() } = {}) {
  ...
  return {
     ...
     */
    decide({ engagementId, approve, allocatedTokens, remainingTokens, by, reason, spendContext = null, autoJoin = false }) {
     ...
     */
    committedForProjectSide(serverNameValue) {
      return commitmentsFor(serverNameValue).reduce((n, e) => n + (e.allocatedTokens ?? 0), 0);
    },
    /**
