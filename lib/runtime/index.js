...
//
// Hagency reaches its agents through a terminal multiplexer: it launches them in
...
//   killSession(session)       -> boolean         (false = nothing was there to stop)
...
export function createNullRuntime({ reason = 'no agent runtime available' } = {}) {
  return Object.freeze({
    ...
    async killSession() { return false; },
...
export function assertRuntimeContract(runtime, label = 'runtime') {
  ...
  for (const method of [
    'isAvailable', 'sessionExists', 'killSession', 'listPanes', 'capturePane', 'sendKeys',
    'isEmptyServerError',
  ]) {
