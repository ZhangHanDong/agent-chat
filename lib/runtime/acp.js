 ...
 *
 * Hagency's other runtime drives agents by typing into a tmux pane and reading
...
export function createAcpRuntime({
  ...
  function handleAgentRequest(entry, message) {
    ...
    respond(entry, id, {
      error: { code: -32601, message: `hagency does not implement ${method}` },
    });
  }
  ...
  return Object.freeze({
     ...
     * Hagency's coordination tools (check_inbox, send_message, ...) — the same set
     ...
     * Hagency reading "TokyoThe command exited with code 7…" — "Tokyo" was the
