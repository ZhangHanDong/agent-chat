...
export function resolveAgentTokenMode(env = process.env) {
  const configuredMode = (env.HAGENCY_AGENT_TOKEN_MODE || 'audit').trim().toLowerCase() || 'audit';
 ...
 *
 *   - `bin/hagency-up` says so in its own comment: "the launcher sends only the operator bearer, so a
 ...
 *     an existing agent at all under `HAGENCY_AGENT_TOKEN_MODE=hard`.
...
export function buildServerCredentialReadiness({ env = process.env } = {}) {
  ...
  const serverTokenConfigured = Boolean(normalizeSecret(env.HAGENCY_SERVER_TOKEN));
  return {
