...
import { KINDS, CEILING_KINDS } from './parsers.js';
...
const PERIODS_PER_AGENT = Math.max(
  2,
  Number.parseInt(process.env.HAGENCY_LEDGER_PERIODS || '14', 10) || 14,
);

/** Per-agent session detail beyond this is folded into `retired`. */
const SESSIONS_PER_AGENT = Math.max(
  10,
  Number.parseInt(process.env.HAGENCY_LEDGER_SESSIONS_PER_AGENT || '500', 10) || 500,
);
...
/** Everything measured, for DISPLAY. All four kinds. */
const totalOf = (t) => KINDS.reduce((n, k) => n + int(t?.[k]), 0);
/**
 * The part that draws against a ceiling, for ENFORCEMENT. Fresh tokens only.
 *
 * These two must stay distinct at every layer. `totalOf` fed enforcement directly, so a
 * ceiling was drawn down by cache reads at parity — and `parsers.js`, in the same
 * subsystem, states that folding cache reads in is "a five-order-of-magnitude error, not a
 * rounding one". The module that established the separation was undone by the one that
 * consumed it, and the symptom was an agent locked out at 136% of a ceiling it had used 7%
 * of. See CEILING_KINDS for the ruling.
 */
const drawnOf = (t) => CEILING_KINDS.reduce((n, k) => n + int(t?.[k]), 0);
  ...
  return {
    ...
    totalsFor(name) {
      ...
      return {
        ...
        totals: acc,
        total: totalOf(acc),
        // What a ceiling would draw against, alongside what was consumed.
        drawn: drawnOf(acc),
    ...
    currentPeriod(name, period = 'monthly', at = now()) {
      ...
      return {
        period: granularity,
        key: periodKey(at, granularity),
        totals: bucket,
        total: totalOf(bucket),
        // Enforcement reads THIS. `total` is for showing a contributor what was consumed.
        drawn: drawnOf(bucket),
      };
    },
