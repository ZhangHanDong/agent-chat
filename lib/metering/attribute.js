 ...
 * which agent hagency started in it. Splitting them would be a guess, so the total is
...
export function agentTranscriptWorkspace(agent) {
  // A managed runner's configured workdir is a search key, not an observation
  // that a process is alive there or that usage has actually been recorded.
  return [agent?.lastWorkspacePath, agent?.workspacePath,
    ...(agent?.runner?.mode === 'on-demand' ? [agent?.workdir, agent?.homeDir] : [])]
...
export async function meterAgent({ agent, homeDir, readSessions }) {
   ...
   */
  const workspace = agentTranscriptWorkspace(agent);
  if (!workspace) {
    return {
 ...
 * the directory it ran in, not which agent hagency started there, so two agents in one
