...
import { agentTranscriptWorkspace, meterAgent, summarizeFleet } from './attribute.js';
...
export const WINDOW_MS = int(process.env.HAGENCY_METERING_WINDOW_MS, 30 * 24 * 60 * 60 * 1000);
/** How many candidate files one agent's scan may open. */
export const MAX_FILES = int(process.env.HAGENCY_METERING_MAX_FILES, 200);
...
export const MAX_BYTES = int(process.env.HAGENCY_METERING_MAX_BYTES, 8 * 1024 * 1024);
/** How long a computed figure stays fresh. */
export const CACHE_TTL_MS = int(process.env.HAGENCY_METERING_CACHE_MS, 60_000);
...
function fleetKey(agents) {
  return JSON.stringify(agents
    .map((a) => [a?.name ?? '', a?.type ?? '', agentTranscriptWorkspace(a) ?? ''])
