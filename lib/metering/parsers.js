 ...
 * nothing else.
 *
 * WHY READ FILES RATHER THAN PROXY THE PROVIDER. Hagency launches a CLI that talks to
 * its provider directly, so no API response passes through Hagency to read a figure
 * from. The alternative to reading transcripts is standing a proxy in front of every
 * agent, which only works in api-key mode, puts prompts and completions through Hagency
...
/**
 * The kinds a CEILING draws against. Operator ruling, 2026-08-12.
 *
 * `cacheRead` is measured, reported, and never drawn. Not a discount and not an omission —
 * a ceiling that counts cache reads at parity is not a ceiling. The evidence is in this
 * file's own header: one real session recorded 4,800,089,833 cache reads against 19,765
 * fresh input tokens. Any ceiling a contributor would plausibly declare is exhausted by a
 * single context-heavy session, so enforcement against a parity total refuses work for a
 * reason that has nothing to do with how much work was done.
 *
 * That is not hypothetical. It happened the moment metering started working: an agent with
 * 681,089 fresh tokens against a 10,000,000 ceiling was locked out at 13,609,601 — 95% of
 * which was cache reads.
 *
 * Weighting the kinds instead would need prices, and ADR-013's 2026-08-10 amendment
 * withdrew pricing: the unit of account is the token. So the choice is which tokens, and
 * these are they.
 *
 * KEPT SEPARATE FROM `KINDS` ON PURPOSE. Consumption reporting must still show all four —
 * a contributor lending an agent is entitled to see the cache-read volume even though it
 * does not spend their ceiling. Code that totals for DISPLAY uses KINDS; code that totals
 * for ENFORCEMENT uses this. Conflating them is the defect this constant exists to prevent.
 */
export const CEILING_KINDS = ['input', 'output', 'cacheWrite'];
