 ...
 * registration. Hagency's receiver has always lived inside the bridge, so that url had to point at
 * Hagency — which for a fleet on a laptop or an internal network means exposing it. The operator said
 ...
 * `127.0.0.1`. Hagency dials THIS process — outbound, from behind whatever NAT it lives behind, over the
 ...
 *   Hagency    ──▶ this, long-poll               outbound: NAT blocks new connections IN, not data
 ...
 * Hagency, wait to be told they were processed, answer. Every judgement stays on the private side, which
 ...
 * Hagency says it processed them. Nothing is queued, nothing is persisted, and nothing needs to be:
 * unacknowledged work is simply work the homeserver has not been told about, and it will send it again.
 */

import { createHash, timingSafeEqual } from 'crypto';

/** How long the homeserver's request is held while waiting for Hagency, before giving it a retryable 500. */
export const DEFAULT_DELIVER_TIMEOUT_MS = 30_000;
/** How long Hagency's long-poll waits for work before being told to come back. */
...
export function createAppserviceEdge({
  ...
  linkToken,
  /*
   * THE ADDRESS THE HOMESERVER MUST DIAL, which is NOT the address Hagency collects from — and conflating
   ...
   * (`http://69.194.3.128:8097`, how Hagency reaches this edge) while this process, bound to loopback, was
  ...
  function fromHagency({ headers }) {
    const auth = headers?.['x-hagency-link'] ?? headers?.['X-Hagency-Link'];
  ...
  async function handle({ method, path, query = {}, headers = {}, body = null } = {}) {
    ...
    if (txnMatch && method === 'PUT') {
      ...
      return new Promise((resolve) => {
        const timer = setTimeoutImpl(() => {
          ...
          resolve({
            status: 500,
            body: { errcode: 'M_UNKNOWN', error: 'Hagency did not acknowledge in time; retry' },
          });
        }, deliverTimeoutMs);
     ...
     * Hagency's namespace claims users that already exist — the appservice does not create accounts on
    ...
    // ── Hagency's side, reached by Hagency dialling out ──────────────────
    if (path === '/_hagency/edge/pull' && method === 'GET') {
      if (!fromHagency({ headers })) return { status: 403, body: { error: 'bad link token' } };
      traffic.hagencyLastSeenAt = now();
      if (waitingPoller) {
        /*
    ...
    if (path === '/_hagency/edge/ack' && method === 'POST') {
      if (!fromHagency({ headers })) return { status: 403, body: { error: 'bad link token' } };
      traffic.hagencyLastSeenAt = now();
      ...
      /*
       * MATCHED ON THE TRANSACTION ID. An ack for the wrong one would release a transaction Hagency has
      ...
      if (ok) {
        ...
        /*
         * Hagency FAILED to process, and says so. Passing that through as a 500 is what makes the
         * homeserver retry — the same contract the in-process receiver keeps when `onEvents` throws.
         */
        pending.resolve({
          status: 500,
          body: { errcode: 'M_UNKNOWN', error: 'Hagency failed to process this transaction' },
        });
      }
      return { status: 200, body: { ok: true } };
    }

    if (path === '/_hagency/edge/status' && method === 'GET') {
      /*
      ...
      return {
        ...
        body: {
          ...traffic,
          inFlight: inFlight ? { txnId: inFlight.txnId, takenByHagency: Boolean(inFlight.takenAt) } : null,
          hagencyWaiting: Boolean(waitingPoller),
          ...
          registrationUrl,
          /*
           * WHEN THIS PROCESS STARTED, so "the homeserver has never called" can be told apart from "the
           * homeserver has not called in the ninety seconds since I restarted". Without it, every deploy
           * makes a healthy fleet show an inbound warning — and a warning that fires on healthy systems is
           * one an operator learns to ignore, which costs more than it saves.
           */
          startedAt,
        },
      };
