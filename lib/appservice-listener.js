 ...
 * (`PUT /_matrix/app/v1/transactions/{txnId}`). Hagency lends agents, so the intuition is that the
...
export function resolveAppserviceListenerConfig(env = process.env) {
  ...
  if (!rawPort) return { enabled: false, reason: 'HAGENCY_APPSERVICE_PORT is not set' };
  /*
  ...
  if (!Number.isInteger(port) || port < 1 || port > 65535) {
    return { enabled: false, reason: `HAGENCY_APPSERVICE_PORT is not a usable port: ${rawPort}` };
  }
  const host = String(env.HAGENCY_APPSERVICE_BIND ?? '').trim() || DEFAULT_APPSERVICE_BIND;
  return {
    enabled: true,
    port,
    host,
...
export async function startAppserviceListener({ receiver, port, host = DEFAULT_APPSERVICE_BIND, logger = console } = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    server.listen(port, host, () => {
      ...
      logger.log?.(`[appservice] listening on http://${host}:${address.port} (homeserver → Hagency)`);
      resolve({
        port: address.port,
        host,
