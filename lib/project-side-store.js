...
function credential(value) {
  ...
  if (kind === 'appservice') {
    ...
    if (value.transport !== undefined) {
      ...
      if ([value.asToken, value.hsToken].includes(transport.token)) throw new ProjectSideStoreError('bad_request', 'outbound credentials must be independent');
    }
    return {
      kind,
      ...(transport ? { transport } : {}),
...
export class ProjectSideStore {
   ...
   *
   * WHY STAGING EXISTS. Issuing replaced the live credential immediately, which broke HAFleet's own outbound
   ...
   * correctly said `rejected`, and the operator was handed a repair job for a state HAFleet had created.
   *
