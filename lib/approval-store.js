...
export class ApprovalStore {
  ...
  _installMutationGuards() {
    const names = [
      ...
      'migrateMarkerRoomsV2',
      'upsertProjectionPublisher',
    ];
  ...
  _refreshBindingMarkers(agent) {
    ...
    for (const scope of Object.values(this.state.markerRoomScopes)) {
      if (!(scope.associations || []).some((entry) => entry.agent === agent)) continue;
      try {
        this._advanceRoomMarker(scope);
      } catch (error) {
        if (!(error instanceof ApprovalStoreError) || error.code !== 'conflict') throw error;
      }
    }
  }
  ...
  _advanceRoomMarker(scope, force = false) {
    ...
    for (const row of this.state.markerOutbox) {
      if (row.approvalRoomId === scope.approvalRoomId && !row.eventId) row.superseded = true;
    }
  ...
  syncBindingMarker(input = {}) {
    ...
    if (!publisher || publisher.publisherMxid !== publisherMxid
  ...
  migrateMarkerRoomsV2(options = {}) {
    ...
    const rooms = [...new Set(Object.values(this.state.markerScopes)
      ...
      .sort(compareText)
      .filter((room) => !migration.cursor || compareText(room, migration.cursor) > 0);
    ...
    for (const approvalRoomId of page) {
      ...
      migration.cursor = approvalRoomId;
    }
    const wasComplete = migration.complete === true;
    migration.complete = page.length === rooms.length;
    if (page.length || migration.complete !== wasComplete) this._save();
    return { examined: page.length, complete: migration.complete, cursor: migration.cursor };
  }
  ...
  _queueV1Retirement(v2Row) {
    ...
    if (existing) return;
    const identity = `${v2Row.approvalRoomId}\0${v2Row.bindingGeneration}\0${MARKER_RETIREMENT_CHANNEL}`;
    this.state.markerOutbox.push({
      identity,
      casToken: createHash('sha256').update(identity).digest('hex'),
      ...
      publisherScope: v2Row.publisherScope,
