...
export class ApprovalStore {
  ...
  _migrateLegacy() {
    ...
    const pending = Object.values(this.state.requests).filter((record) => !record.projectionRevision);
    let count = 0;
    for (const record of pending.slice(0, this.migrationBatchSize)) {
      record.projectionRevision = 1;
      record.projectionMigration = 'legacy_v1';
      if (record.ownerDmRoomId) this._enqueueProjection(record, 'private_status', record.status, 'legacy_v1');
      count += 1;
    }
    const remaining = Object.values(this.state.requests).filter((record) => !record.projectionRevision);
    this.state.version = remaining.length ? 1 : STORE_VERSION;
    this.state.migration = { cursor: remaining[0]?.id || null };
    this._migrationChanged = count > 0 || !remaining.length;
    return count;
  }
  ...
  _enqueueProjection(record, channel, state = record.status, migrationKind = 'native_v2') {
    ...
    this.state.projectionOutbox.push({ identity, casToken: createHash('sha256').update(identity).digest('hex'), requestId: record.id,
      revision, channel, state, targetRoomId: channel === 'public_notice' ? record.projectRoomId : record.ownerDmRoomId,
  ...
  sweepExpired(options = {}) {
    ...
    const now = this.now(); let scanned = 0; let expired = 0;
    for (const record of Object.values(this.state.requests)) {
  ...
  listDueProjections(options = {}) {
    const limit = Math.min(Math.max(Number(options.limit) || 100, 1), 200);
    return (this.state.projectionOutbox || []).filter((r) => !r.eventId).slice(0, limit).map((r) => ({
      request_id: r.requestId, revision: r.revision, channel: r.channel, state: r.state,
      target_room_id: r.targetRoomId, migration_kind: r.migrationKind, cas_token: r.casToken,
      plan: r.plan ? { ...r.plan, cas_token: r.casToken, attempt_state: r.attemptState } : null,
    }));
  }

  prepareProjection(casToken, input) {
    const row = this.state.projectionOutbox.find((r) => r.casToken === casToken && !r.eventId);
    if (!row) throw new ApprovalStoreError('conflict', 'projection row changed');
    if (!row.plan) {
      const payload = input?.prepared_payload;
      if (Buffer.byteLength(JSON.stringify(payload ?? null)) > PROJECTION_PAYLOAD_MAX_BYTES) throw new ApprovalStoreError('bad_request', 'prepared payload too large');
      const publisher = fullMxid(input?.publisher_mxid, 'publisher_mxid');
      const generation = text(input?.credential_generation, 'credential_generation', 255);
      const txn = `hafleet_${createHash('sha256').update(`${row.identity}\0${publisher}\0${generation}`).digest('hex').slice(0, 32)}`;
      row.plan = { publisher_mxid: publisher, homeserver: text(input?.homeserver, 'homeserver', 255), credential_kind: text(input?.credential_kind, 'credential_kind', 64), credential_generation: generation, transaction_id: txn, payload_version: Number(input?.payload_version) || 1, prepared_event_type: text(input?.prepared_event_type, 'prepared_event_type', 255), prepared_payload: structuredClone(payload) };
      row.attemptState = 'ready'; this._save();
    }
    return { plan: { ...structuredClone(row.plan), cas_token: row.casToken, attempt_state: row.attemptState } };
  }

  beginProjectionSend(casToken) {
    const row = this.state.projectionOutbox.find((r) => r.casToken === casToken && r.plan && !r.eventId);
    if (!row) throw new ApprovalStoreError('conflict', 'projection plan changed');
    if (row.attemptState === 'ready') { row.attemptState = 'attempted'; this._save(); }
    return { plan: { ...structuredClone(row.plan), cas_token: row.casToken, attempt_state: row.attemptState } };
  }

  receiptProjection(casToken, input) {
    const row = this.state.projectionOutbox.find((r) => r.casToken === casToken && r.plan && !r.eventId);
    if (!row || row.attemptState !== 'attempted' || input?.publisher_mxid !== row.plan.publisher_mxid) throw new ApprovalStoreError('conflict', 'projection receipt mismatch');
    row.eventId = text(input?.event_id, 'event_id', 255); this._save(); return { event_id: row.eventId };
  }

  retryProjection(casToken, input = {}) {
    const row = this.state.projectionOutbox.find((r) => r.casToken === casToken && r.plan && !r.eventId);
    if (!row) throw new ApprovalStoreError('conflict', 'projection plan changed');
    row.attemptState = 'uncertain'; row.nextAttemptAt = Math.max(this.now(), Number(input.retry_at) || this.now()); row.lastErrorCode = optionalText(input.error_code, 128); this._save();
    return { attempt_state: row.attemptState };
  }
