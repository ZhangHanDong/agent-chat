 ...
 * window_activity counts a blinking cursor as activity. Reminders (`hagency reminder`) are scheduled
 ...
 * queue was the one live service on that port: `hagency send` posts to it and the backend pushes
...
function isBackendNotificationEntry(entry) {
  if (!entry || entry.from !== 'hagency-backend') return false;
...
// Accept queued message from hagency-send
 ...
 * pushing directly cannot drift from what `hagency send` gets.
 *
