 ...
 * customer asks." (`docs/design/hagency-as-pdu.md:239`)
 *
 ...
 * (`bin/hagency-up:1577`, `scripts/configure-v1-subconscious.js:19`).
 *
    ...
    allows(endpoint) {
      ...
      return {
        ...
        reason: `${host} is not an approved memory-export destination; full session `
          ...
          + 'HAGENCY_MEMORY_EXPORT_ALLOWED_HOSTS to approve it.',
      };
    },
  };
}
 ...
 *
 * `HAGENCY_MEMORY_EXPORT === '1'` to enable at all — the repo's default-off form
...
export function memoryExportPolicyFromEnv(env = process.env) {
  const enabled = String(env.HAGENCY_MEMORY_EXPORT ?? '').trim() === '1';
  const policy = createMemoryExportPolicy({
    allowed: env.HAGENCY_MEMORY_EXPORT_ALLOWED_HOSTS ?? '',
    allowLocal: String(env.HAGENCY_MEMORY_EXPORT_ALLOW_LOCAL ?? '1').trim() !== '0',
  });
  return {
    enabled,
    policy,
    ...
    allows(endpoint) {
      if (!enabled) {
        return {
          ok: false,
          reason: 'memory export is off; set HAGENCY_MEMORY_EXPORT=1 to enable it and '
            + 'HAGENCY_MEMORY_EXPORT_ALLOWED_HOSTS to say where transcripts may go',
        };
      }
