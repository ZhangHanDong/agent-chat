...
export const FLEET_PROBE_EVENT = 'com.hagency.connection.probe.v1';
export const FLEET_REQUEST_EVENT = 'com.hagency.engagement.request.v1';
...
export async function verifyFleetTarget(context, side, readRoom) {
  const [membership, powers, binding, rules, encryption] = await Promise.all([
    ...
    readRoom(side, context.targetRoomId, `state/com.hagency.admin.binding.v1/${encodeURIComponent(side.fleetId)}`),
    ...
    async handle({ sideId, registration, method, path, body = {}, generation }) {
      try {
        ...
        if (method === 'POST' && path === '/api/fleet/v1/requests') {
          ...
          // Display metadata comes from the verified target, never the caller's
          // label. Keep it outside the immutable request/replay context.
          let projectName = null;
          try {
            const named = await readRoom(side, context.targetRoomId, 'state/m.room.name/', { optional: true });
            if (typeof named?.name === 'string') projectName = named.name.trim().slice(0, 255) || null;
          } catch { /* A missing label must not change the verified request's authority. */ }
          const result = await backend({ action: 'request', sideId, registration, context,
            ...(projectName ? { projectName } : {}), ...(transportGeneration ? { transportGeneration } : {}) });
