...
/** No administrator credential crosses to Hagency. */
export async function retirePalpoAgent({ engagement, mxid, transport, localStopped, fetchImpl = fetch }) {
  ...
  const response = await fetchImpl(`${link.url}/retire-agent`, {
    ...
    headers: { Authorization: `Bearer ${link.token}`, 'X-Hagency-Generation': String(link.generation), 'Content-Type': 'application/json' },
