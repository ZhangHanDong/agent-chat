...
export function defaultAgentchatHomeDir(env = process.env) {
  // Explicit HAGENCY_HOMEDIR always wins.
  const explicit = normalizeAbsolutePath(env?.HAGENCY_HOMEDIR);
  if (explicit) return explicit;
  // v1.1: when HAGENCY_RUNTIME_DIR is set, place homes under it for dev/live isolation.
  const runtimeDir = normalizeAbsolutePath(env?.HAGENCY_RUNTIME_DIR);
  if (runtimeDir) return path.join(runtimeDir, 'homes');
  return path.resolve(os.homedir(), '.hagency');
}
...
export function allAgentHomeRoots(env = process.env) {
  ...
  const legacy = path.resolve(os.homedir(), '.hagency');
  const roots = [primary];
