...
export function createAppserviceReceiver({
   ...
   */
  async function handle({ method, path, query = {}, headers = {}, body = null, now = null } = {}) {
    ...
    if (txnMatch && method === 'PUT') {
      ...
      try {
        /*
         * F06: the intake mode rides alongside the events (set by the edge puller and the sync
         * collector; a homeserver push omits it and defaults to 'push' at the bridge's adapter
         * wiring). It is transport context, never event data — the receiver does not interpret it.
         */
        await onEvents(events, { txnId, mode: typeof body?.mode === 'string' ? body.mode : undefined });
      } catch (error) {
        /*
