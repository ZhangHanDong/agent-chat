...
export function createMatrixWorkStore({ load, persist, now = Date.now }) {
  ...
  return {
    enqueue({ action, agent, sideId, localpart, roomId = null, mxid = null, engagementId = null, content = null }) {
      const existing = rows.find((r) => r.action === action && r.agent === agent
        && r.sideId === sideId && r.roomId === roomId
        && (action === 'engagement-approved' ? r.engagementId === engagementId : r.state !== 'complete'));
      if (existing) return structuredClone(existing);
      return mutate(() => {
        const row = { id: randomUUID(), action, agent, sideId, localpart, roomId, mxid, engagementId,
          ...(action === 'engagement-approved' ? { content: structuredClone(content) } : {}),
    ...
    claim() {
      const row = rows.find((r) => r.state === 'pending' && (!r.retryAt || r.retryAt <= now())
        || r.state === 'running' && r.leaseUntil <= now());
      if (!row) return null;
      return mutate(() => {
    ...
    complete(id, claimToken, outcome, { retry = true } = {}) {
      ...
      return mutate(() => {
        const eventId = typeof outcome.eventId === 'string' && /^\$[^\s]{1,511}$/.test(outcome.eventId)
          ? outcome.eventId : null;
        if (row.action === 'engagement-approved' && retry && outcome.permanent !== true && (outcome.ok !== true || !eventId)) {
          row.state = 'pending';
          row.retryAt = now() + 30_000;
          row.lastError = typeof outcome.code === 'string' ? outcome.code.slice(0, 80) : 'delivery_unconfirmed';
          return row;
        }
        row.state = 'complete';
        ...
        if (row.action === 'engagement-approved' && eventId) row.outcome.eventId = eventId;
        row.completedAt = now();
        return row;
      });
    },
