...
export const AGENT_OPS_CLIENT_SCHEMA = 'com.hagency.agent_ops.v1';
...
export function normalizeAgentOpsLoopbackOrigin(value) {
  ...
  try {
    ...
    throw new Error('HAGENCY_AGENT_OPS_LOOPBACK_ORIGIN must be an absolute loopback URL');
  }
  if (url.protocol !== 'http:' || !LOOPBACK_HOSTS.has(url.hostname) || !url.port || url.username || url.password) {
    throw new Error('HAGENCY_AGENT_OPS_LOOPBACK_ORIGIN must be http on explicit 127.0.0.1 or [::1] port');
  }
  if (url.pathname !== '/' || url.search || url.hash) {
    throw new Error('HAGENCY_AGENT_OPS_LOOPBACK_ORIGIN must not contain path, query, or fragment');
  }
  return url.origin;
}
