...
export default class BotCommands {
  ...
  async sendInto(roomId, content) {
    if (typeof this.bridge?.sayInRoom === 'function') {
      await this.bridge.sayInRoom(roomId, content);
      return;
    }
  ...
  async cmdRequest(roomId, args = [], senderId = '', context = {}) {
     ...
     *
     * `this.botClient`, not `this.client` — the constructor stores it under the
     * former, so the first version silently fell through its own catch and labelled
     * every engagement with the room id. Harmless-looking, but it made the
     * contributor's queue unreadable: every row said `!rVe3sBZn…:palpo.test` where a
     * project name belongs, and the id was already shown beside it.
     */
    let project = roomId;
    try {
      const name = await this.botClient?.getRoomStateEvent?.(roomId, 'm.room.name', '');
      if (name?.name) project = String(name.name).slice(0, 200);
