...
const DEFAULT_BACKEND_PORT = Number.isFinite(DEFAULT_BACKEND_PORT_RAW) && DEFAULT_BACKEND_PORT_RAW > 0
  ...
  : 8090;
const BACKEND_URL = process.env.HAFLEET_API || `http://127.0.0.1:${DEFAULT_BACKEND_PORT}`;
...
const COMMAND_TIERS = {
   ...
   * definition not an operator of this hafleet. Requiring operator rights would
...
async function api(method, path, body) {
   ...
   */
  const requesterToken = (process.env.HAFLEET_REQUESTER_TOKEN || '').trim();
...
export default class BotCommands {
  ...
  async handleCommand(roomId, senderId, text, context = {}) {
    ...
    if (this.tier0Only && tier > 0) {
      ...
      await this.reply(roomId, `${command} is unavailable: this HAFleet is running without a Matrix bot, so only `
    ...
    if (!auth.ok) {
      ...
      await this.reply(roomId, auth.reason === 'acl_unconfigured'
        ? `Access denied: ${command} is privileged and no operator ACL is configured on this `
          + 'HAFleet. Set MATRIX_OPERATOR_MXIDS (or MATRIX_ADMIN_MXIDS) to the humans who may run it.'
  ...
  async reply(roomId, plain, html) {
     ...
     * had HAFleet and the customer on ONE homeserver, where the bot is in every room.
     *
  ...
  async cmdRequest(roomId, args = [], senderId = '', context = {}) {
    ...
    if (e.autoJoined) {
      ...
      return this.reply(roomId,
         ...
         * own environment variables (`HAFLEET_OWNER_MXID`, `HAFLEET_OWNER_DM_ROOM`) and
