...
const DEFAULT_BACKEND_PORT = Number.isFinite(DEFAULT_BACKEND_PORT_RAW) && DEFAULT_BACKEND_PORT_RAW > 0
  ...
  : 8090;
const BACKEND_URL = process.env.HAGENCY_API || `http://127.0.0.1:${DEFAULT_BACKEND_PORT}`;
...
const COMMAND_TIERS = {
  '!help': 0,
  /*
   * TIER 0 ON PURPOSE. `!request` is the project side asking a contributor for
   * capacity — the one inbound action in the whole design, and the asker is by
   * definition not an operator of this hagency. Requiring operator rights would
   * mean only the contributor could ask themselves.
   *
   * It grants nothing. The request lands as `pending` unless the room is already
   * whitelisted AND inside a published offer AND inside the agent's ceiling, and
   * every one of those is the contributor's own prior decision.
   */
  '!request': 0,
  /*
   * TIER 0, for exactly the reason `!request` is. `!offer` is the project side asking
   * "what could I ask for" — the read that should precede the write. Requiring operator
   * rights would mean only the contributor could enquire of themselves, and a borrower
   * who cannot look first has to discover the caps by having a request refused.
   *
   * It reads a projection built for this purpose (`GET /api/offer-book`): published
   * offers only, this room's own whitelist state, and no ceilings. Nothing it returns is
   * a decision, and nothing it returns is another project's.
   */
  '!offer': 0,
  '!status': 1, '!agents': 1, '!groups': 1, '!group': 1, '!agent': 1,
  '!sessions': 1, '!mcp': 1, '!bridge': 1,
  '!mkgroup': 2, '!bindroom': 2, '!addmember': 2, '!rmember': 2, '!joingroup': 2,
...
/*
 * AN UNCONFIGURED ACL FAILS CLOSED.
 *
 * This used to end with "if no ACL configured at all, allow everything (backward
 * compat)", and both lists default to empty — so a stock deployment handed EVERY
 * sender whose message the bridge processed full tier-3 authority: `!agentctl`,
 * `!spy`, `!ctl`. Any Matrix user who could get into a room with the bot could drive
 * the agents. The permissive branch sat below the specific ones, so it only fired
 * when nothing was configured, which is exactly the case nobody tests.
 *
 * Tier 0 is unaffected. `!help` and `!request` are public by design — `!request` is
 * the project side asking, and the asker is by definition not an operator here — so
 * an unconfigured deployment still does the thing this console exists for. What it
 * no longer does is grant agent control to strangers by default.
 *
 * MATRIX_ALLOW_UNCONFIGURED_ACL=true restores the old behaviour for anyone who was
 * relying on it. It has to be asked for, because "I never set the variable" should
 * not be the way a deployment ends up open.
 */
const ALLOW_UNCONFIGURED_ACL = String(process.env.MATRIX_ALLOW_UNCONFIGURED_ACL || '')
...
function authorizeCommand(senderMxid, tier) {
  ...
  const unconfigured = MATRIX_OPERATOR_MXIDS.size === 0 && MATRIX_ADMIN_MXIDS.size === 0;
  if (unconfigured) {
    if (ALLOW_UNCONFIGURED_ACL) return { ok: true, reason: 'no_acl' };
    return { ok: false, reason: 'acl_unconfigured' };
  }
  return { ok: false, reason: tier >= 3 ? 'admin_required' : 'operator_required' };
}

function isAgentControlBlockedContext(context = {}) {
  return Boolean(context.groupName || context.approvalRoom);
}
...
async function api(method, path, body) {
  ...
  /*
   * The engagement endpoint is bearer-authenticated, not bridge-secret
   * authenticated, and deliberately so: submitting a request is a project-side act
   * with its own credential, separate from the contributor's. Without this the
   * `!request` command reached the backend and was refused by its own bridge.
   *
   * The REQUESTER token is preferred over the operator token where both exist, so
   * the bridge asks with the narrowest credential that can do the job — it can
   * submit and nothing else.
   */
  const requesterToken = (process.env.HAGENCY_REQUESTER_TOKEN || '').trim();
  const apiToken = (process.env.API_TOKEN || '').trim();
  const bearer = requesterToken || apiToken;
  if (bearer) opts.headers.Authorization = `Bearer ${bearer}`;
  // These room/event IDs are taken from the authenticated Matrix callback.
  // The shared submit-only token alone cannot attest a caller-claimed room.
  if ((path === '/api/engagements' || path.startsWith('/api/offer-book?')) && process.env.MATRIX_BRIDGE_SECRET) {
    opts.headers['X-Bridge-Secret'] = process.env.MATRIX_BRIDGE_SECRET;
  }
  if (body) {
...
export default class BotCommands {
  constructor({ botClient, bridge, botUserId, tier0Only = false }) {
    this.botClient = botClient;
    this.bridge = bridge;
    this.botUserId = botUserId;
    this.replyContext = new AsyncLocalStorage();
    /*
     * A bot-less bridge has no bot side: no SSE room creation, no bot DMs, no member management.
     * Tier-2 commands would half-succeed there — `!mkgroup` writes the backend group and answers
     * "the room will be created", and nothing ever creates it. So the degraded bridge constructs
     * this handler tier0Only: the public commands (the customer's whole inbound surface) work,
     * every privileged command is refused before any mutation with a reason that names the fix.
     */
    this.tier0Only = Boolean(tier0Only);
  }
  ...
  async handle(roomId, senderId, text, context = {}) {
    // Scope every response to its input event, including replies after awaits.
    // A shared mutable current-event field would mix concurrent room commands.
    const eventId = typeof context.eventId === 'string' && context.eventId
      ? context.eventId : randomUUID();
    return this.replyContext.run({ eventId, nextReply: 0, targetAgent: context.targetAgent ?? null,
      receivedAt: context.receivedAt ?? null },
      () => this.handleCommand(roomId, senderId, text, context));
  }

  async handleCommand(roomId, senderId, text, context = {}) {
    ...
    // Project rooms and UI-only approval rooms never expose generic terminal
    // control, even to a global administrator. This prevents !ctl key/send/status
    // from becoming a side door around owner-scoped approval.
    if ((command === '!ctl' || command === '!agentctl') && isAgentControlBlockedContext(context)) {
      console.warn(`[acl] Denied ${command} in non-control room ${roomId} for ${senderId}`);
      await this.reply(roomId, 'Access denied: agent terminal control is unavailable in project and approval rooms.');
      return;
    }

    // Command ACL check (5.8.2)
    const tier = classifyCommand(command);
    if (this.tier0Only && tier > 0) {
      console.warn(`[acl] Refused ${command} (tier ${tier}) for ${senderId}: bridge is running without a bot`);
      await this.reply(roomId, `${command} is unavailable: this Hagency is running without a Matrix bot, so only `
        + 'public commands (!help, !offer, !request) work here. Configure MATRIX_BOT_PASSWORD to enable the rest.');
      return;
    }
    const auth = authorizeCommand(senderId, tier);
    if (!auth.ok) {
      console.warn(`[acl] Denied ${command} (tier ${tier}) for ${senderId}: ${auth.reason}`);
      // The unconfigured case gets its own message: "requires operator privileges" is
      // unactionable when the operator list is empty and nobody can ever satisfy it.
      await this.reply(roomId, auth.reason === 'acl_unconfigured'
        ? `Access denied: ${command} is privileged and no operator ACL is configured on this `
          + 'Hagency. Set MATRIX_OPERATOR_MXIDS (or MATRIX_ADMIN_MXIDS) to the humans who may run it.'
        : `Access denied: ${command} requires ${auth.reason === 'admin_required' ? 'admin' : 'operator'} privileges.`);
      return;
    }
    ...
    try {
      switch (command) {
        ...
        case '!request':  return await this.cmdRequest(roomId, args, senderId, context);
        case '!offer':    return await this.cmdOffer(roomId);
        ...
        case '!mkgroup':  return await this.cmdMkgroup(roomId, args, humanName);
        case '!bindroom': return await this.cmdBindroom(roomId, args);
      ...
      if (e?.commandReplyFailure) throw e;
  ...
  async reply(roomId, plain, html) {
    ...
    /*
     * THROUGH THE BRIDGE, so the answer comes from whoever is actually in the room.
     *
     * This was `this.botClient.sendMessage` unconditionally, and on a project side the bot is not a member:
     * `!offer` from a customer's room was dispatched, handled, and its answer died with `M_FORBIDDEN:
     * sender's membership is not 'join'`. The customer saw nothing at all. Every deployment this had run on
     * had Hagency and the customer on ONE homeserver, where the bot is in every room.
     *
     * `sayInRoom` picks the identity from the room's own server. The bot remains the sender for our rooms.
     */
    await this.sendInto(roomId, content);
  }

  /**
   * The command layer's ONE exit to Matrix, so the identity question is answered in a single place.
   *
   * `reply` used to call `botClient.sendMessage` directly and so did the `!dm` nudge — two sites, one bug.
   * Routing both through here is what makes a third site impossible to add by accident, and a test asserts
   * that only the fallback below remains.
   */
  async sendInto(roomId, content) {
    if (typeof this.bridge?.sayInRoom === 'function') {
      const context = this.replyContext.getStore();
      // Include content so a successful retry is not suppressed by an earlier
      // error response occupying the same reply position.
      const txnSeed = context
        ? JSON.stringify(['command-reply', context.eventId, roomId, context.nextReply++, content])
        : null;
      await this.bridge.sayInRoom(roomId, content, { txnSeed,
        ...(context?.targetAgent ? { targetAgent: context.targetAgent } : {}),
  ...
  /*
   * `!request <role> <tokens> [rate-per-day]` — the inbound half of L3.
   *
   * THE ROOM ID IS NOT A PARAMETER. It is where the message was sent, taken from
   * Matrix, so a project cannot claim to be a room it is not in. That matters more
   * than it looks: the whitelist keys on `projectRoomId`, so a body-supplied room id
   * would let any caller assert someone else's trusted room and auto-join on it.
   * The same reasoning is why the whitelist keys on the room rather than a display
   * name, which is spoofable by renaming.
   *
   * The sender's MXID becomes the requester, again from Matrix rather than the body.
   *
   * Nothing here decides anything. The backend routes the request; the contributor
   * approves it in the console or it auto-joins under terms they published earlier.
   */
  /**
   * `!offer` — what this contributor has published, and what would serve it.
   *
   * The read that should precede `!request`. Before the transparency ruling there was
   * nothing sensible to answer: a list of role names with caps, and the one fact a borrower
   * most needs — what actually serves the role — withheld by policy. Now the answer is
   * useful, so the command exists.
   *
   * The room id comes from the ROOM, never from arguments, for the same reason it does in
   * `!request`: the whitelist keys on the authenticated `projectRoomId`, so a room supplied
   * in message content must not decide what a caller is told about their own trust state.
   * The command therefore takes no arguments at all.
   */
  async cmdOffer(roomId) {
    const book = await api('GET', `/api/offer-book?projectRoomId=${encodeURIComponent(roomId)}`);
    if (book?.error) return this.reply(roomId, `Cannot read the offer book: ${book.error}`);
    const roles = Array.isArray(book?.roles) ? book.roles : [];
    if (!roles.length) {
      /*
       * Distinguished from an error on purpose. "Nothing published" is a real and common
       * state — a contributor who has configured capacity but not advertised it — and
       * reporting it as a failure would send a project to ask why the bot is broken.
       */
      return this.reply(roomId, 'This contributor has nothing on offer right now.');
    }

    const lines = roles.map((r) => {
      const s = r.serving;
      // The transparency ruling, in the place a borrower reads: framework, model, level, tier.
      const by = s?.model
        ? `${s.framework ?? '?'} · ${s.model}${s.reasoning ? ` (${s.reasoning})` : ''}`
          + `${s.tier ? ` · ${s.tier}` : ''}`
          + `${s.provisioningRequired ? ' · new agent required' : ''}`
        /*
         * A published role with nothing able to serve it is worth SAYING rather than
         * omitting: the project can stop waiting and ask for something else, and the
         * contributor probably wants to know their offer is unfillable.
         */
        : 'nothing currently qualifies';
      const caps = [
        r.budgetCapPerEngagement ? `up to ${r.budgetCapPerEngagement} tokens` : null,
        r.rateCap ? `${r.rateCap}/day` : null,
        r.count !== null && r.count !== undefined
          ? `${r.runningNow ?? 0} of ${r.count} running`
          : null,
      ].filter(Boolean).join(', ');
      return `  ${r.role} — ${by}${caps ? `\n      ${caps}` : ''}`;
    });

    /*
     * Whether YOUR request auto-joins. The most actionable line in the reply, and the reason
     * the endpoint takes a room at all — `null` means no room was identified, which is not
     * the same as "not trusted", so it is not reported as a refusal.
     */
    const trust = book.whitelisted === true
      ? 'This room is whitelisted: a request inside the caps above joins automatically.'
      : book.whitelisted === false
        ? 'This room is not whitelisted, so a request waits for the contributor to decide.'
        : null;

    return this.reply(roomId, [
  ...
  async cmdRequest(roomId, args = [], senderId = '', context = {}) {
    const [role, tokensRaw, rateRaw] = args;
    if (!role || !tokensRaw) {
      return this.reply(roomId,
        'Usage: !request <role> <tokens> [tokens-per-day]\n'
        + 'Example: !request architect 400000 20000');
    }
    const requestedTokens = Number(String(tokensRaw).replace(/[_,]/g, ''));
    if (!Number.isFinite(requestedTokens) || requestedTokens <= 0) {
      return this.reply(roomId, `Not a token amount: ${tokensRaw}`);
    }
    const ratePerDay = rateRaw ? Number(String(rateRaw).replace(/[_,]/g, '')) : null;

    /*
     * The room's own name is the project LABEL; the id is the identity.
     *
     * The bridge reads with the recorded side representative when its home bot
     * is absent from the project. The label never changes the room's identity or
     * authority; an unreadable/unnamed room keeps its authenticated opaque ID.
     */
    let project = roomId;
    try {
      const name = this.bridge?.roomNameOf
        ? await this.bridge.roomNameOf(roomId)
        : (await this.botClient?.getRoomStateEvent?.(roomId, 'm.room.name', ''))?.name;
      if (name) project = String(name).slice(0, 200);
    } catch { /* an unnamed room is fine: the id still identifies it */ }

    const result = await api('POST', '/api/engagements', {
      project,
      projectRoomId: roomId,
      role: String(role).toLowerCase(),
      requester: senderId,
      requestedTokens: Math.floor(requestedTokens),
      ...(Number.isFinite(ratePerDay) && ratePerDay > 0 ? { ratePerDay: Math.floor(ratePerDay) } : {}),
      /*
       * THE SHARED IDENTIFIER BOTH SIDES ALREADY HAVE.
       *
       * PRD A-R0-1 wants a `request_id` agreed between requester and provider, and the
       * interface is a human typing `!request coding 400000 20000` — nobody is going to
       * type a UUID. The Matrix event id is already stable, already visible to both
       * sides, and already the thing the sender cannot forge, so it IS the request id.
       *
       * Taken from the event, never from the message body, for the same reason the room
       * id is: a body-supplied key would let a sender claim someone else's request.
       * Absent only when a caller drives the handler directly, and the store records
       * that the request could not be deduped rather than inventing a key.
       */
      ...(context?.eventId ? { requestId: context.eventId } : {}),
    });

    if (result?.error) {
      return this.reply(roomId, `Request refused: ${result.error}`);
    }
    const e = result?.engagement;
    if (!e) return this.reply(roomId, 'Request failed: no engagement returned.');

    if (e.autoJoined) {
      const bound = result.binding?.bound;
      /*
       * WHAT THE PROJECT IS TOLD IT GOT.
       *
       * Operator ruling 2026-08-11: the coding agent and its model are transparent to the
       * borrower, not hidden. This line was already naming the agent — and this deployment's
       * names encode the framework (`claude-agent`, `octos-agent`) — so it was leaking the
       * identity while withholding the one fact a borrower can act on. Now it states the
       * configuration outright.
       *
       * The reasoning level is included when known because the same model at a different
       * level qualifies at a different tier, so a model name alone is not the answer.
       */
      const s = result.serving;
      const config = s?.model
        ? `${s.framework ?? '?'} · ${s.model}${s.reasoning ? ` (${s.reasoning})` : ''}`
          + `${s.tier ? ` · ${s.tier}` : ''}`
        : null;
      return this.reply(roomId,
        `Joined automatically as ${e.role} — ${e.allocatedTokens} tokens, served by ${e.agent}`
        + (config ? ` running ${config}.` : '.')
        /*
         * The failure is reported WITHOUT the remedy. `binding.error` names the provider's
         * own environment variables (`HAGENCY_OWNER_MXID`, `HAGENCY_OWNER_DM_ROOM`) and
         * posting that into a project room tells a borrower about the provider's deployment
         * — which is on the private side of the transparency line under any policy, and was
         * wrong under the previous one too. The provider sees the full reason on their own
         * surfaces, where it is actionable; the project sees only that the attach did not
         * happen, which is all they can act on.
         */
        + (bound === false
    ...
    /*
     * WHY it is waiting, in the project's own terms. "Pending" alone tells them
     * nothing they can act on; "not whitelisted" and "over the published cap" are
     * different situations with different next steps.
     */
    const why = {
      notWhitelisted: 'this room is not on the contributor\'s whitelist',
      overOffer: 'the amount is above what they have published',
      overCeiling: 'the amount is above what the serving agent has left',
    }[e.route] || 'it needs a decision';
    return this.reply(roomId, `Requested ${e.role} for ${e.requestedTokens} tokens — awaiting a decision, because ${why}.`);
  }
  ...
  async cmdHelp(roomId) {
    ...
    const html = [
      ...
      '<code>!bindroom &lt;group&gt;</code> — Bind THIS room to an existing group<br>',
  ...
  async cmdMkgroup(roomId, args, humanName) {
    if (args.length < 1) {
      await this.reply(roomId, 'Usage: !mkgroup <name> [member1] [member2] ...');
      return;
    }
    const name = args[0];
    const initiator = String(humanName || '').trim();
    const initiatorKey = initiator.toLowerCase();
    const seenMembers = new Set();
    const members = args.slice(1)
      .map(member => String(member || '').trim())
      .filter((member) => {
        const key = member.toLowerCase();
        if (!key || key === initiatorKey || seenMembers.has(key)) return false;
        seenMembers.add(key);
        return true;
      });
    if (initiator) members.push(initiator);
    ...
    /*
     * SAY THAT A SEPARATE ROOM IS BEING MADE, because the operator cannot see it happen.
     *
     * `!mkgroup` and `!bindroom` are ALTERNATIVES — the comment on `cmdBindroom` below says so
     * outright — but nothing told anybody, so the obvious-looking sequence is to run both: create the
     * group here, then bind this room to it. That leaves two rooms claiming one group. The group's own
     * room is created asynchronously off the backend's SSE echo, so it does not exist yet at this reply
     * and cannot be named; what CAN be said is that it is coming, and what to type instead if this room
     * was meant to be the group's room.
     *
     * Walked on the rig: `!mkgroup X` then `!bindroom X` in the same room left the group pointing at an
     * auto-created room nobody was in, twice out of five attempts — and the failure is silent. Every
     * one-argument command in the operator's own room then answers `Usage:`, because the room it is
     * standing in is not the group's room any more.
     */
    const tail = 'A separate Matrix room is being created for it. If you meant THIS room to be the '
      + `group's room, run: !bindroom ${name}`;
  ...
  // Bind THIS existing room to an existing backend group. The multi-instance
  // counterpart of !mkgroup: in a shared room, each member's bridge binds the
  // SAME Matrix room to its OWN group — no room creation, no group creation.
  async cmdBindroom(roomId, args) {
    const name = args[0];
    if (!name) {
      await this.reply(roomId, 'Usage: !bindroom <group> — bind THIS room to an existing group');
      return;
    }
    const group = await api('GET', `/api/groups/${encodeURIComponent(name)}`);
    if (group.error) {
    ...
    const prev = this.bridge.groupForRoom?.(roomId) || null;
    /*
     * THE ROOM THAT LOSES IS TOLD ABOUT TOO — or rather, the operator is told it exists.
     *
     * A group holds exactly one room, so binding here silently unbinds whatever room held it before.
     * This reported only the gaining side: "Room bound to group X", with no hint that another room just
     * stopped being X's room. When that other room is the one `!mkgroup` created moments earlier, the
     * operator ends up with an orphan they never knew was made; when it is a real project room, they
     * have just moved a conversation and nobody said so.
     */
    const prevRoom = this.bridge.groupRoomMap?.[name] || null;
    const stolen = prevRoom && prevRoom !== roomId ? prevRoom : null;
    this.bridge.bindRoom(roomId, name);
    const text = (prev && prev !== name
      ? `Room rebound: ${prev} → ${name}`
      : `Room bound to group "${name}"`)
      + (stolen ? `\nNote: ${name} was bound to ${stolen}; that room is no longer the group's room.` : '');
    await this.reply(roomId, text,
      (prev && prev !== name
        ? `Room rebound: <code>${escHtml(prev)}</code> → <b>${escHtml(name)}</b>`
        : `Room bound to group <b>${escHtml(name)}</b>`)
        + (stolen ? `<br>Note: <b>${escHtml(name)}</b> was bound to <code>${escHtml(stolen)}</code>; `
          + 'that room is no longer the group\'s room.' : ''));
  }
  ...
  async cmdDm(roomId, args, humanName) {
    ...
    if (dmRoomId) {
      ...
      if (dmResult.humanStatus === 'joined' || dmResult.humanStatus === 'unknown') {
        /*
         * THE SAME RULE AS `reply`, and this is the second site of one defect. A DM room can be on a project
         * side too — `ensureHumanDmRoom` creates it wherever the human lives — and there the bot is not a
         * member, so this nudge would die with `M_FORBIDDEN` exactly as `!offer`'s answer did.
         */
        await this.sendInto(dmRoomId, {
          msgtype: 'm.text',
...
export { resetBotCommandsTestHooks, setBotCommandsTestHooks, classifyCommand, authorizeCommand, isAgentControlBlockedContext, COMMAND_TIERS };
