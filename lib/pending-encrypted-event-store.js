...
export class PendingEncryptedEventStore {
  ...
  putMany(inputs) {
    const records = inputs.map(input => normalizeRecord({ ...input, receivedAt: input?.receivedAt ?? this.now() }));
    const previous = new Map(this.records);
    try {
      let changed = false;
      for (const record of records) {
        if (this.records.has(record.eventId)) continue;
        if (!changed) this.prune({ persist: false });
        if (this.records.size >= this.maxEntries) throw new Error(`pending encrypted event capacity exceeded (${this.maxEntries})`);
        this.records.set(record.eventId, record); changed = true;
      }
      if (changed) this._save();
      return records.map(record => this.records.get(record.eventId));
