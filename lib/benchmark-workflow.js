...
export const BENCHMARK_RUNTIME_SCHEMA = 'hagency.benchmark.runtime-root/v1';
export const BENCHMARK_PROFILE_SCHEMA = 'hagency.benchmark.profile-version/v1';
export const BENCHMARK_RUN_SCHEMA = 'hagency.benchmark.run/v1';
export const BENCHMARK_TRIAL_SCHEMA = 'hagency.benchmark.trial/v1';
...
export function defaultBenchmarkRuntimeRoot(env = process.env) {
  const explicit = normalizeText(env?.HAGENCY_BENCH_RUNTIME_DIR, 4096);
  if (explicit) return path.resolve(explicit);
  return path.resolve(os.homedir(), '.hagency', 'bench-runtime');
}
...
export function ensureBenchmarkRuntimeRoot(rootPath, env = process.env) {
  ...
  if (!existsSync(layout.markerPath)) {
    writeJson(layout.markerPath, {
      ...
      notes: 'Benchmark runtime root isolated from HAGENCY_RUNTIME_DIR and live/human-operated agents.',
    });
  }
  return layout;
}
...
export function buildBenchmarkTrialPaths(rootPath, runId, trialId, trialAgentName, trialAgentId) {
  ...
  const v1Paths = buildV1AgentPaths(trialAgentName, { ...process.env, HAGENCY_HOMEDIR: runPaths.homesRoot }, trialAgentId);
  return {
    ...runPaths,
    trialRoot,
