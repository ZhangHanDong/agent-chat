 ...
 * `409 another poller is already waiting`, and HAFleet is one puller per edge, so that other poller is
...
export function resolveEdgeLinkConfig(env = process.env) {
  ...
  let side = String(env.HAFLEET_EDGE_SIDE ?? '').trim();
  /*
  ...
  if (side) {
    if (/\/\s*$/.test(side) || /^https?:\/\//i.test(side)) {
      return { enabled: false, reason: `HAFLEET_EDGE_SIDE must be a side id, not a URL or slash-suffixed value, got ${side}` };
    }
    ...
    while (!stopped && shouldContinue()) {
      try {
        ...
        const result = await router.handle({
          ...
          /*
           * F06: intake mode rides the body's non-event field — the router passes request.body
           * through to the receiver's onEvents meta, and the bridge's adapter wiring stamps the
           * full provenance from it. 'edge' distinguishes the co-located pull from a pushed txn.
           */
          body: { events, mode: 'edge' },
        });
