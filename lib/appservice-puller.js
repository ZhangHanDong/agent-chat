/*
 * Hagency's half of the co-located appservice: dial out, collect, acknowledge.
 *
...
/*
 * A POLLER CONFLICT IS OUR OWN PREVIOUS SELF, and it clears on a clock we know.
 *
 * The edge allows one waiting poller so two cannot race for one transaction. It learns a poller is gone
 * when the long poll times out — it holds a promise, not a socket, so a bridge killed mid-poll leaves the
 * slot occupied for the rest of the edge's poll timeout (25s by default). Every restart therefore meets
 * `409 another poller is already waiting`, and Hagency is one puller per edge, so that other poller is
 * always the instance that just died.
 *
 * Backing off exponentially made a bounded 25s wait into up to ~55s of blindness: 1+2+5+10s of climb and
 * then 30s sleeps that can begin just before the slot frees. Observed on a live rig during a deploy.
 *
 * So a 409 retries at a FIXED short interval instead — the wait is already bounded by the far end, and
 * adding our own growing delay to it only extends the window in which a customer's message is not being
 * collected. It is still logged, because "the previous instance is still holding the slot" and "the edge
 * is wedged" look identical from outside and only one of them clears itself.
 */
const POLLER_CONFLICT_RETRY_MS = 1000;

export function resolveEdgeLinkConfig(env = process.env) {
  const url = String(env.HAGENCY_EDGE_URL ?? '').trim().replace(/\/+$/, '');
  const token = String(env.HAGENCY_EDGE_LINK_TOKEN ?? '').trim();
  let side = String(env.HAGENCY_EDGE_SIDE ?? '').trim();
  /*
   * NORMALIZED LIKE THE SYNC SIDE, for the same reason the sync side is: the intake mutex
   * compares edge and sync side ids, and `Side-A` here plus `side-a` there is one side
   * wearing two spellings — the mutex would wave it through and the side would be served
   * twice. URL-shaped or slash-suffixed values are refused; a side id is an identifier.
   */
  if (side) {
    if (/\/\s*$/.test(side) || /^https?:\/\//i.test(side)) {
      return { enabled: false, reason: `HAGENCY_EDGE_SIDE must be a side id, not a URL or slash-suffixed value, got ${side}` };
    }
    side = side.toLowerCase();
  }
  if (!side) return { enabled: false, reason: 'HAGENCY_EDGE_URL is set but HAGENCY_EDGE_SIDE names no project side' };
  if (!url && !token) {
    return { enabled: false, reason: 'HAGENCY_EDGE_URL is not set, so no co-located appservice is expected' };
  }
  /*
   ...
   */
  if (!url) return { enabled: false, reason: 'HAGENCY_EDGE_LINK_TOKEN is set but HAGENCY_EDGE_URL is not' };
  if (!token) return { enabled: false, reason: 'HAGENCY_EDGE_URL is set but HAGENCY_EDGE_LINK_TOKEN is not' };
  if (/^https?:\/\//i.test(url)) {
    // fallthrough: url already validated as absolute
  } else {
    return { enabled: false, reason: `HAGENCY_EDGE_URL must be an absolute http(s) URL, got ${url}` };
  }
...
export function startEdgePuller({
  ...
  router,
  side = null,
  /** Returns the hs_token for the side this edge serves, from Hagency's own store. */
  hsTokenFor,
  fetchImpl = fetch,
  logger = console,
  ...
  async function acknowledge(txnId, ok) {
    const res = await fetchImpl(`${url}/_hagency/edge/ack`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'x-hagency-link': token },
  ...
  const loop = (async () => {
    while (!stopped && shouldContinue()) {
      try {
        const res = await fetchImpl(`${url}/_hagency/edge/pull`, {
          headers: { 'x-hagency-link': token },
        ...
        if (res.status === 403) {
          ...
          logger.error?.('[edge-puller] the edge rejected our link token — check HAGENCY_EDGE_LINK_TOKEN');
          await sleep(BACKOFF_MS.at(-1));
          continue;
        }
        if (res.status === 409) {
          /*
           * NOT COUNTED AS A FAILURE. `stats.failed` is what says events could not be collected, and a
           * restart bumping it reads as data loss when nothing has been lost yet — the homeserver still
           * holds the transaction and will still hand it over.
           */
          stats.lastError = 'the edge is still holding a poll slot for our previous instance';
          stats.lastErrorAt = Date.now();
          logger.warn?.(
            '[edge-puller] the edge says another poller is already waiting — that is our own previous '
            + `instance, and it frees when its long poll times out. Retrying every ${POLLER_CONFLICT_RETRY_MS}ms.`,
          );
          await sleep(POLLER_CONFLICT_RETRY_MS);
          continue;
        }
        ...
        const result = await router.handle({
          ...
          // Adapter metadata is separate from the edge's submitted event body.
          body: { events },
          transport: { mode: 'edge', sideId: side },
        });
