...
export async function sendToRoomOnSide({
  ...
  if (isProjection && !/^[A-Za-z0-9._~-]{1,128}$/.test(finalTxnId)) {
  ...
  const txnId = isProjection
    ? finalTxnId
    : `hafleet_${createHash('sha256').update(txnSeed).digest('hex').slice(0, 32)}`;
...
export async function sendEmptyStateToRoomOnSide({
  ...
  try {
    ...
    const data = await readBody(res);
    if (!res.ok) throw matrixError(res, data, 'send state');
    const eventId = typeof data?.event_id === 'string' ? data.event_id.trim() : '';
