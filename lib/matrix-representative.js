...
export async function joinRoomOnSideAsAgent({
  ...
  /*
   * F10 (17-r1): the agent join goes through the single exit too — the verdict
   * above stays as the caller-facing refusal reason, and the parameter is set
   * by the one function that composes a masquerade.
   */
  setMasqueradeUserParam(url, agentUserId, { label: 'agent-join' });
...
export async function joinRoomOnSideAsRepresentative({
  ...
  if (credential.kind !== 'appservice') {
    return {
      joined: false,
      reason: 'only an appservice side can answer an invite as the representative; a registrationToken '
        + 'side joins with the representative token directly',
    };
  }
  if (!roomBelongsToSide(roomId, side)) {
    return {
  ...
  const representative = `@${String(credential.senderLocalpart || '').toLowerCase()}:${side.serverName}`;
  const url = new URL(
    `${String(side.apiBaseUrl).replace(/\/+$/, '')}/_matrix/client/v3/join/${encodeURIComponent(roomId)}`,
  );
  setMasqueradeUserParam(url, representative, { label: 'representative' });
...
export async function leaveRoomOnSideAsAgent({
  ...
  setMasqueradeUserParam(url, agentUserId, { label: 'agent-leave' });
