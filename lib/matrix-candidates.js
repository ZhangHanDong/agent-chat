 ...
 * served by every homeserver, while the latter is a Hagency convention that a customer's server has
 ...
 * `accepted` after exercising the OUTBOUND direction only: Hagency could act as the representative, so the
 ...
 *   `not-collected` the homeserver calls and Hagency never picks up. The bridge is not running. This is
...
export function diagnoseEdgeInbound(status, { now = () => Date.now() } = {}) {
  ...
  const collecting = Boolean(status.hagencyWaiting) || Boolean(status.hagencyLastSeenAt);
  ...
  if (transactions === 0) {
     ...
     * Hagency to be connected too, because a fresh edge nobody is collecting from is a real problem however
     * young it is.
     */
    ...
    return {
      ...
      detail: settling
        ? `this edge started ${Math.round(ageMs / 1000)}s ago and nothing has happened in a room it can see `
          + 'since. Hagency is connected and waiting, so there is nothing to fix yet.'
  ...
  if (!collecting) {
    return {
      state: 'not-collected',
      detail: `the homeserver has called ${transactions} time(s) and Hagency has never collected. The `
  ...
  if (delivered === 0) {
    return {
      state: 'not-collected',
      detail: `Hagency is connected but has taken nothing of the ${transactions} transaction(s) offered.`,
    };
  }
  return {
    state: 'flowing',
 ...
 * discover last. With no `HAGENCY_APPSERVICE_PORT` the bridge opens no socket at all, so a perfectly
...
export async function describeMatrixReach({
  ...
  const rawPort = String(env.HAGENCY_APPSERVICE_PORT ?? '').trim();
   ...
   *
   * With no `HAGENCY_APPSERVICE_PORT` this used to report "the bridge opens no socket… a registration
   ...
   */
  const edgeUrl = String(env.HAGENCY_EDGE_URL ?? '').trim();
  const edgeToken = String(env.HAGENCY_EDGE_LINK_TOKEN ?? '').trim();
  const edgeSide = String(env.HAGENCY_EDGE_SIDE ?? '').trim();
      ...
      : edge
        ...
        : sync
          ...
          : {
            ...
            port: null,
            inboundVia: null,
            reason: 'HAGENCY_APPSERVICE_PORT is not set, no co-located edge is configured and no sync '
              + 'intake is configured, so nothing will receive your homeserver\'s events. Set the port, '
              + 'run bin/hagency-appservice-edge beside the homeserver and set HAGENCY_EDGE_URL, or set '
              + 'HAGENCY_APPSERVICE_SYNC_SIDE and HAGENCY_APPSERVICE_SYNC_URL.',
            callbackCandidates: [],
          },
  };
}
...
export async function verifyCallbackFromHomeserver({
   ...
   * Hagency's own port is irrelevant.
   */
  edgeUrl = null,
  execFileImpl,
  interfaces = networkInterfaces(),
   ...
   *
   * With an edge, the address to verify is the EDGE'S, and Hagency's own port is beside the point. The check
  ...
  for (const candidate of candidates) {
    /*
     * A 403 IS THE PASS. Hagency's appservice endpoint refuses an unauthenticated GET, so "it answered at
  ...
  return {
    ...
    reason: winner
      ? `answered from inside ${container}, so your homeserver can reach Hagency there`
      : `no candidate answered from inside ${container} — Hagency may be bound somewhere that container cannot see`,
  };
}
