...
async function fixture({ legacy = false, disk = null } = {}) {
  ...
  const context = await createBackendTestContext('single-worker-', {
    ...
    env: { MATRIX_BRIDGE_SECRET: SECRET, MATRIX_SERVER_NAME: 'test', MATRIX_BOT_USERNAME: 'bot',
      MATRIX_HOMESERVER: matrixUrl, HAFLEET_API: 'http://127.0.0.1:1', HAFLEET_AGENT_TOKEN_MODE: 'hard',
      HAFLEET_APPROVAL_DM_MODE: 'required', HAFLEET_ALLOW_PLAINTEXT_APPROVAL_TEST: '' },
  });
  ...
  process.env.HAFLEET_API = backendUrl;
  ...
  if (legacy) {
    ...
    const original = mod.buildOwnerApprovalRequest(approval);
