...
// Observed on mini5: an agent created by plain `hagency up` launched, went
...
// why. The cause was .mcp.json omitting HAGENCY_AGENT_STATE_DIR, because the
...
describe('the agent state directory is derived once', () => {
  ...
  test('the fallback is the default agent home, not empty', () => {
    ...
    expect(body).toContain('HAGENCY_HOMEDIR:-$HOME/.hagency');
...
describe('the generated .mcp.json is complete', () => {
  ...
  test('it always sets HAGENCY_AGENT_STATE_DIR', () => {
    expect(generator).toContain('HAGENCY_AGENT_STATE_DIR');
...
describe('the Codex path gets the same treatment', () => {
  test('HAGENCY_AGENT_STATE_DIR is injected unconditionally', () => {
    // Codex strips ambient env, so it needs the value passed through -c. It was
    // guarded on SAVED_STATE_DIR being non-empty, so plain `hagency up` codex
    // agents had the identical silent-auth-failure bug.
    expect(source).toMatch(/codex_mcp_env HAGENCY_AGENT_STATE_DIR "\$\(resolve_agent_state_dir\)"/);
  });
});
