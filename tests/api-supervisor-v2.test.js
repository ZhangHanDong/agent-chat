...
describe('supervisor v2 API', () => {
  ...
  async function setup(opts = {}) {
    ...
    context = await createBackendTestContext('hagency-supervisor-v2-test-', {
      agents: {
      ...
      groups: {},
      ...restOpts,
      env: { HAGENCY_HOMEDIR: homeDir, ...env },
    });
    return context;
  }
  ...
  test('rejects PATCH with wrong token in enforce mode', async () => {
    ...
    context = await createBackendTestContext('hagency-supervisor-v2-test-', {
      ...
      groups: {},
      env: { HAGENCY_HOMEDIR: homeDir, HAGENCY_AGENT_TOKEN_MODE: 'hard' },
    });
    const res = await request(context.app)
      .patch('/api/supervisor-state/ac-topleader')
