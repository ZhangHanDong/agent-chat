...
async function importBackend(seed = {}) {
  const runtimeDir = mkdtempSync(path.join(os.tmpdir(), 'hagency-normalization-test-'));
...
describe('normalization helpers', () => {
  ...
  test('blocked patterns retain intended tiers and match representative prompts', () => {
    ...
    expect(byReason.get('update-required')?.re.test('Updates available: run hagency-update now')).toBe(true);
  });
});
