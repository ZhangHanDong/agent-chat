...
const autodeployScript = path.join(repoRoot, 'scripts', 'hagency-stable-autodeploy.sh');
const tmpRoots = [];
...
async function setupRepo() {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), 'hagency-stable-cd-'));
  tmpRoots.push(tmp);
  const ctx = {
    tmp,
...
async function runAutodeploy(ctx, env = {}) {
  const baseEnv = {
    ...process.env,
    HAGENCY_LIVE_DIR: ctx.live,
    HAGENCY_DEPLOY_USER: os.userInfo().username,
    HAGENCY_DEPLOY_BRANCH: 'stable',
    HAGENCY_POLL_SEC: '5',
    HAGENCY_ONCE: '1',
    HAGENCY_DEPLOY_STATE_DIR: ctx.state,
    HAGENCY_BACKEND_HEALTH_TIMEOUT: '1',
    HAGENCY_DEPLOY_SERVICES: 'hagency-backend hagency',
    HAGENCY_SYSTEMCTL_BIN: path.join(ctx.binDir, 'systemctl'),
    HAGENCY_CURL_BIN: path.join(ctx.binDir, 'curl'),
    HAGENCY_SLEEP_BIN: path.join(ctx.binDir, 'sleep'),
    HAGENCY_NPM_BIN: path.join(ctx.binDir, 'npm'),
    HAGENCY_RELEASE_GATE: 'none',
    HAGENCY_RELEASE_GATE_ARGS: '',
...
describe('stable autodeploy CD gate', () => {
  ...
  test('worktree release gate failure does not reset or restart the live checkout', async () => {
    ...
    const { stdout } = await runAutodeploy(ctx, { HAGENCY_RELEASE_GATE: 'worktree' });
  ...
  test('worktree release gate success resets and restarts the live checkout', async () => {
    ...
    expect(commandLog).toContain('systemctl:restart hagency-backend');
    expect(commandLog).toContain('systemctl:restart hagency');
  });
  ...
  test('default deploy state dir under .git supports the worktree gate', async () => {
    ...
    await runAutodeploy(ctx, {
      ...
      HAGENCY_DEPLOY_STATE_DIR: undefined,
    });

    await expect(git(ctx.live, ['rev-parse', 'HEAD'])).resolves.toBe(nextRef);
    await expect(readIfExists(path.join(liveGitDir, 'hagency-autodeploy', 'last-successful-ref'))).resolves.toBe(`${nextRef}\n`);
  });
  ...
  test('dependency install failure persists retry state after the live checkout already reset', async () => {
    ...
    await runAutodeploy(ctx, { HAGENCY_FAIL_INSTALL_ONCE: failOnce });
    ...
    expect(commandLog).toContain('systemctl:restart hagency-backend');
    expect(commandLog).toContain('systemctl:restart hagency');
    expect(commandLog).toContain('systemctl:is-active --quiet hagency-backend');
  });
});
