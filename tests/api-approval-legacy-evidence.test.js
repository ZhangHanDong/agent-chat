...
beforeAll(async () => {
  context = await createBackendTestContext('hafleet-legacy-evidence-api-', {
    ...
    env: { MATRIX_BRIDGE_SECRET: secret, HAFLEET_AGENT_TOKEN_MODE: 'hard', MATRIX_SERVER_NAME: 'test', MATRIX_BOT_USERNAME: 'bot' },
  });
...
async function isolated(record, env = {}) {
  const c = await createBackendTestContext('hafleet-legacy-api-case-', {
    ...
    env: { MATRIX_BRIDGE_SECRET: secret, HAFLEET_AGENT_TOKEN_MODE: 'hard', MATRIX_SERVER_NAME: 'test', MATRIX_BOT_USERNAME: 'bot',
      HAFLEET_APPROVAL_DM_MODE: 'required', HAFLEET_ALLOW_PLAINTEXT_APPROVAL_TEST: '', ...env },
  });
...
test('legacy API permits only the existing explicit plaintext-test policy', async () => {
  const f = await isolated(undefined, { HAFLEET_APPROVAL_DM_MODE: 'plaintext-test', HAFLEET_ALLOW_PLAINTEXT_APPROVAL_TEST: '1', NODE_ENV: 'test' });
  try {
