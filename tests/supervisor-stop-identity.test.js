...
// `hagency stop` refused on a fleet host with "supervisor PID belongs to an unexpected
// process", leaving a crash-looping service that could not be removed. The
// supervisor is launched from the runtime root as
//   node services/hagency-services.mjs run --profile ... --runtime <runtime-root>
...
const RUNTIME = '/srv/hagency';
const CLI = `${RUNTIME}/services/hagency-services.mjs`;
...
describe('the supervisor is recognised however it was launched', () => {
  test('the exact command line observed on a fleet host matches', () => {
    expect(run(`node services/hagency-services.mjs run --profile ${RUNTIME}/services-macos.json --runtime ${RUNTIME}\n`)).toBe(true);
  });
  ...
  test('a different process on the same host does not match', () => {
    ...
    expect(run(`node scripts/hagency-acp-agent.mjs --name octos-agent --runtime ${RUNTIME}\n`)).toBe(false);
    expect(run('/usr/bin/vim services/hagency-services.mjs\n')).toBe(false);
  });

  test('the same CLI for a different runtime root does not match', () => {
    expect(run('node services/hagency-services.mjs run --profile p.json --runtime /Users/other/hagency\n')).toBe(false);
  });

  test('a status invocation is not mistaken for the running supervisor', () => {
    expect(run(`node services/hagency-services.mjs status --profile p.json --runtime ${RUNTIME}\n`)).toBe(false);
  });
  ...
  test('a cli outside the runtime root does not fall back to a loose match', () => {
    ...
    const outside = new Function('path', 'command', 'cliPath', 'runtimeRoot',
      readFileSync('services/hagency-services.mjs', 'utf-8')
        .match(/const relativeCli[\s\S]*?return namesThisCli[^;]*;/)[0]);
    expect(outside(path, `node /opt/hagency/services/hagency-services.mjs run --runtime ${RUNTIME}\n`,
      '/opt/hagency/services/hagency-services.mjs', RUNTIME)).toBe(true);
    expect(outside(path, `node something-else run --runtime ${RUNTIME}\n`,
      '/opt/hagency/services/hagency-services.mjs', RUNTIME)).toBe(false);
  });
});
