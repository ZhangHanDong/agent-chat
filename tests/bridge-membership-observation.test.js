...
async function drainSse() {
  let iterations = 0;
  while (pendingSse.length) {
    if (++iterations > 20) throw new Error('membership SSE feedback exceeded the independent fixture watchdog');
    await bridge.onGroupMembersChanged(pendingSse.shift());
  }
}
