...
async function assertNoCiTempLogs(tempDir) {
  ...
  expect(entries.filter((entry) => entry.startsWith('hagency-verify-ci.'))).toEqual([]);
  expect(entries.filter((entry) => entry.startsWith('hagency-kernel-tests.'))).toEqual([]);
}
...
const blockingDescendantScript = `bash -c 'sleep 60 & echo "$!" >>"$HAGENCY_TEST_CHILD_PID_FILE"; wait "$!"' &
child="$!"
echo "$child" >>"$HAGENCY_TEST_CHILD_PID_FILE"
wait "$child"
`;

const leakingDescendantScript = `bash -c 'sleep 60 & echo "$!" >>"$HAGENCY_TEST_CHILD_PID_FILE"; exit 0' &
child="$!"
echo "$child" >>"$HAGENCY_TEST_CHILD_PID_FILE"
wait "$child"
`;
...
describe('CI script signal cleanup', () => {
  ...
  for (const { signal, exitCode } of signalCases) {
    test(`verify-ci ${signal} cleanup kills descendants from the tracked environment step`, async () => {
      const tempDir = await makeTempDir('hagency-ci-cleanup-');
...
`);
      ...
      const child = spawn('bash', [verifyCiScript], {
        ...
        env: {
          ...process.env,
          HAGENCY_VERIFY_CI_TIMEOUT_ACTIVE: '1',
          HAGENCY_TEST_CHILD_PID_FILE: childPidFile,
...
`);
  ...
  for (const { signal, exitCode } of signalCases) {
    test(`run-kernel-tests ${signal} cleanup kills shard descendants`, async () => {
      const tempDir = await makeTempDir('hagency-kernel-cleanup-');
      const childPidFile = path.join(tempDir, 'child-pids');
