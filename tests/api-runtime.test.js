...
describe('backend runtime API', () => {
  ...
  test('safe JSON writes clean up temp files and preserve the target on rename failure', async () => {
    context = await createBackendTestContext('hagency-runtime-safe-write-test-', {
      agents: {},
      groups: {},
    });
  ...
  test('runtime reports persist backend-derived observation provenance', async () => {
    context = await createBackendTestContext('hagency-runtime-provenance-test-', {
      agents: {
        alpha: {
          name: 'alpha',
          type: 'agent',
  ...
  test('MCP heartbeat restores liveness without clearing blocked runtime state', async () => {
    ...
    context = await createBackendTestContext('hagency-mcp-heartbeat-test-', {
      agents: {
        alpha: {
          name: 'alpha',
          type: 'agent',
  ...
  test('recent MCP heartbeat prevents runtime heuristic from marking MCP missing', async () => {
    context = await createBackendTestContext('hagency-mcp-heartbeat-authority-test-', {
      agents: {
        alpha: {
          name: 'alpha',
          type: 'claude',
  ...
  test('stale MCP heartbeat allows runtime heuristic to mark MCP missing', async () => {
    ...
    context = await createBackendTestContext('hagency-mcp-heartbeat-stale-test-', {
      agents: {
        alpha: {
          name: 'alpha',
          type: 'claude',
  ...
  test('MCP heartbeat can re-register an agent after backend state loss', async () => {
    context = await createBackendTestContext('hagency-mcp-heartbeat-register-test-', {
      agents: {},
      groups: {},
    });
  ...
  test('remote runtime reports keep API_TOKEN compatibility and do not accept server token yet', async () => {
    context = await createBackendTestContext('hagency-runtime-auth-test-', {
      agents: {
        alpha: {
          name: 'alpha',
          type: 'agent',
      ...
      env: {
        ...
        HAGENCY_SERVER_TOKEN: 'server-token',
      },
    });
  ...
  test('runtime reports preserve unknown activity instead of reporting idle', async () => {
    context = await createBackendTestContext('hagency-runtime-unknown-activity-test-', {
      agents: {
        alpha: {
          name: 'alpha',
          type: 'agent',
  ...
  test('push-delivered ignores stale queue acknowledgements', async () => {
    context = await createBackendTestContext('hagency-runtime-push-delivered-test-', {
      agents: {
        alpha: {
          name: 'alpha',
          type: 'agent',
  ...
  test('push-delivered does not reopen inbox gate after check_inbox consumed the source', async () => {
    context = await createBackendTestContext('hagency-runtime-push-delivered-read-ack-test-', {
      agents: {
        alpha: {
          name: 'alpha',
          type: 'agent',
  ...
  test('blocked notifications use tiered debounce and never notify transient blockers', async () => {
    context = await createBackendTestContext('hagency-runtime-test-', {
      agents: {
        alpha: {
          name: 'alpha',
          type: 'agent',
  ...
  test('severity rebroadcast only happens when blocked tier increases', async () => {
    ...
    const hardFirst = await request(context.app).post('/api/agents/alpha/runtime').send({
      ...
      tail: 'update available: run hagency-update',
      command: 'codex',
    });
  ...
  test('codex agent reports mcpPresent=null and does not trigger mcp_missing', async () => {
    context = await createBackendTestContext('hagency-mcp-type-test-', {
      agents: {
        codexbot: {
          name: 'codexbot',
          type: 'codex',
...
describe('inbox gate semantics', () => {
  ...
  test('send blocks on pending gate; filtered reads never clear it; only a full unfiltered read clears it', async () => {
    context = await createBackendTestContext('hagency-inbox-gate-contract-test-', {
      agents: {
        alpha: {
          name: 'alpha',
          type: 'agent',
  ...
  test('a full unfiltered read clears the gate even when the source message has already scrolled behind the cursor', async () => {
    context = await createBackendTestContext('hagency-inbox-gate-behind-cursor-test-', {
      agents: {
        alpha: {
          name: 'alpha',
          type: 'agent',
  ...
  test('a full unfiltered read clears the gate even when unread is empty, and records one read acknowledgement', async () => {
    context = await createBackendTestContext('hagency-inbox-gate-empty-unread-test-', {
      agents: {
        alpha: {
          name: 'alpha',
          type: 'agent',
