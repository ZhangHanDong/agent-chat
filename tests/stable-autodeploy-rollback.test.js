...
printf 'systemctl:%s\\n' "$*" >> "$HAGENCY_TEST_LOG"
if [ -f "$HAGENCY_LIVE_DIR/breaks-service" ]; then
...
async function setupRepo({ brokenFromStart = false } = {}) {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), 'hagency-cd-rollback-'));
  tmpRoots.push(tmp);
  const ctx = {
    tmp,
...
describe('autodeploy rollback', () => {
  ...
  test('alerting is opt-in and never masks the deploy failure', async () => {
    ...
    const { stdout } = await runAutodeploy(ctx, {
      HAGENCY_ALERT_URL: 'http://127.0.0.1:9/api/alerts',
      HAGENCY_ALERT_TOKEN: 'test-token',
    });
