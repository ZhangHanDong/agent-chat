...
describe('services-team Compose contract', () => {
  test('defines exactly four durable host-network services', () => {
    ...
    expect(compose.match(/hagency-runtime:\/var\/lib\/hagency/g)).toHaveLength(4);
  ...
  test('uses backend health as the startup dependency', () => {
    ...
    expect(compose).toContain('$${process.env.HAGENCY_BACKEND_PORT || 8090}');
    expect(compose).toContain('$${process.env.HAGENCY_WEB_PORT || 8084}');
  });

  test('contains no literal credentials and requires an operator env file', () => {
    const compose = readFileSync(composePath, 'utf8');

    expect(compose).toMatch(/env_file:\n\s+- \$\{HAGENCY_ENV_FILE:-\.\.\/\.env\}/);
    ...
    expect(compose).not.toMatch(/^\s+HAGENCY_(?:API|WEB_URL):/m);
  });
  ...
  test('builds a non-root Node 22 production image', () => {
    ...
    expect(dockerfile).toMatch(/\/var\/lib\/hagency/);
