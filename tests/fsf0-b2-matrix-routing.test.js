...
beforeAll(async () => {
  runtimeDir = mkdtempSync(path.join(os.tmpdir(), 'hagency-fsf0-b2-routing-'));
...
test('accepted_before_checkpoint_replay_zero_duplicates', async () => {
  const backend = await createBackendTestContext('hagency-fsf0-b2-crash-window-', {
    agents: {
      wf_coordinator: {
...
test('mapped_room_coordinator_delivered_by_backend', async () => {
  const backend = await createBackendTestContext('hagency-fsf0-b2-group-delivery-', {
    agents: {
