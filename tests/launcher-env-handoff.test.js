...
function fixture() {
  const root = mkdtempSync(path.join(os.tmpdir(), 'hafleet-launch-env-'));
  cleanups.push(() => rmSync(root, { recursive: true, force: true }));
  for (const dir of ['bin', 'scripts', 'runtime', 'workdir', 'state']) mkdirSync(path.join(root, dir));
  for (const name of ['hafleet-up-v1', 'hafleet-up']) copyFileSync(path.resolve('bin', name), path.join(root, 'bin', name));
...
function run(root, entry, args, extraEnv) {
  ...
  for (const key of Object.keys(env)) {
    if (key.startsWith('HAFLEET_') || key.startsWith('AGENT_') || key === 'API_TOKEN' || key === 'BASH_ENV') delete env[key];
  }
  return new Promise((resolve, reject) => {
    const child = spawn('/bin/bash', [path.join(root, 'bin', entry), ...args], {
      cwd: root, env: { ...env, HAFLEET_INTERNAL_DISPATCH: '1', ...extraEnv }, stdio: ['ignore', 'pipe', 'pipe'],
    });
    let stdout = '', stderr = '';
...
test('backend v1 launcher uses its own authenticated loopback deployment despite repository dotenv', async () => {
  ...
  writeFileSync(path.join(root, 'bin/hafleet-up'), '#!/bin/bash\nnode -e \'require("fs").writeFileSync(process.env.HANDOFF_FILE, JSON.stringify({token:process.env.API_TOKEN,port:process.env.HAFLEET_BACKEND_PORT,ready:process.env.HAFLEET_LAUNCH_ENV_READY}))\'\n', { mode: 0o755 });
  const result = await run(root, 'hafleet-up-v1', ['example', 'claude'], {
    API_TOKEN: 'deployment-token', HAFLEET_BACKEND_PORT: String(port),
    HAFLEET_RUNTIME_DIR: path.join(root, 'runtime'), HAFLEET_LAUNCH_MODEL: 'selected-model',
    HAFLEET_LAUNCH_ENV_READY: '1', HANDOFF_FILE: path.join(root, 'handoff.json'),
  });
...
function inspectOnExit(root) {
  writeFileSync(path.join(root, 'exit-inspect.js'), 'require("fs").writeFileSync(process.env.ENV_RECEIPT,JSON.stringify({token:process.env.API_TOKEN,port:process.env.HAFLEET_BACKEND_PORT}))');
...
test('tmux launcher preserves the resolved backend environment', async () => {
  const root = fixture();
  writeFileSync(path.join(root, '.env'), 'API_TOKEN=wrong-token\nHAFLEET_BACKEND_PORT=19001\n');
  const result = await run(root, 'hafleet-up', ['--help'], {
    ...inspectOnExit(root), API_TOKEN: 'deployment-token', HAFLEET_BACKEND_PORT: '19002',
    HAFLEET_RUNTIME_DIR: path.join(root, 'runtime'), HAFLEET_LAUNCH_ENV_READY: '1',
  });
  expect(result.code, result.stderr).toBe(0);
...
test('standalone launchers still load runtime dotenv after repository defaults', async () => {
  for (const entry of ['hafleet-up-v1', 'hafleet-up']) {
    const root = fixture();
    writeFileSync(path.join(root, '.env'), 'API_TOKEN=repository-token\nHAFLEET_BACKEND_PORT=19001\n');
    writeFileSync(path.join(root, 'runtime/.env'), 'API_TOKEN=runtime-token\nHAFLEET_BACKEND_PORT=19002\n');
    const result = await run(root, entry, ['--help'], {
      ...inspectOnExit(root), HAFLEET_RUNTIME_DIR: path.join(root, 'runtime'),
    });
    expect(result.code, result.stderr).toBe(0);
