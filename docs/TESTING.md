...
- **LOOP-R6** — in retry/breaker state machines, every reset assignment must answer in
  ...
  variable found and fixed a second instance the same day.)

## The heap ceiling on npm test (2026-08-31)

`npm test` and `npm run test:ci` run under an explicit `--max-old-space-size=4096`.

Why not lower: a 2048 ceiling was tried and the plain serial suite ITSELF dies at
~2 GB around `tests/api-usage-metering.test.js` — mark-compact reclaims nothing
...
clear FATAL in under two minutes instead of depending on machine defaults. When
developing a loop test in isolation, run it with a LOW ceiling on purpose:
`NODE_OPTIONS=--max-old-space-size=256 npx vitest run tests/<file>` makes a
runaway die in seconds.

The ceiling can come down after the module-retention debt is paid (split the
...
in `specs/task-<name>.spec.md` — model it on
`specs/task-appservice-sync-intake.spec.md`: scenarios bound to named tests, the
spec as the single source of truth for acceptance. Implementation and review
follow the spec, not the other way around.
...
| 2026-08-16 | `router-launch-recovery` | `backend requeues a wrapper that dies before takePayload without losing its input` | **yes** — `launch_failures: 1` where 2 was expected, so a poll loop gave up rather than an assertion being wrong. **On GitHub Actions**, 8/8 clean locally in isolation, and the sibling PR on the same master base passed the same job . **Recurred 2026-08-22**, on GitHub Actions again, identical assertion, on a branch whose only changes were one test helper and this file — passed on a plain re-run of the same job |
| 2026-08-18 | `local-service-supervisor` | `doctor names an explicitly stopped bridge` | **yes** — a NEW file for this list, and it took 3235ms in the whole-suite run against a 3/3 clean isolation. The change under test touched `app.delete('/api/agents/:name')`, which that file never reaches |
| 2026-08-18 | `api-project-sides` | twice in five runs, a DIFFERENT test each time (`AGENTS ARE RETIRED BEFORE THE SIDE IS FORGOTTEN`, then `pending invitations on the side are DECLINED`) | **probably** — and worth reading as the honest version. Both failures were `expected 404 to be 200`. The branch had just added 3 more backend contexts to this file, so "my change made it flaky" was the first suspicion: clean master ran 3/3, the branch failed 2 of 5. Then the branch ran **13 consecutive clean runs** of the same file. The two failures happened while several other vitest runs and ssh sessions were in flight, which matches every other entry here. Recorded as this class, NOT as proof the change is innocent — 13 clean runs cannot prove that, and a third context-count sighting in this file should re-open it |
...
| 2026-08-18 | THREE files in one run: `api-tasks`, `api-agents`, `api-agents-self-declaration` | one test each | **yes** — 55/55 clean when the three run together, three times, and the next whole-suite run of the same tree was 3489/3489. The change under test touched the appservice edge, the inbound diagnosis and one console page; none of those three files reaches any of it. Worth logging as another three-file-in-one-run sighting: the 2026-08-14 entry above is the four-file case, so a single run failing in several unrelated files is now seen repeatedly rather than once |
