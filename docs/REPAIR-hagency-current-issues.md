...
Scope: **仅 hagency 自身**（启动、registry、调度、Matrix bridge、可观测性）

---

## 1. 目的

把当前线上/联调里反复出现的 hagency 故障，收敛成一份可执行的修复清单：
...
本文不讨论外部系统（如 OpenFab / agentd）应如何演进；只回答：**hagency 现在哪里坏了、怎么修。**

---

## 2. 现状摘要

hagency 已具备完整本地多 agent 控制面：
...
| `bridge-matrix.js` | Matrix ↔ hagency |
| `bin/hagency-up*` / `hagency-down` / `hagency-ls` | 启动与运维 CLI |
...
**涉及**
- `bin/hagency-up`, `bin/hagency-up-v1`
...
### P-09 OpenFab signoff 语义被误用为强制门禁（hagency 侧）

**现象**
- 执行完成后，系统/调用方把“未 signoff”当成“未交付”
- hagency 路径混入认证语义，排障困难
...
- backend groups
- project 管理（`hagency-project` / agent home projects）
...
1. Matrix 只发：
   ...
   - 状态摘要
   - 结果链接 / task id
2. 完整 transcript、diff、日志、产物进 hagency storage（消息库/对象存储/agent outputs）
3. 长内容默认转存储 + 摘要回房
