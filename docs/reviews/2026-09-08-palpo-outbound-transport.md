# Palpo / Hagency outbound transport review
...
Reviewed Hagency `39196ee` and Palpo `0736f991` / `0b11e786` after the operator
...
The deployment direction should be Hagency initiating connections to public
...
- Palpo `web-admin/lib/workflow.mjs:41` builds its provider client from
  ...
  submission at line309 and request status at line352 all call Hagency.
- Hagency `lib/fleet-protocol.js:98` consumes protocol events but records a
...
- Hagency `lib/appservice-edge.js:19` describes its deliberate nonpersistent
...
Hagency's generic sync collector also filters `to_device` events
...
|---|---|
| Resource pools, models and roles | Hagency POSTs a versioned snapshot; Palpo persists and serves it locally. |
| Heartbeat, decisions and Agent state | Hagency POSTs monotonic updates; Palpo records the observed time and online/offline state. |
| Project Agent requests | Palpo validates the actor and project, persists the request and returns its stable ID. Offline Hagency leaves it pending. |
| Request delivery | Hagency long-polls Palpo, durably records each request, then acknowledges delivery. Hagency retains allocation/approval authority. |
| Matrix AS events | Palpo's homeserver pushes to a colocated relay; Hagency initiates event polling from that relay. |
...
event and owner checks when Hagency ingests a business request.
...
ID before acknowledging it to the homeserver. Hagency acknowledges the relay
...
2. Implement durable business queues and Hagency publishing/polling. Make the web
...
4. Exercise an isolated deployment with no route from Palpo to Hagency. Then migrate
