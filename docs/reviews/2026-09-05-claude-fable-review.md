...
# HAFleet Independent Code Review — commit `75ca1ec`
...
- **Trigger:** any holder of `HAFLEET_REQUESTER_TOKEN` (a single shared secret; `lib/bot-commands.js:121-124` shows it is intended to travel to submitting parties) who knows or guesses another project's whitelisted room id.
...
- **Location:** `/Users/yuechen/home/hagency/backend-v2.js:13372-13376` (`bindEngagement` error names `HAFLEET_OWNER_MXID` / `HAFLEET_OWNER_DM_ROOM`), returned verbatim as `binding.error` in the `POST /api/engagements` auto-join response (`:13663-13667`) — a route authenticated by the *requester* (project-side) credential.
