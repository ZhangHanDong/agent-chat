...
- **owner 审批房没有审批卡或 owner 按钮不可用** → GUI 审批验收 **FAIL**;先复核 §2.5 的成员与精确 state-event 权限。API verdict 只能作为分层诊断,不能替代卡片/按钮验收。
- **marker 写入 403** 与全局 `429`(本次观察为约 50 次 burst 后出现)是不同信号。worker 的 2 个并发 job 只限制同时在途工作,不证明 Matrix 请求速率受控;在实际 pacing 测试覆盖持续 drain、重试和 read/write 组合之前,不得声称节流已修复。
