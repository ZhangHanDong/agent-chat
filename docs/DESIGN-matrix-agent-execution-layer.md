# Design — matrix-Agent execution layer (hagency side) + the OpenFab driving contract
...
**hagency schedules** (picks/spins an agent from the pool and runs it). The code below lands
in **hagency** (a separate repo); OpenFab only gains a richer dispatch contract.

## Where hagency is today
- Each agent record already has a `role` field; agents are named `<team>_<role>`
  ...
  `beta_*` teams). Launched via `hagency up-v1 <name> <runtime>` (claude / codex / …).
...
Unchanged in hagency — humans are room members who set direction; the *gate* itself is
OpenFab's N-of-M sign-off. No new hagency mechanism needed.
...
- Bridge env / task: `role` + `capability` (e.g. `role=coding capability=medium`) instead of a
  hard-coded assignee. The Bridge calls hagency `POST /api/dispatch {role, capability, ...}`
...
- OpenFab's trust line is untouched: it still drives, verifies, signs, gates exactly the same —
  ...
  (pool + scheduling + 6 roles) entirely inside hagency, which is where matrix-Agent lives.
