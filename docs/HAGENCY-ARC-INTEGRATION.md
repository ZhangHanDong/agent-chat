...
- 版本:v1(2026-07-20)
- 范围:HAgency(robrix + hagency)× agent-spec × ARC × octos
- 定位参考:`docs/AGENTTEAMS-BORROWINGS-AND-S3-INTEGRATION.md`(协作栈路线图 v2)

## 0. 名词与一句话

- **HAgency** = robrix2(人机界面)+ hagency(编排 / 运行时 / 消息总线)。
...
- **ARC** = 需求编译器 + 可视化(ARC-Bench):把结构化需求树编译成接口、
  ...
  **hagency 集成层是我们在自有 fork(`ZhangHanDong`)上加的**(见 §2)。
...
agent-spec ──arc-native──▶ ARC(编译/可视化/编排/系统所有的验证)
   ...
   人(robrix)          coding agent(可插拔:ARC自带 / Claude / Codex / octos)
     └────────── 全程在 hagency 房间里编排、审计 ────────────┘
```

铁律:**hagency 协议是主,ARC 侧适配它**。而且这个适配是**我们主动加的**
(见 §2 的分支事实):hagency 集成三级全部由 AlexZ 写在 ARC 的 fork 分支上,
所以 `arc_stage` 等 schema 的定义权与协议主动权都在我们手里。hagency 核心
永不 import ARC/octos 内部,一切经 `task_request/task_result` 协议门;ARC 上游
未来独立演进,HAgency 一行不改。

## 2. 关键事实:ARC 已实现的三级 hagency 集成
...
的分支 **`feat/hagency-progress-reporting`** 上;上游是
...
`HAGENCY_URL` / `HAGENCY_TOKEN` / `HAGENCY_AGENT_TOKEN` /
`HAGENCY_AGENT_NAME`(默认 `arc-compiler`)。
...
/ `ARC_DELEGATE_TO` + `HAGENCY_URL`;可与 `--serve` 组合(任务串行,不与 worker
的 cursor 读竞争)。
...
octos 能在 Matrix/hagency 通道里以一个 agent 身份存在;当它实现了
