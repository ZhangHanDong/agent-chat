...
│          Outside（域外）                      │
│  和当前任务无关的一切                         │
│  例：hagency 的消息格式、backtest-web 的    │
│  前端组件、GPU 集群的硬件配置                 │
└─────────────────────────────────────────────┘
```
...
- 遇到 Adjacent 依赖的问题时，记录现象并通过 hagency 转交给负责人
...
它是一个**独立的传统进程**（Node.js 或 Python 脚本），不是 Claude/Codex Agent。它通过 hagency REST API 和 tmux 与系统交互。
...
连续 2 次检测到同一方向的非 FOCUSED:
  → 通过 hagency 发送轻提醒（DM, type: inform）
...
| Agent blocked（hagency runtime 报告 blocked） | 跳过，blocked 有自己的处理机制 |
...
- hagency backend API（获取在线 agent 列表、发送消息）
- tmux（capture-pane）
