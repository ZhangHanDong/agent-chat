...
4. **alerts**, if `HAGENCY_ALERT_URL` and `HAGENCY_ALERT_TOKEN` are set
...
State lives in `$(git rev-parse --absolute-git-dir)/hagency-autodeploy/`
unless `HAGENCY_DEPLOY_STATE_DIR` overrides it:
...
```bash
STATE="$(git rev-parse --absolute-git-dir)/hagency-autodeploy"
...
journalctl -u hagency-stable-autodeploy -n 100
```
...
```bash
cd /path/to/install
systemctl stop hagency hagency-push-relay bridge-matrix
...
systemctl restart hagency hagency-push-relay
```
...
```bash
systemctl stop hagency hagency-backend
tar -czf ~/hagency-data-$(date -u +%Y%m%dT%H%M%SZ).tar.gz data/
```

`.env` and `~/.hagency` are likewise untouched by rollback, which is
