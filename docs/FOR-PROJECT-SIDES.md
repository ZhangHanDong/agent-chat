...
You have a Matrix homeserver and you want Hagency's agents working in one of your rooms. This is
everything you must do, why each step exists, and what Hagency will not do for you.
...
Nothing here asks you to trust Hagency with your homeserver's admin. Every step is a grant you can see
in your own room state and withdraw the same way.

## 1. Give Hagency a credential — one of two kinds

Hagency needs to be able to act on your homeserver as *itself*, and to give its dispatched agents
...
Hagency and nobody can natively register a lookalike agent account — keep it that
...
1. Re-issue with the flag, keeping the same address:
   ```bash
   curl -X POST "$HAGENCY/api/project-sides/<side>/registration-file" \
     -H "Authorization: Bearer $API_TOKEN" -H 'Content-Type: application/json' \
     -d '{"url":"http://<hagency host>:8009","exclusive":true}'
   ```
...
**`<hagency host>` is reachable FROM YOUR HOMESERVER, which is not always where you are typing.** If your
homeserver runs in a container, `127.0.0.1` there is the container itself and Hagency will never receive a
...
```bash
curl -s -o /dev/null -w '%{http_code}\n' http://<hagency host>:8009/
# 403 is the right answer — Hagency refusing an unauthenticated GET means it is listening.
...
**A registration token** is the alternative: Hagency registers a representative account and one account
per agent, each with its own access token. More accounts, no appservice, no restart.

Whichever you choose, the credential is **write-only** from Hagency's console: it can be entered and
replaced, and no endpoint will ever hand it back. To check whether it works you read a verdict
(`accessState`), never the token.

### If Hagency is not reachable from your homeserver — run the doorway instead
...
`HAGENCY_APPSERVICE_SYNC_SIDE` and `HAGENCY_APPSERVICE_SYNC_URL` on the bridge and it logs in
...
An appservice is INBOUND: your homeserver pushes transactions to the `url` above, so Hagency has to be
...
**A registration token instead** (see the alternative below). Purely outbound: Hagency connects to you.
Nothing to install, nothing to restart.

**Or co-locate the appservice.** You run one small Hagency process beside your homeserver:

```bash
node bin/hagency-appservice-edge \
  --registration /path/to/the-registration.yaml \
  --link-token "<a secret you and Hagency share>" \
  --port 8095
```

and the registration's `url` points at THAT, on your own machine. Hagency then dials it to collect. Your
homeserver never leaves your host, and Hagency never accepts a connection.

`--check` reports whether your homeserver has ever called and whether Hagency has ever collected, which
...
parties: your homeserver, from inside its own namespace, and Hagency, from outside. Putting the doorway in
...
Hagency only if that port is PUBLISHED on the container that owns the namespace (`-p 127.0.0.1:8095:8095`
...
Hagency still collected nothing. `--check` distinguishes them — "the homeserver has never called" versus
"the homeserver reaches me, but Hagency has never collected".

**What that process is and is not.** It holds no fleet credential, stores nothing, and makes no decision —
which agent, which room, whether to answer at all stays in Hagency. It authenticates your homeserver with
the `hs_token` from the registration already on your disk, and authenticates Hagency with a separate link
...
**It answers your homeserver only after Hagency has processed the events.** Matrix retries on anything but
...
The room where you talk to Hagency — where you ask for work — must not be encrypted.

This is not a preference. An appservice has **no crypto store**, so an encrypted room is one Hagency's
intake cannot read at all. It will not fail loudly on your side; it will simply never see your messages.
Hagency raises an alarm when it detects this, and there is no fix after the fact: **encryption cannot be
...
Hagency's representative knocks. You accept by inviting it. It joins, and from that moment your requests
in that room reach Hagency.

A knock is a **pull** — Hagency asks and waits. It is not access, and accepting it is not accepting any
...
When Hagency enters a room *you* created, it arrives with `users_default` power — normally **0**. A
...
{
  "invite": 50,
  "users": { "@you:your-server": 100, "@hagency:your-server": 50 }
}
```

or invite each agent yourself when Hagency tells you which one it assigned. The approval response names
it (`roomAdmission.mxid`), so it is not a guess.

If you skip this, Hagency will not fail silently: the approval answers
...
Admission happens at **acceptance**: Hagency checks your allocation when an engagement is approved, not
...
The gate stops new ones. An agent can therefore be past a ceiling you lowered, and Hagency shows and
pages that state rather than hiding it.

## What Hagency will not do
...
- **It will not delete your rooms.** Rooms Hagency creates on your server stay yours to remove.
...
**A repeated invite is not a new event.** If Hagency's representative was already invited and you invite
it again, most homeservers record nothing new — so nothing is pushed to Hagency and nothing happens.
Kick it and invite again, or send any message in the room, to produce an event the appservice can see.

**The `hs_token` in your registration file is the one Hagency must be told.** Getting it wrong is refused
with a fingerprint pair in Hagency's log (`presented …, configured …`) rather than a hint about which of
the two is stale — and after you fix it, Hagency's bridge picks the new value up on its next refresh,
about a minute.
...
- Nothing detects a **federating** homeserver in production yet in a way that has been proven live —
  Hagency's own deployment has a single homeserver, so the federation path is tested but unwitnessed.
- If your side is removed from Hagency, its own records are swept, but the **bridge's local room state**
  is not — stale rooms can linger on Hagency's side, harmless and untidy.
