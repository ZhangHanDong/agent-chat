# Hagency as a PDU — capability review against the digital-labour model

What Hagency is, stated as a business function rather than a feature list, and how much of
...
**Hagency is a resource connector: it takes in coding agents wherever they run, classifies
...
phases, is the customer's own business, conducted in Matrix. Hagency supplies the workforce;
...
| agents, tasks, dispatch, pool, alerts, presets, frameworks | agent token / loopback | **Hagency** |

A project is a Matrix room (`bridge-matrix.js` keeps `roomGroupMap`/`groupRoomMap`, and
`approval-store.js:255` filters bindings on `project || projectRoomId`). Hagency reads it and
...
| Paneless ACP onboarding: model-flag pre-validation, agent-token provisioning at mode 600, profile registration or detached host, readiness-marker wait with **sustained** health sampling | `bin/hagency-acp-up`, `docs/agent-onboarding.md` §What onboarding does | works |
| tmux onboarding | `hagency up` | works |
...
team remembering more. Hagency has more of this than any other PDU function — and it is split
...
| what Hagency must record | usage per lease + a rate table | plan type, window length, consumption in window, reset time |
...
Hagency never sees a credential by design: the agent authenticates itself before it joins,
...
fields; `hagency acp-up`, `hagency up`, `register-agents` and the ACP host all send neither.
...
Hagency has invested in most, and the gap is not the one it first appears to be. **The
