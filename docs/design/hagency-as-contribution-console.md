# Hagency as a resource-contribution console — UX design for review
...
[`../PRD-hagency-pdu.md`](../PRD-hagency-pdu.md). Read the ADR for the build order and the
...
a PDU manager staffing projects. That was wrong about who Hagency serves.
...
| item | state | evidence |
|---|---|---|
| agent registry | ✅ | `GET /api/agents`, `agents` in `backend-v2.js:2906`-style store; `bin/hagency-ls` |
...
| capability / sandbox policy | ✅ | per-framework `guards` — Hagency refuses `--yolo`, `--danger-full-access`, `--dangerously-skip-permissions` |
...
1. **框架** — from the five real manifests. Shows the manifest's `permissionSummary` and the
   flags Hagency will refuse, so the contributor sees the sandbox they are lending.
...
3. **Editing the whitelist is a privilege escalation, not a preference.** Adding a project
   ...
   `TRUSTED_HAGENCY_COORDINATION_TOOLS` in `lib/codex-permission-hook.js:15` is a closed
