...
- do not edit or synchronize `bin/hagency-up` and `remote/bin/hagency-up` in Phase 0/1.
...
- Root `bin/hagency` is the full checkout CLI.
- `remote/bin/hagency` is a remote-profile CLI and must advertise only commands included in the remote package.
- `remote/bin/hagency-up` is temporarily profile-specific and frozen until launch work clears.
...
| --- | --- | --- |
| `hagency up` | Start or resume a tmux agent on the current runtime host. | Same, using remote launch profile. |
| `hagency down` | Stop a tmux agent on the current runtime host; backend update is best effort. | Same. |
| `hagency ls` | Host-local session view plus backend context. | Host-local session view plus central backend context. |
| `hagency send` | Inject into a host-local target. | Inject into a host-local target. |
| `hagency update` | Checkout/runtime update helper; service side effects depend on local install. | Remote checkout/package update helper. |
| `hagency service` | Service control for configured local/remote profiles. | Remote service control only. |
| `hagency verify-remote` | Verify a remote runtime host from checkout. | Verify this remote host against central backend. |
| `hagency check-mcp` | Full checkout MCP wiring check. | Not packaged in Phase 1. |
| `hagency up-v1`, `project`, `graph`, `resume-id`, `benchmark` | Full checkout commands. | Not packaged in Phase 1. |
...
- remote sync checks may pass while explicitly treating `remote/bin/hagency-up` as profile-specific pending Phase 5.
