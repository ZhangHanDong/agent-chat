...
- Keep launch/remote-hagency-up repairs coordinated with the active bash 3.2/tmux launch work.
...
- R-015: make `hagency-service` compatible with macOS bash 3.2.

Deferred for this pass:

- R-012: move MCP media cache out of project CWD.
- R-013: allow emergency local `hagency-down --kill` during backend outage.
...
bash -n bin/hagency-down bin/hagency-service
```
...
- Remote launch files are under active work. Do not touch remote `hagency-up` until ac-topleader/operator clears that work.
