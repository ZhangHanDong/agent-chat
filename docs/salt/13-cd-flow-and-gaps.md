...
- `hagency-push-relay` was restarted through `hagency service restart --profile remote`.
- `hagency verify-remote --no-service --samples 2 --interval 16 --agent salt` passed. A 3 second interval failed because it was below the 15 second heartbeat cadence, which is expected.
- `/api/servers` reported `kamico-MBP` online with `version=f24cb17`.
- `hagency cli status aoi`, `hagency cli status nazuna`, and `hagency cli status yamori` agreed with raw `/api/agents/<name>` runtime fields: inactive panes reported `activeNow=false`.
- `hagency cli status salt` stayed active while tools were running, which is correct because the active Codex pane content is changing.
...
- `scripts/hagency-stable-autodeploy.sh:12` defaults `HAGENCY_RELEASE_GATE` to `none`.
- `scripts/hagency-stable-autodeploy.sh:220` supports a `worktree` release gate that prepares a detached target worktree and runs `npm ci`.
- `scripts/hagency-stable-autodeploy.sh:243` runs `verify:cd-preflight` in that gate before `scripts/hagency-stable-autodeploy.sh:343` resets the live checkout.

Impact:

A bad commit pushed to `stable` can still deploy on hosts that keep the default `HAGENCY_RELEASE_GATE=none`. The worktree gate exists, but CD policy does not yet require it or consume GitHub check-run status.
...
- `scripts/hagency-remote-autodeploy.sh:134` defines `verify_remote_deploy`.
- `scripts/hagency-remote-autodeploy.sh:138` passes `--api`, `--server`, deploy-safe `--samples`, `--interval`, `--service`, and `--expect-version`.
- `scripts/hagency-remote-autodeploy.sh:160` passes `--token` when `API_TOKEN` is configured, and `scripts/hagency-remote-autodeploy.sh:163` passes optional `VERIFY_AGENT`.
- `scripts/hagency-remote-autodeploy.sh:262` calls verification only after relay restart succeeds.
- `scripts/hagency-remote-autodeploy.sh:265` keeps `deploy_pending=true` when verification fails, so the next poll retries instead of declaring success.
...
- `scripts/hagency-remote-autodeploy.sh:72` sets the dependency install target to `$REPO_DIR/remote`.
- `scripts/hagency-remote-autodeploy.sh:87` watches `remote/package.json` and `remote/package-lock.json`.
...
- `scripts/hagency-stable-autodeploy.sh:127` stores deploy state under the checkout's git dir by default.
- `scripts/hagency-stable-autodeploy.sh:199` touches the install-needed marker before install and `scripts/hagency-stable-autodeploy.sh:352` uses that marker to force retry from the last successful ref.
- `scripts/hagency-remote-autodeploy.sh:46` stores remote deploy state and `scripts/hagency-remote-autodeploy.sh:192` forces install retry when the marker remains.

### CD-003 macOS Remote Hosts Do Not Get An Autodeploy Watcher

Evidence:

- `remote/install-remote.sh:190` installs `hagency-remote-autodeploy` only in the Linux/systemd branch.
...
macOS remote hosts can run push-relay, but they do not get the polling CD watcher from `install-remote.sh`. They rely on manual `hagency update` or operator restarts, so remote/local CD behavior diverges.
...
- or add a launchd plist/runner for `hagency-remote-autodeploy.sh`.
...
- `scripts/hagency-remote-autodeploy.sh:245` computes the expected short commit after reset.
- `scripts/hagency-remote-autodeploy.sh:262` passes that expected version to post-restart verification.
...
- `scripts/check-remote-package-smoke.sh:231` checks the generated MCP wrapper with `HAGENCY_WRAPPER_SMOKE=1`.
...
- `bin/hagency-cli:81` builds a tmux snapshot from `#{session_activity}`.
...
Implemented for the current backend/CLI contract. Backend runtime now preserves `activeNow=null`, and `hagency cli status` displays unknown instead of idle for that state. RLP idle repair now keeps normal/high push notifications queued while the pane shows interactive busy markers and removes max-hold force delivery for active panes; urgent remains the explicit bypass. The legacy `session_activity` fallback remains only for older backends that omit the activity field entirely.
...
- `scripts/hagency-stable-autodeploy.sh:208` runs a dirty-worktree cleaner.
- `scripts/hagency-stable-autodeploy.sh:343` resets the live checkout to `origin/stable`.
...
For the remote profile, the deploy service is `hagency-push-relay`.

### Stage D: Post-Deploy Verification

Runs after service restart:

- `hagency service status --profile remote`;
- `hagency verify-remote --samples 2 --interval 16`;
- compare backend `/api/servers[].version` to the expected short commit;
- when a known agent is configured, run `hagency verify-remote --agent <name>`;
- run `hagency cli status <agent>` or `hagency cli status` as a human-readable diagnostic smoke.
...
| CD-003 | P1 | macOS CD | Decide and implement launchd autodeploy watcher or document manual-only policy. | macOS launchctl status and `hagency service status --profile remote`. |
