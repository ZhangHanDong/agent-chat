...
```json
{
  ...
  "status": "pending",
  "targetAgent": "hagency-worker",
  "targetServer": "local",
  "target": {
    "kind": "tmux",
    "tmux": "hagency-worker:0.0"
  },
  "source": {
    "messageId": "msg_...",
    "messageIds": ["msg_..."],
...
```json
{
  ...
  "mcpPresent": true,
  "paneCommand": "codex",
  "workspacePath": "/home/shisui/laplace/hagency"
}
```
...
- Ensure central machine runs `hagency-push-relay.service` as the local runtime host.
...
### `bin/hagency-up` And `remote/bin/hagency-up`

Not part of delivery queue migration, but required for full local/remote unification:

- both launchers must inject the same `HAGENCY_SERVER` into MCP;
...
- missing `HAGENCY_SERVER` in remote profile fails startup or registers no agent.
...
- Keep old `server.js /api/queue` delivery path behind a feature flag such as `HAGENCY_LEGACY_WEB_QUEUE_DELIVERY=1`.
- New backend task creation can be disabled with `HAGENCY_DELIVERY_QUEUE_ENABLED=0`.
