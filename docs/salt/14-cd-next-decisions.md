...
4. `hagency verify-remote --expect-version <short-sha>` can verify heartbeat continuity and loaded remote relay commit after deployment.
...
1. stable/live release gate support exists but must be enabled explicitly with `HAGENCY_RELEASE_GATE=worktree` on the deploy host.
...
- salt is effectively on the remote/client side and can observe remote relay, MCP wrapper, loaded commit, heartbeat, and CLI status behavior through hagency.
...
`scripts/hagency-stable-autodeploy.sh` fetches `origin/stable` and resets the live checkout without waiting for the target commit's CI result.
...
1. `HAGENCY_RELEASE_GATE=none|worktree` now exists, defaulting to `none` until the live deploy service opts in;
...
After this batch reaches stable, ac-topleader should decide when to set `HAGENCY_RELEASE_GATE=worktree` in the live stable autodeploy environment. Until that is configured, the code path exists and is tested but the live watcher keeps current behavior.
...
- `scripts/hagency-stable-autodeploy.sh`
- `scripts/hagency-remote-autodeploy.sh`
...
1. `HAGENCY_DEPLOY_STATE_DIR` defaults under the absolute git dir at `.git/hagency-autodeploy`, so `git clean -fd` in the live checkout does not delete deploy state;
...
Resolved for the current remote profile. `scripts/hagency-remote-autodeploy.sh` watches `remote/package.json` and `remote/package-lock.json`, then runs `npm install --omit=dev` inside `remote/`. Root `package*.json` changes still deploy and restart the relay, but they do not trigger a root dependency install.
...
Resolved for the current git-checkout remote profile. `scripts/hagency-remote-autodeploy.sh` no longer treats service-manager restart success alone as deploy success.
...
Linux remote install provisions `hagency-remote-autodeploy` as a systemd service. macOS remote install provisions only the push-relay launchd service.
...
If macOS remote hosts are first-class deploy targets, add a launchd watcher. If they are mainly developer/client machines, document manual-update-only and require `hagency update` plus `verify-remote` for macOS updates.
...
`remote/install-remote.sh` can run from a full source clone. In that case it currently prefers the root `bin/` directory when root `bin/hagency-up` exists, so the installed `hagency` helper can expose the root command surface instead of the remote-scoped `remote/bin/hagency` surface. The generated remote package smoke validates the remote-scoped command surface, not this full-clone install path.

Options:

1. Remote profile always links `remote/bin/hagency`, even from a full clone.
2. Full clone remotes intentionally expose the root command surface and docs/tests say so.
3. Add an explicit install flag, such as `HAGENCY_REMOTE_BIN_PROFILE=root|remote`, and require operators to choose.
...
6. A manual two-sided acceptance run after stable merge:
   ...
   - remote side: run `hagency verify-remote --samples 2 --interval 16 --expect-version <short-sha>` and spot-check `hagency cli status`.
...
Suggested CD-A harness knobs for `scripts/hagency-stable-autodeploy.sh`:

1. `HAGENCY_ONCE=1` to execute one poll/deploy cycle and exit;
2. `HAGENCY_SYSTEMCTL_BIN=<path>` or a wrapper function so tests can fake restart and status checks;
3. `HAGENCY_NPM_BIN=<path>` or a wrapper function so tests can force install pass/fail;
4. `HAGENCY_RELEASE_GATE=none|worktree`;
5. `HAGENCY_DEPLOY_STATE_DIR=<tmpdir>`.
...
1. Batch CD-A: stable release gate and dependency retry state for `scripts/hagency-stable-autodeploy.sh` only, with fake-repo tests. Implemented.
