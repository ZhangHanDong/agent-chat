...
Root docs such as `README.md`, `OPERATIONS.md`, `remote/README.md`, and `skills/hagency/SKILL.md` are later synchronization targets, not Phase 0 edit targets.

## Phase 1: Make Package Shape Honest

Type: packaging/docs/tests. Avoid `hagency-up` internals until approved.
...
- `remote/bin/hagency-up` drift is under active launch work and is explicitly deferred.
...
- Make `HAGENCY_SERVER` explicit for every runtime host.
...
- Make `HAGENCY_AGENT_STATE_DIR` a derived injected value, not a human-authored truth source.
...
- Tests cover runtime-dir derived home root and explicit `HAGENCY_HOMEDIR`.
...
Type: launcher refactor after active `hagency-up` work clears.

Objectives:

- Extract launch responsibilities from `hagency-up` into testable pieces:
  - profile config;
  - v1 home resolver;
...
- Keep `bin/hagency-up` and `remote/bin/hagency-up` untouched until this phase is explicitly approved.
...
- Make `hagency service` require explicit profile or derive safe profile from install metadata.
- Rename or scope `hagency update` if it remains remote-specific.
- Make `hagency-ls` explicitly distinguish host-local sessions from backend registry.
- Make `hagency-down` clearly host-local.
- Advertise `check-mcp` only where it is packaged.

Exit criteria:

- `hagency <command> --help` states profile support.
...
Phase 3 and Phase 4 can partly run in parallel after Phase 2 starts, but Phase 5 should wait until active `hagency-up` work is no longer in flight.
