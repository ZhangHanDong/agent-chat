...
5. remote loaded-version verification is exposed as `hagency verify-remote --expect-version <short-sha>`.
...
2. do not edit `bin/hagency-up` or `remote/bin/hagency-up` until Phase 5 is approved;
...
| Phase 5 launch | Explicitly frozen because `hagency-up` launch work was active. | Keep frozen; only write design/tests until approval clears launch files. | Required |
| Phase 6 CLI/ops profile | Remote command honesty is enforced; RLP6-A made `service`, `update`, `ls`, and `down` help/docs profile-scoped; CLI status/list now report unknown runtime activity without treating it as idle; RLP6-B aligned `hagency-ls --all` v1 home discovery with the shared resolver. | Continue with `hagency-down` resolver tests only after separate approval; do not change shutdown safety behavior implicitly. | Required |
...
Make the central-local runtime host visible through the same `servers.json` liveness model without changing delivery. This can be done by recording a local server row for `HAGENCY_SERVER` and associating local agents with it.
...
3. added opt-in `HAGENCY_RECORD_LOCAL_SERVER=1` to record a central-local server row;
...
1. document and test the exact `HAGENCY_AGENT_TOKEN_MODE` behavior;
...
2. document that heartbeat/offline/runtime report currently use `API_TOKEN` compatibility bearer and `HAGENCY_SERVER_TOKEN` is not accepted or enforced yet;
...
1. introduce a server/relay credential such as `HAGENCY_SERVER_TOKEN` or a per-server token store;
...
3. non-local callers must send `Authorization: Bearer $HAGENCY_DASHBOARD_TOKEN`;
...
1. compute MCP media cache under `HAGENCY_AGENT_STATE_DIR` when available;
2. otherwise use a stable runtime data dir derived from `HAGENCY_RUNTIME_DIR` or the hagency home;
...
Completed batch RLP4-B: v1 home/runtime resolver tests.

Scope:

1. add tests for `lib/agent-home-v1.js` env precedence and absolute normalization;
2. preserve legacy `~/.hagency` fallback behavior;
3. do not change launch files or provisioning behavior in the same batch.

Implementation:

1. `HAGENCY_HOMEDIR` wins only when it is an absolute path;
2. `HAGENCY_RUNTIME_DIR` contributes `<runtime>/homes` only when it is absolute;
3. relative manifest `homeDir`/`stateDir`/`workdir` values are rejected instead of being resolved under the caller cwd;
4. legacy `~/.hagency` remains the fallback lookup root;
...
- Phase 1 made root and remote `hagency` command surfaces honest.
...
1. make `hagency service`, `hagency update`, `hagency-ls`, and `hagency-down` help text explicit about current-host services, remote relay, backend registry context, and local tmux shutdown scope;
...
1. align `hagency-ls` home/runtime defaults with `lib/agent-home-v1.js` semantics without touching launch;
2. add shell-level fake `tmux`/`curl` tests for `hagency-ls --all`;
3. keep `hagency-down` shutdown behavior unchanged.

Implemented:

1. `hagency-ls --all` now searches absolute `HAGENCY_HOMEDIR`, absolute `HAGENCY_RUNTIME_DIR/homes`, and legacy `~/.hagency` fallback;
2. relative `HAGENCY_HOMEDIR` is ignored for v1 discovery, matching `lib/agent-home-v1.js`;
3. root and remote mirrored `hagency-ls` stay synchronized;
4. `hagency-down` resolver/backend-unavailable behavior remains a future test/design item because changing local shutdown safety requires explicit approval.
...
- Live release gate enforcement still requires deploy-host configuration: `HAGENCY_RELEASE_GATE=worktree`.
...
4. after stable deployment, verify the remote side with `hagency verify-remote --expect-version <short-sha>`;
...
5. CD-A stable release gate and dependency retry. Implemented after operator resumed CD work; pending ac-topleader acceptance and deploy-host opt-in for `HAGENCY_RELEASE_GATE=worktree`.
...
3. RLP6 `hagency-down` resolver/backend-unavailable tests, pending shutdown-safety approval;
