...
- writes `mcp-server.pid` under `HAGENCY_AGENT_STATE_DIR` when available.
...
- `push-relay` can also run locally and consume backend SSE, but its default `HAGENCY_SERVER` differs from backend/MCP defaults.
...
- `hagency` helpers manage local tmux agents and verify the central record.
...
| Server id | Backend/MCP default to `local`; relay defaults to hostname. | Remote expects explicit `HAGENCY_SERVER`. | Same env var has inconsistent defaults and meanings. |
...
bash scripts/check-remote-sync.sh
  bin/hagency != remote/bin/hagency
  bin/hagency-service != remote/bin/hagency-service
  bin/hagency-up != remote/bin/hagency-up
  generated remote package out of date

bash scripts/build-remote-package.sh --check
  remote/bin/hagency drifted
  remote/bin/hagency-service drifted
  remote/bin/hagency-up drifted
  remote/lib/push-relay-core.js missing
  remote/lib/mcp-server-core.js drifted
```

`remote/bin/hagency` advertises commands that the remote package does not contain, including `hagency-up-v1`, `hagency-project`, `hagency-graph`, `hagency-benchmark`, `check-mcp`, and `hagency-resume-id`. Some included commands also assume git-checkout layout or root `remote/install-remote.sh`, which a standalone generated package does not have.
...
`API_TOKEN` currently means backend admin bearer, remote relay credential, and dashboard backend proxy credential. `HAGENCY_SERVER` currently means backend local id, relay id, MCP auto-register id, and dashboard locality check. These overloads make profile behavior hard to reason about and hard to secure.
...
Commands such as `hagency update`, `hagency service`, `hagency-up`, `hagency-down`, `hagency-ls`, and `check-mcp` do not clearly state whether they operate on central services, remote relay services, local tmux sessions, or backend registry records.
