...
- Agent registration is launched primarily by `hagency-up`, with MCP core doing best-effort auto-registration.
...
- Remote `hagency` advertises commands whose scripts are absent from `remote/bin`.
- Codex MCP launch auth can be incomplete for non-v1 agents because explicit injected env omits `API_TOKEN`; this touches active `hagency-up` work and must be coordinated before any edit.
- MCP media cache is rooted in the current working directory, which can pollute project repos.
- `hagency-down --kill` can still be blocked by backend unavailability before local tmux kill.
- Remote `hagency-up` has drifted from root `hagency-up`; this touches active launch work and must be coordinated before any edit.
- `hagency-service` uses bash associative arrays and may fail on macOS bash 3.2.
...
- Historical finding: dashboard `server.js` acted as an unauthenticated privileged proxy that could call backend with `API_TOKEN` and write into tmux through queue/delivery APIs. RLP3-B1 now gates mutating dashboard APIs to loopback or `HAGENCY_DASHBOARD_TOKEN`; full browser web auth remains separate.
...
- Use the phrase "hagency is a stateful-individual chat kernel" as the documentation anchor, with Task/Supervisor/Matrix/Dashboard/Remote relay as replaceable edges.
...
### Descartes: R-015 hagency-service Bash compatibility
...
- Remote `hagency` advertises commands not included in the remote package.
...
- `HAGENCY_SERVER` currently conflates backend local id, relay id, MCP auto-register id, and dashboard locality checks.
...
- `hagency update`, `hagency service`, `hagency-up`, `hagency-down`, `hagency-ls`, and `check-mcp` are the most confusing operator surfaces.
...
- Make `remote/bin/hagency` advertise only packaged commands.
...
- Treat `bin/hagency` and `remote/bin/hagency` as profile-specific, not byte-for-byte mirrors.
- Keep `remote/bin/hagency-up` drift outside this batch; launch work remains deferred.
