...
- Dashboard calls backend with `API_TOKEN`. RLP3-B1 gates mutating dashboard `/api/*` surfaces to loopback callers or `HAGENCY_DASHBOARD_TOKEN`, but there is still no full browser login/session model.
