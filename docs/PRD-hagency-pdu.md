# PRD — Hagency as a digital-labour resource plane
...
>    *retained* models. ADR-013's 2026-08-10 amendment withdrew both. Hagency's unit of account is the
...
> above.** ADR-013 is the amending record this PRD demands in §2.1 and §R0. It rules that Hagency's
...
| [`design/hagency-as-contribution-console.md`](design/hagency-as-contribution-console.md) | the UX design ADR-013 distils, with the per-layer exists/absent grounding table behind every claim |
| [`design/hagency-as-pdu.md`](design/hagency-as-pdu.md) | the capability review this PRD rests on: per-function inventory with `file:line` evidence, and the three-memories diagnosis behind §6.5 |
...
**Hagency will be the resource plane and workforce manager for digital coding agents.** It will
...
~~Hagency is analogous to a digital-employee outsourcing house or PDU. The analogy defines its
responsibility for hiring, qualification, staffing, capacity, health, cost, performance evidence and
development. It does not make Hagency the owner of the customer's product lifecycle.~~

> **`[W]` §8 — the product statement.** The struck clause and the whole PDU/outsourcing-house
> analogy are withdrawn. ADR-013 rules that Hagency's user is the **resource contributor** — 带资入组
...
> Hagency a staffing request; a project invites an agent, and the contributor accepts. The retained
...
At the implementation baseline, Accepted ADR-009 and REQ-PROJECT-BOARD define an Hagency group as
...
Hagency may publish resource status and delivery claims into Matrix. Those messages are projections,
...
| `[W]` §8 | `AssignmentRequest` | Matrix/Robrix2 creates `request_id` and authoritative source event; Hagency stores a digest-pinned received copy | asks Hagency for one qualified allocation under explicit scope |
| `[W]` §8 | `StaffingAssignment` | Hagency creates `assignment_id` | records who/what was assigned, why and under which commercial policy |
...
| `[W]` §8·dep | `DeliveryAcceptanceObservation` | Matrix/Robrix2 creates the authoritative event; Hagency stores a verified projection | records accepted or rework outcome; Hagency cannot originate it |
...
> project's staffing request to a Hagency allocation that is gone. What replaces the request direction
...
flowchart LR
  ...
  subgraph FLEET["Resource plane — Hagency"]
    PROFILE[Qualified agent profiles]
    ASSIGN[Staffing assignment]
    HOLD[Durable Hagency resource hold]
...
Credential material is not a Hagency business object, but credential binding and security are in
...
|---|---|---|
| Connect | Five framework manifests and Hagency ACP launch paths exist; ACP onboarding provisions an agent token, registers a profile and performs sustained health sampling | supported/launchable status differs by adapter; qualification is largely transport liveness |
...
Hagency must accept an immutable, idempotent `AssignmentRequest` and create a durable
...
payload digest and revocation. Matrix-to-Hagency delivery uses a durable inbox/outbox and replay
protection.
...
Hagency generates `assignment_id` only after request validation. PDU MVP requests exactly one
...
| Delivery acceptance | authoritative Matrix event: `accepted` or `rework_requested`; Hagency stores a source-event/digest-pinned observation and cannot originate or silently revoke it |
...
delivery digest, actor and binding version and is idempotently projected into Hagency.
...
Building on R0's Phase 2 assignment/event ledger `[W]` §8, Hagency persists append-only capacity intervals
...
> ADR-013 decision 5 makes the seat the **accounting root**, because `bin/hagency-up` unsets
...
*Acceptance A-R8-2 (Phase 2):* `[W]` §8·dep — the saga's Hagency half holds an *assignment*'s
resources and its router half is the withdrawn execution path. Hagency-owned agent/seat/budget holds and router-owned workspace/named-resource
...
> — is denominated in money, which the 2026-08-10 amendment puts outside Hagency altogether. "Hagency's
> unit of account is the **token**. Converting tokens to money depends on the contract, the plan, the
> region and the negotiated rate — none of which Hagency observes." Nothing in R11 survives that with
...
Hagency presents an internal, read-only resource lens: assignments, agent-opened issues/change
...
typed child-room bindings. The PDU MVP beta may configure these bindings manually; Hagency never infers authority
...
Full evidence in [`design/hagency-as-pdu.md`](design/hagency-as-pdu.md) §5.
...
| `[W]` §8 | `AssignmentRequest` / received copy | client request ID and source event; Hagency digest-pinned received copy, demand, budget, deliverables and idempotency | R0 |
| `[W]` §8 | `StaffingAssignment` | Hagency assignment ID, lifecycle, selected offer, policy/qualification versions and task mapping | R0, R9, R15 |
...
| `[W]` §8·dep | `ResourceHold` / `ResourceLease` | Hagency hold and router lease joined by saga ID, fence, expiry and compensation state | R0, R8 |
...
restart reconciliation; Hagency hold ↔ router lease saga; deterministic eligibility/static-capacity
...
- ~~accrued costs reconcile against provider invoices with approved variance/correction handling;~~
  `[W]` §8·am10 — Hagency never sees an invoice;
...
|---|---|---|---|---|---|
| ~~R0 / A-R0-1~~ | ~~0 contract, 2 runtime~~ | request SoR, digest and idempotency | CT/IT-R0-IDEMP | Hagency + Robrix2 owners | **`[W]` §8 — withdrawn, do not schedule** |
...
| ~~R0 / A-R0-5..6~~ | ~~0 contract, 2 runtime~~ | claim/acceptance event contracts | CT/IT-R0-ACCEPT | Matrix + Hagency owners | **`[W]` §8 — withdrawn, do not schedule** |
...
| R3 / A-R3-1..2 | 1 core, 5 reporting | versioned profile and pinned assignment | IT-R3-VERSION | Hagency owner | TBD before approval |
| R4 / A-R4-1..2 | 1 | QualificationEvidence | E2E-R4-QUALIFY | Fleet operator | TBD before approval |
| R5 / A-R5-1..2 | 1 | detection API and i18n | CT/UI-R5-DETECT | Hagency UI owner | TBD before approval |
...
| ~~R8 / A-R8-2~~ | ~~0 contract, 2 runtime~~ | hold/lease saga | IT-R8-SAGA | Hagency + Router owners | **`[W]` §8·dep — withdrawn, do not schedule** |
...
| R14 / A-R14-1..3 | 0 governance, 2 runtime | superseding project ADR/REQ, ProjectRef; existing project lens in `mockup/app/projects` | CT/E2E-R14-SPACE | Matrix + Hagency owners | TBD before approval |
...
| Customer phase/stage engine | belongs to the customer project plane; workflow IDs are not a Hagency lifecycle | separate Matrix/Robrix2 product decision |
...
| D02 | Amend/supersede ADR-009 and REQ-PROJECT-BOARD for stable ProjectRef, Space and typed rooms | Matrix + Hagency owners | proposed | Phase 0 | project identity/cost attribution cannot proceed |
| D03 | ~~Hagency hold and router lease use a fail-closed saga; no cross-store atomicity claim~~ | Hagency + Router owners | **`[W]` §8·dep — withdrawn** | ~~Phase 0~~ | none while there is no assignment to hold resources for |
...
- Matrix/Robrix2 is the customer collaboration surface; Hagency Dashboard is the internal workforce
  and operator surface. **`[W]` §8 in part** — the Hagency console's user is the contributor, and it is
...
5. ~~The Hagency-hold/router-lease saga has a single source of truth for each resource, canonical
...
describes a product Hagency is not building, with the parts that were withdrawn marked. Marked is
