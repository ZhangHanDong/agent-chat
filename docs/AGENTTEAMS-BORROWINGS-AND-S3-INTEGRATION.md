...
- 范围:**robrix2 + hagency + agent-spec(核心)/ ARC(合作)**;
...
1. **核心项目定标准,合作项目接标准**:agentd/hagency 与 agent-spec 是
   重心资产,负责协议、合约与验收把门;ARC 是合作项目,经由稳定协议面接入
   (serve mode 已证明依赖方向正确:是 ARC 适配了 hagency 的方言)。
...
| 项目 | 定位 | 投资姿势 |
|---|---|---|
| hagency / agentd | 核心:运行时与执行面 | 深耕(下表批次) |
...
| # | 病灶 | 修法 |
|---|---|---|
| N1 | `MATRIX_IGNORED_SENDER_MXIDS` 精确黑名单 = O(N²) 维护 | **反转为 O(1) 白名单**:bridge 默认只路由"人类 + 本团队 agents",其余忽略——代码缺口,hagency 小 PR(接替原 F1 的黑名单生成思路) |
...
- provision-team 成员开通脚本(hagency master)
- 三节工作流约定:gh 集成 / 多人协作 / shared-room(robrix2 #259)
- 共享房三件套:`MATRIX_DEFAULT_WAKE=off`、`!bindroom`(hagency master)、
...
| F6 hagency-embedded 镜像 | Palpo + 四件套单镜像 supervisord,成员开通 = docker run |
