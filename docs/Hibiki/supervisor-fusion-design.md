...
3. **扩展而非改造** — 在 subconscious 的基础上增加 hagency 感知，不重写核心逻辑
...
### 方向 1：让 Subconscious 感知 hagency 工作流
...
**实现方式：** fork claude-subconscious 的 `send_messages_to_letta.ts`，在构造 XML 前读取 `docs/{agent}/plan.md` 和 `docs/{agent}/agents.md`。agent 名从环境变量 `AGENT_NAME` 获取（hagency-up 启动时注入）。

### 方向 2：Subconscious 事件上报到 hagency 审计 UI

当前审计 UI 消费 supervisor 的事件格式。让 subconscious 也能产生兼容的事件，审计 UI 就能展示 subconscious 的判断。

#### 方案：Letta guidance → 审计事件

Subconscious 通过 `guidance` memory block 表达判断。新增一个机制：每当 guidance block 变化时，解析其中的焦点判断并推送到 hagency。

**在 sync_letta_memory.ts 中增加逻辑：**

当检测到 `guidance` block 变化且包含焦点相关内容时，POST 到 hagency：
...
if (guidanceChanged && guidanceContent.includes('[focus:')) {
  ...
  // POST to hagency backend
  await fetch(`${HAGENCY_URL}/api/supervisor/events`, {
    method: 'POST',
...
export HAGENCY_URL="http://localhost:8090"
```
...
### Q4: 和现有 hagency MCP 的关系？

Subconscious 的 hooks 和 hagency 的 MCP server 是独立的：
- **MCP** 提供 agent 通讯能力（send_message, post, check_inbox）
- **Hooks** 提供观察和注入能力（观察 transcript，注入 guidance）

两者不冲突。hagency-up 同时配置 MCP server 和 hooks 即可。
...
| `scripts/sync_letta_memory.ts` | 检测 guidance 变化时 POST 事件到 hagency |
| `hooks/hooks.json` | 可能需要调整 hook 命令以支持 AGENT_NAME 环境变量 |
| `scripts/agent_config.ts` | 支持从 hagency 的 data/agents/ 读取 Letta agent ID |
...
| `bin/hagency-up` | 新增 Letta agent 初始化 + hooks 配置逻辑 |
...
| 文件 | 用途 |
|------|------|
| `subconscious/Subconscious-hagency.af` | 定制的 agent 定义（包含 workspace_conventions） |
...
Agent Claude Code 会话
  ...
  │                              ──→ 如果有 [focus:...] 标记 ──→ POST to hagency
  │                              ──→ 注入 guidance 到 Claude context
  │
  └─ hagency 审计 UI ──→ 展示事件（来源标记 subconscious）
                          ──→ 和 supervisor 事件并存
```
...
**用 claude-subconscious 的成熟 hooks 架构和 Letta 持久化记忆替代自研 supervisor 的无状态判断，通过定制 memory block 让 Letta 理解 hagency 的 plan.md 工作流，通过事件上报让审计 UI 统一展示——本质上是让每个 agent 拥有一个"知道 hagency 规矩的潜意识"。**
