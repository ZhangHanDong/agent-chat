...
Agent 通讯:
hagency (跨服务器消息) ← gastown (多Agent编排)
```
...
| **Agent 通讯** | hagency | ★★★★☆ | DM/群组/MCP/Matrix/远程推送齐全 |
...
| 子 Agent 委派 | 部分——gastown 有 Polecat 概念 | hagency 层无任务分解/派发协议 |
| 持久化策略 | 基础——JSON 文件存消息 | 无 Agent 级 memory、scratchpad、todo |
| 评估与可观测性 | 基础——活动追踪/阻塞检测 | 无 trace 日志、无回放测试、无量化评分 |

**本质问题：hagency 解决了 Agent 之间"怎么说话"，但没解决"说什么"、"为什么说"、"说了之后做什么"。**
...
```
路线 A: hagency → Agent Framework（Agent 能力升级）
...
**目标：** 在 hagency 之上构建真正的 Agentic 框架层，参考 AVB 的五维设计。
...
- **仪表盘：** 扩展 hagency 的 Web Dashboard，展示 Agent 效能指标
