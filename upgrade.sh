#!/usr/bin/env bash
# Upgrade a Hagency install to a tagged release, reverting automatically if the
...
#   --services "A B"  Services to restart (default: hagency-backend hagency hagency-push-relay)
#   --skip-gate       Skip the pre-upgrade verify:ci gate
#   --yes             Do not prompt for confirmation
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
INSTALL_DIR="${HAGENCY_INSTALL_DIR:-$SCRIPT_DIR}"
...
SKIP_GATE=false
ASSUME_YES=false
SERVICES="${HAGENCY_UPGRADE_SERVICES:-hagency-backend hagency hagency-push-relay}"
BACKEND_SERVICE="${HAGENCY_BACKEND_SERVICE:-hagency-backend}"
HEALTH_URL="${HAGENCY_BACKEND_HEALTH_URL:-http://127.0.0.1:8090/api/agents}"
HEALTH_TIMEOUT="${HAGENCY_BACKEND_HEALTH_TIMEOUT:-30}"
...
systemctl_run() {
  if [ "$(id -u)" = "0" ] || [ -n "${HAGENCY_SYSTEMCTL_BIN:-}" ] || ! command -v sudo >/dev/null 2>&1; then
    "$SYSTEMCTL_BIN" "$@"
  else
    sudo "$SYSTEMCTL_BIN" "$@"
  fi
