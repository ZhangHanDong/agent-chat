...
hagency-remote/
  ├── push-relay.js
  ├── mcp-server.js
  ├── bin/hagency-up       ← 快速起 agent 脚本
  ├── .env.example       ← HAGENCY_API, API_TOKEN
...
4. `hagency-up <name> <path>` 起 agent
...
Phase 1 和 Phase 5 可以并行。hagency-up 脚本不依赖远端支持，可以先做。
