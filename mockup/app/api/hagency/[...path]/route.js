/*
 * Server-side proxy to the hagency backend.
 *
 ...
 *    defaults to `https://hagency.example.com`, so a browser on :3100 is refused.
...
const BACKEND = process.env.HAGENCY_BACKEND ?? 'http://127.0.0.1:8090';
const TOKEN = process.env.HAGENCY_API_TOKEN ?? process.env.API_TOKEN ?? '';

/*
 * Default-deny, on the model of TRUSTED_HAGENCY_COORDINATION_TOOLS
...
const WRITES = [
   ...
   * for Hagency's own secrets living only in `.env`.
   *
   ...
   * only add homeservers Hagency already knows about — circular, since a new customer is in neither list.
   ...
   * plaintext — and must never reach a browser. This one writes that YAML to a 0600 file on the Hagency
 ...
 *
 *   sent      DELETE /api/hagency/whitelist/%252e%252e/agents/victim
 ...
 *
 *  - `HAGENCY_CONSOLE_TOKEN`, when set, is required. That is the supported way to
 ...
 *    them.
 */
const CONSOLE_TOKEN = process.env.HAGENCY_CONSOLE_TOKEN ?? '';
  ...
  if (!callerAllowed(req)) {
    return Response.json({
      error: CONSOLE_TOKEN
        ...
        : 'this proxy carries operator authority and is bound to loopback; set HAGENCY_CONSOLE_TOKEN to use it from elsewhere',
    }, { status: 403 });
  }
