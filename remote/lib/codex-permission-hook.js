...
const HOOK_DIGEST_ARGUMENT = '--hagency-hook-sha256=';
const TRUSTED_HAGENCY_COORDINATION_TOOLS = new Set([
  'mcp__hagency__whoami',
  'mcp__hagency__send_message',
  'mcp__hagency__post',
  'mcp__hagency__check_inbox',
  'mcp__hagency__check_group',
  'mcp__hagency__list_tasks',
  'mcp__hagency__get_task',
  'mcp__hagency__accept_task',
  'mcp__hagency__transition_task',
  'mcp__hagency__comment_task',
  'mcp__hagency__update_task_execution',
]);
const HAGENCY_ATTACHMENT_TOOLS = new Set([
  'mcp__hagency__send_message',
  'mcp__hagency__post',
]);

function resolveAgentToken(agentName, env = process.env) {
  const explicit = typeof env.HAGENCY_AGENT_TOKEN === 'string' ? env.HAGENCY_AGENT_TOKEN.trim() : '';
  if (explicit) return explicit;
  const stateDir = typeof env.HAGENCY_AGENT_STATE_DIR === 'string' ? env.HAGENCY_AGENT_STATE_DIR.trim() : '';
  const home = typeof env.HAGENCY_HOMEDIR === 'string' && env.HAGENCY_HOMEDIR.trim()
    ? path.resolve(env.HAGENCY_HOMEDIR.trim())
...
export function codexPermissionRequestNeedsOwnerApproval(input) {
  ...
  if (!TRUSTED_HAGENCY_COORDINATION_TOOLS.has(toolName)) return true;
  if (!HAGENCY_ATTACHMENT_TOOLS.has(toolName)) return false;

  const toolInput = input?.tool_input;
  if (!toolInput || typeof toolInput !== 'object' || Array.isArray(toolInput)) return true;
  if (!Object.prototype.hasOwnProperty.call(toolInput, 'attachments')) return false;
  return !Array.isArray(toolInput.attachments) || toolInput.attachments.length > 0;
}
...
async function decideCodexPermission({
  ...
  if (!codexPermissionRequestNeedsOwnerApproval(input)) {
    return {
      behavior: 'allow',
      approval: null,
  ...
  const apiBase = (env.HAGENCY_API || 'http://127.0.0.1:8090').trim().replace(/\/$/, '');
