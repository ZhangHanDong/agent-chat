...
export function resolveApprovalTtlMs(env = process.env) {
  const raw = Number.parseInt(env.HAGENCY_APPROVAL_TTL_MS || String(DEFAULT_APPROVAL_TTL_MS), 10);
  return Number.isFinite(raw) && raw > 0 ? Math.max(5_000, raw) : DEFAULT_APPROVAL_TTL_MS;
}

export function approvalAdapterTimeoutMs(env = process.env) {
  return resolveApprovalTtlMs(env) + (APPROVAL_HOOK_TIMEOUT_MARGIN_SECONDS * 1000);
}

export function approvalHookTimeoutSeconds(env = process.env) {
  return Math.ceil(approvalAdapterTimeoutMs(env) / 1000);
}
...
export async function resolveAndConsumeRuntimeApproval({
  ...
  const consumption = await consumeRuntimeApproval(api, outcome.approval, agent);
  return {
    ...outcome,
    consumption,
