...
import { sessionPolicyFromEnv } from './session-policy.js';
...
const DEFAULT_BACKEND_PORT = Number.isFinite(DEFAULT_BACKEND_PORT_RAW) && DEFAULT_BACKEND_PORT_RAW > 0
  ...
  : 8090;
const API_BASE = (process.env.HAGENCY_API || `http://127.0.0.1:${DEFAULT_BACKEND_PORT}`).replace(/\/$/, '');
...
// Task 8: standalone cross-component doctor. This process has no HTTP endpoint of its
// own, so push-relay.js periodically snapshots this state via getRelayHealthSnapshot()
// and writes it to data/health/push-relay.json for the doctor to read cross-process.
// Deliberately just three fields: two coarse timestamps and a short error code — never
// a token, password, or message body (lastErrorCode is always a fixed short code, never
// raw error text or payload content).
const relayHealthState = {
  lastSuccessfulBackendContactAtMs: null,
  lastSuccessfulOutboundDeliveryAtMs: null,
  lastErrorCode: null,
};
// Which tmux sessions this relay may manage. Everything downstream reads
// localAgents, including the two send-keys guards, so filtering at the single
// point where sessions are enumerated gates both discovery and delivery.
const sessionPolicy = sessionPolicyFromEnv();
for (const warning of sessionPolicy.warnings) console.warn(`[push-relay] ${warning}`);
...
function listLocalTmuxSessions() {
  ...
  try {
    ...
    const discovered = raw ? raw.split('\n').filter(Boolean) : [];
    if (sessionPolicy.unrestricted) return new Set(discovered);
    const { kept, rejected } = sessionPolicy.filter(discovered);
    // Log only when the rejected set changes: this runs on every scan, and a
    // silent skip is the failure mode that makes an agent look mysteriously
    // absent from the dashboard.
    const signature = rejected.join(',');
    if (rejected.length && signature !== lastRejectedSignature) {
      console.log(`[push-relay] session policy excluded ${rejected.length}: ${rejected.join(', ')}`);
    }
    if (signature !== lastRejectedSignature) lastRejectedSignature = signature;
    return new Set(kept);
  } catch {
    return new Set();
  }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (res.status === 409) {
        ...
        relayHealthState.lastErrorCode = 'heartbeat_lease_rejected';
        return { ok: false, leaseRejected: true };
      }
      const httpError = new Error(`status ${res.status} ${body}`.trim());
      httpError.heartbeatHttpStatus = res.status;
      throw httpError;
    }
    relayHealthState.lastSuccessfulBackendContactAtMs = Date.now();
    return { ok: true, leaseRejected: false };
  } catch (e) {
    relayHealthState.lastErrorCode = e?.heartbeatHttpStatus ? `heartbeat_http_${e.heartbeatHttpStatus}` : 'heartbeat_error';
...
function mcpPidFilePath(agentName) {
  const homeRoot = (process.env.HAGENCY_HOMEDIR || '').trim() || path.join(os.homedir(), '.hagency');
...
async function buildNotification(agentName, msg) {
  ...
  if (hasMcp) {
    ...
    const sendHint = `Reply using the hagency MCP tool: send_message(to="${replyTo}", summary="your reply", full="detailed reply")`;
  ...
  const replyHint = `Reply using /agent-message skill or: hagency-send ${senderTmux} "<your reply>"`;
...
function shouldHandleDirectMessage(msg) {
  // The local dashboard queue owns backend message delivery. Remote relays
  // still consume the same SSE because they have no dashboard queue process.
  if (!PUSH_RELAY_REMOTE_MODE && msg?.deliveryOwner === 'dashboard-queue') return false;
  return true;
}
  ...
  for (const agentName of messageRecipients(msg)) {
    ...
    try {
      ...
      if (delivery.ok) {
        ...
        relayHealthState.lastSuccessfulOutboundDeliveryAtMs = Date.now();
        void recordDeliveryEvent({
        ...
        relayHealthState.lastErrorCode = 'tmux_inject_failed';
        void recordDeliveryEvent({
  ...
  for (const [agentName, entries] of relayQueue) {
    ...
    while (entries.length > 0) {
      ...
      if (delivery.ok) {
        ...
        relayHealthState.lastSuccessfulOutboundDeliveryAtMs = now;
        void recordDeliveryEvent({
          type: 'relay.delivered',
...
function connectSse() {
  ...
  es.on('error', (e) => {
    if (runtimeStopping || currentEs !== es) return;
    relayHealthState.lastErrorCode = 'sse_error';
...
function resetRelayState() {
  ...
  relayHealthState.lastSuccessfulBackendContactAtMs = null;
  relayHealthState.lastSuccessfulOutboundDeliveryAtMs = null;
  relayHealthState.lastErrorCode = null;
...
// Task 8: standalone cross-component doctor. push-relay.js polls this and writes it to
// data/health/push-relay.json via src/health-record.mjs; a shallow copy so callers can't
// mutate this module's internal state through the returned object.
function getRelayHealthSnapshot() {
  return { ...relayHealthState };
}
