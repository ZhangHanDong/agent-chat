...
SERVICE_NAME="${SERVICE_NAME:-hagency-push-relay}"
...
LEGACY_SERVICE_NAME="com.hagency.push-relay"
...
if [ ! -f "$ENV_FILE" ]; then
  ...
  echo "Created $ENV_FILE (please fill API_TOKEN and HAGENCY_SERVER)."
fi
...
MISSING_ENV=()
[ -n "${HAGENCY_API:-}" ] || MISSING_ENV+=("HAGENCY_API")
[ -n "${HAGENCY_SERVER:-}" ] || MISSING_ENV+=("HAGENCY_SERVER")
if [ "${#MISSING_ENV[@]}" -gt 0 ]; then
  echo "Error: missing required env in $ENV_FILE: ${MISSING_ENV[*]}" >&2
  exit 1
fi
SERVER_ID_TRIMMED="${HAGENCY_SERVER#"${HAGENCY_SERVER%%[![:space:]]*}"}"
...
if [ "$SERVER_ID_LOWER" = "local" ]; then
  echo "Error: HAGENCY_SERVER must be this remote host's unique server id, not 'local'." >&2
  exit 1
fi
if [ -z "${API_TOKEN:-}" ]; then
...
required_cmds=(hagency hagency-up hagency-down hagency-ls hagency-send hagency-update hagency-service verify-remote hagency-maintain)
optional_cmds=(self-time-reminder hagency-cli hagency hagency-prune-agents)
BIN_SOURCE_DIR="$SCRIPT_DIR/bin"
if [ -d "$REPO_ROOT/bin" ] && [ -f "$REPO_ROOT/bin/hagency-up" ]; then
...
if [ "$IS_LINUX" = true ]; then
  ...
  AUTODEPLOY_SERVICE="hagency-remote-autodeploy"
  AUTODEPLOY_UNIT="/etc/systemd/system/${AUTODEPLOY_SERVICE}.service"
  AUTODEPLOY_SCRIPT="$REPO_ROOT/scripts/hagency-remote-autodeploy.sh"
  ...
  if [ ! -d "$REPO_ROOT/.git" ]; then
    ...
    SUDOERS_FILE="/etc/sudoers.d/hagency-autodeploy"
...
MCP_ENV_VARS=()
[ -n "${HAGENCY_API:-}" ] && MCP_ENV_VARS+=("HAGENCY_API=$HAGENCY_API")
...
if command -v claude >/dev/null 2>&1; then
  CLAUDECODE= claude mcp remove -s user hagency 2>/dev/null || true
  CLAUDECODE= claude mcp remove -s local hagency 2>/dev/null || true
  CLAUDECODE= claude mcp remove -s project hagency 2>/dev/null || true
  ...
  CLAUDE_CMD+=(-- hagency node "$SCRIPT_DIR/mcp-server.js")
  if CLAUDECODE= "${CLAUDE_CMD[@]}"; then
    echo "  MCP server 'hagency' configured for Claude Code."
  else
    echo "Error: failed to configure Claude MCP server 'hagency'." >&2
    exit 1
  fi
else
  echo "  Warning: 'claude' CLI not found, skipping MCP configuration."
  echo "  Run manually: claude mcp add -s user -e HAGENCY_API=<url> -e API_TOKEN=<token> -- hagency node $SCRIPT_DIR/mcp-server.js"
fi

echo "[7/9] Configuring Codex MCP server..."
if command -v codex >/dev/null 2>&1; then
  codex mcp remove hagency 2>/dev/null || true
  CODEX_CMD=(codex mcp add hagency)
  for env_kv in "${MCP_ENV_VARS[@]}"; do
    CODEX_CMD+=(--env "$env_kv")
  done
  CODEX_CMD+=(-- node "$SCRIPT_DIR/mcp-server.js")
  if "${CODEX_CMD[@]}"; then
    echo "  MCP server 'hagency' configured for Codex."
  else
    echo "Error: failed to configure Codex MCP server 'hagency'." >&2
    exit 1
  fi
else
  echo "  Warning: 'codex' CLI not found, skipping MCP configuration."
  echo "  Run manually: codex mcp add hagency --env HAGENCY_API=<url> --env API_TOKEN=<token> -- node $SCRIPT_DIR/mcp-server.js"
fi
...
echo "  2) Start agents: hagency-up <name> <path> [claude|codex]"
