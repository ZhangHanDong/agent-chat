...
const remoteServerId = normalizeWrapperValue(process.env.HAGENCY_SERVER);
if (!remoteServerId) {
  console.error('[push-relay] FATAL: remote entrypoint requires HAGENCY_SERVER');
  process.exit(1);
}
if (remoteServerId.toLowerCase() === 'local') {
  console.error("[push-relay] FATAL: remote entrypoint requires HAGENCY_SERVER to be a non-local server id, got 'local'");
  process.exit(1);
}
...
if (process.env.HAGENCY_WRAPPER_SMOKE === '1') {
  process.exit(0);
}
