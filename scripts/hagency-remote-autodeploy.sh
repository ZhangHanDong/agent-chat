...
REPO_DIR="${HAGENCY_REPO_DIR:-${HAGENCY_REPO_ROOT:-$DEFAULT_REPO_DIR}}"
...
load_remote_env
REPO_DIR="${HAGENCY_REPO_DIR:-${HAGENCY_REPO_ROOT:-$REPO_DIR}}"
DEPLOY_BRANCH="${HAGENCY_DEPLOY_BRANCH:-stable}"
POLL_SEC="${HAGENCY_POLL_SEC:-60}"
RELAY_SERVICE="${HAGENCY_RELAY_SERVICE:-hagency-push-relay}"
ONCE="${HAGENCY_ONCE:-false}"
GIT_BIN="${HAGENCY_GIT_BIN:-git}"
NPM_BIN="${HAGENCY_NPM_BIN:-npm}"
SYSTEMCTL_BIN="${HAGENCY_SYSTEMCTL_BIN:-systemctl}"
LAUNCHCTL_BIN="${HAGENCY_LAUNCHCTL_BIN:-launchctl}"
SUDO_BIN="${HAGENCY_SUDO_BIN:-sudo}"
SLEEP_BIN="${HAGENCY_SLEEP_BIN:-sleep}"
VERIFY_REMOTE_BIN="${HAGENCY_VERIFY_REMOTE_BIN:-$REPO_DIR/bin/verify-remote}"
...
init_deploy_state() {
  if [ -z "$DEPLOY_STATE_DIR" ]; then
    DEPLOY_STATE_DIR="$(run_git rev-parse --absolute-git-dir)/hagency-remote-autodeploy"
  fi
...
verify_remote_deploy() {
  ...
  if [ -z "${HAGENCY_API:-}" ]; then
    log "ERROR: missing HAGENCY_API; cannot verify remote deploy"
    return 1
  fi
  if [ -z "${HAGENCY_SERVER:-}" ]; then
    log "ERROR: missing HAGENCY_SERVER; cannot verify remote deploy"
    return 1
  fi
