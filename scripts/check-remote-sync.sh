...
PROFILE_SPECIFIC_FILES=(
  "bin/hagency:remote profile-specific command surface"
  "bin/hagency-up:remote profile-specific launcher with shared permission baseline"
)
...
check_hagency_dispatch_targets() {
  local file="$1"
  local label="$2"
  ...
  if [ ! -f "$file" ]; then
    echo "[FAIL] Missing hagency CLI for dispatch check: $file"
    failures=$((failures + 1))
    return
  fi
...
echo "Checking hagency dispatch targets..."
check_hagency_dispatch_targets "bin/hagency" "root"
check_hagency_dispatch_targets "remote/bin/hagency" "remote"
