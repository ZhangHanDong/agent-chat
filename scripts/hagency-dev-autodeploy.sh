...
REPO_DIR="${HAGENCY_DEPLOY_DIR:?HAGENCY_DEPLOY_DIR must be set}"
DEPLOY_BRANCH="${HAGENCY_DEPLOY_BRANCH:-master}"
POLL_SEC="${HAGENCY_POLL_SEC:-30}"
DEPLOY_SERVICES="${HAGENCY_DEPLOY_SERVICES:-hagency-dev-backend.service hagency-dev-web.service}"
...
init_deploy_state() {
  if [ -z "$DEPLOY_STATE_DIR" ]; then
    DEPLOY_STATE_DIR="$(run_git rev-parse --absolute-git-dir)/hagency-dev-autodeploy"
  fi
