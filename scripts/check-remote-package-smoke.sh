...
assert_contains "$autodeploy_service" "ExecStart=/usr/bin/env bash __REPODIR__/scripts/hagency-remote-autodeploy.sh" "generated remote autodeploy service"
echo "[OK] Generated remote autodeploy service contract is stable"

help_output="$("$PKG_DIR/bin/hagency" --help)"
printf '%s' "$help_output" | grep -q 'Usage: hagency <command> \[args\]' || fail "generated hagency help missing usage"
...
GRAPH_OUT="$TMP_DIR/hagency-remote-graph.out"
GRAPH_ERR="$TMP_DIR/hagency-remote-graph.err"
if "$PKG_DIR/bin/hagency" graph >"$GRAPH_OUT" 2>"$GRAPH_ERR"; then
  fail "generated remote hagency graph unexpectedly succeeded"
fi
...
AUDIT_OUT="$TMP_DIR/hagency-remote-audit.out"
AUDIT_ERR="$TMP_DIR/hagency-remote-audit.err"
if "$PKG_DIR/bin/hagency" audit --quiet >"$AUDIT_OUT" 2>"$AUDIT_ERR"; then
  fail "generated remote hagency audit --quiet unexpectedly succeeded"
fi
...
SYNC_SKILLS_OUT="$TMP_DIR/hagency-remote-sync-skills.out"
SYNC_SKILLS_ERR="$TMP_DIR/hagency-remote-sync-skills.err"
if "$PKG_DIR/bin/hagency" sync-skills --check >"$SYNC_SKILLS_OUT" 2>"$SYNC_SKILLS_ERR"; then
  fail "generated remote hagency sync-skills --check unexpectedly succeeded"
fi
...
service_help="$("$PKG_DIR/bin/hagency" service --help)"
assert_contains "$service_help" "Controls services on the current host only." "generated remote service help"
assert_contains "$service_help" "remote relay service hagency-push-relay" "generated remote service help"
update_help="$("$PKG_DIR/bin/hagency" update --help)"
...
assert_contains "$update_help" "Service flags only control the remote relay service: hagency-push-relay." "generated remote update help"
ls_help="$("$PKG_DIR/bin/hagency" ls --help)"
...
down_help="$("$PKG_DIR/bin/hagency" down --help)"
...
UPDATE_OUT="$TMP_DIR/hagency-update-check.out"
UPDATE_ERR="$TMP_DIR/hagency-update-check.err"
if HOME="$TMP_DIR/home" HAGENCY_REPO_DIR= HAGENCY_REPO_ROOT= "$PKG_DIR/bin/hagency" update --check >"$UPDATE_OUT" 2>"$UPDATE_ERR"; then
  fail "generated standalone hagency update --check unexpectedly succeeded"
fi
...
if ! HAGENCY_WRAPPER_SMOKE=1 HAGENCY_SERVER=remote-smoke node "$PKG_DIR/push-relay.js"; then
...
if ! HAGENCY_WRAPPER_SMOKE=1 node "$PKG_DIR/mcp-server.js"; then
