...
if (!name || !workspace || !frameworkId) {
  process.stderr.write('usage: hagency-acp-agent.mjs --name <n> --workspace <dir> --framework <id> '
    + '[--model <m>] [--provider <p>]\n');
  process.exit(2);
}
...
// ── Backend access ────────────────────────────────────────────────────────────
// Same layout hagency-acp-up provisions the token into.
const HOME_DIR = process.env.HAGENCY_HOMEDIR || path.join(os.homedir(), '.hagency');
const STATE_DIR = process.env.HAGENCY_AGENT_STATE_DIR
  || path.join(HOME_DIR, 'agents', `agent_${name}`, 'state');
const BACKEND_PORT = process.env.HAGENCY_BACKEND_PORT || '8090';
const API = (process.env.HAGENCY_API || `http://127.0.0.1:${BACKEND_PORT}`).replace(/\/$/, '');
const MCP_SERVER_NAME = process.env.HAGENCY_MCP_SERVER_NAME || 'hagency';
...
/**
 * Hagency's own MCP server, handed to the agent at session/new.
 *
...
const mcpServers = agentToken ? [{
  ...
  env: [
    ...
    { name: 'HAGENCY_API', value: API },
    { name: 'HAGENCY_BACKEND_PORT', value: String(BACKEND_PORT) },
    { name: 'HAGENCY_MCP_SERVER_NAME', value: MCP_SERVER_NAME },
    { name: 'HAGENCY_AGENT_STATE_DIR', value: STATE_DIR },
  ],
}] : [];
 ...
 * rule allows Hagency's own coordination tools — reading an inbox is not shell
...
// mcp=hagency here read as a working tool channel that did not exist.
...
// own pid.
//
// bin/hagency-acp-up registered on the operator's behalf, which is correct only
  ...
  try {
    await api('/api/agents', {
      ...
      body: {
        ...
        transport: 'acp',
        acpPid: process.pid,
        server: process.env.HAGENCY_SERVER || os.hostname(),
      },
    });
  ...
  try {
    await api(`/api/agents/${encodeURIComponent(name)}/runtime`, {
      ...
      body: {
         ...
         * is what ties either back to an agent. Hagency launches the agent knowing that
...
//
// A tmux agent calls Hagency's MCP tools directly. octos 2.0.2 cannot: it accepts
...
const outboxDir = path.join(path.resolve(workspace), '.hagency', 'outbox');
 ...
 */
/**
 * The progress policy, from the same file `bin/hagency-progress` reads.
 *
 * `$HOME/.hagency/progress-filter.json` rather than the runtime directory, because the router does not pass
 * `HAGENCY_RUNTIME_DIR` to a dispatched agent — see the note in the reporter. One location for one policy:
...
function readProgressFilter() {
  const file = (process.env.HAGENCY_PROGRESS_FILTER || '').trim()
    || path.join(os.homedir(), '.hagency', 'progress-filter.json');
  try {
...
function makeProgressEmitter({ name, messages, filterDoc, cursor, runtime: rt, post, log: logLine }) {
  ...
  function collect() {
     ...
     */
    const seen = process.env.HAGENCY_PROGRESS_DEBUG ? {} : null;
...
function buildNudge(pending, messages) {
  ...
  // unconditionally, so the same `hagency tell` produced silence from claude and
...
async function pollAndDeliver() {
  ...
  // prompt. That was right when octos had no way to reach Hagency, and wrong once
