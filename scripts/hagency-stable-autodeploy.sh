...
REPO_DIR="${HAGENCY_LIVE_DIR:-${HAGENCY_LIVE_ROOT:-${HAGENCY_REPO_ROOT:-$DEFAULT_REPO_DIR}}}"
DEPLOY_USER="${HAGENCY_DEPLOY_USER:?HAGENCY_DEPLOY_USER must be set}"
DEPLOY_BRANCH="${HAGENCY_DEPLOY_BRANCH:-stable}"
POLL_SEC="${HAGENCY_POLL_SEC:-30}"
BACKEND_SERVICE="${HAGENCY_BACKEND_SERVICE:-hagency-backend}"
DEPLOY_SERVICES="${HAGENCY_DEPLOY_SERVICES:-hagency ${BACKEND_SERVICE} bridge-matrix}"
...
RELEASE_GATE="${HAGENCY_RELEASE_GATE:-worktree}"
RELEASE_GATE_ARGS="${HAGENCY_RELEASE_GATE_ARGS:-}"
ONCE="${HAGENCY_ONCE:-false}"
SYSTEMCTL_BIN="${HAGENCY_SYSTEMCTL_BIN:-systemctl}"
CURL_BIN="${HAGENCY_CURL_BIN:-curl}"
SLEEP_BIN="${HAGENCY_SLEEP_BIN:-sleep}"
NPM_BIN="${HAGENCY_NPM_BIN:-npm}"
DEPLOY_STATE_DIR="${HAGENCY_DEPLOY_STATE_DIR:-}"
...
run_systemctl() {
  # This watcher runs unprivileged (see hagency-stable-autodeploy.service).
  # Restarts escalate through the narrow /etc/sudoers.d/hagency-autodeploy
...
BACKEND_HEALTH_URL="${HAGENCY_BACKEND_HEALTH_URL:-http://127.0.0.1:8090/api/agents}"
BACKEND_HEALTH_TIMEOUT="${HAGENCY_BACKEND_HEALTH_TIMEOUT:-30}"
...
init_deploy_state() {
  if [ -z "$DEPLOY_STATE_DIR" ]; then
    DEPLOY_STATE_DIR="$(run_git rev-parse --absolute-git-dir)/hagency-autodeploy"
  fi
...
emit_alert() {
  local summary="$1"
  [ -n "${HAGENCY_ALERT_URL:-}" ] || return 0
  [ -n "${HAGENCY_ALERT_TOKEN:-}" ] || return 0
  "$CURL_BIN" -sf -m 10 -X POST "$HAGENCY_ALERT_URL" \
    -H "Authorization: Bearer $HAGENCY_ALERT_TOKEN" \
...
run_release_gate() {
  ...
  case "$RELEASE_GATE" in
    ...
    worktree)
      ...
      ;;
    *)
      log "FATAL: unsupported HAGENCY_RELEASE_GATE='$RELEASE_GATE'"
      exit 1
      ;;
  esac
}
...
validate_startup() {
  ...
  if ! [[ "$POLL_SEC" =~ ^[0-9]+$ ]] || [ "$POLL_SEC" -lt 5 ]; then
    log "FATAL: invalid HAGENCY_POLL_SEC='$POLL_SEC' (must be >= 5)"
    exit 1
  fi
  case "$RELEASE_GATE" in
    none|worktree|"") ;;
    *)
      log "FATAL: invalid HAGENCY_RELEASE_GATE='$RELEASE_GATE' (expected none or worktree)"
      exit 1
      ;;
  esac
}
