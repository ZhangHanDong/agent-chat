#!/usr/bin/env node
/**
 * Register or remove a paneless ACP agent in a service profile, so Hagency's own
...
if (!['add', 'remove'].includes(action) || !name || !profilePath) {
  process.stderr.write(
    'usage: hagency-supervise-agent.mjs <add|remove> --name <agent> --profile <file>\n'
...
if (action === 'add') {
  ...
  const command = [
    'node', 'scripts/hagency-acp-agent.mjs',
    '--name', name,
