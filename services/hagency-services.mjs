...
function supervisorCommandMatches(pid, runtimeRoot) {
  try {
    ...
    // The supervisor is started as `node services/hagency-services.mjs run ...`
  ...
  process.on('SIGHUP', () => {
    ...
    (async () => {
      try {
        ...
        process.stdout.write(`[hagency-services] reload: ${summary}\n`);
      } catch (error) {
        // A reload that fails must leave the running fleet exactly as it was.
        process.stderr.write(`[hagency-services] reload failed, keeping the running profile: ${error.message}\n`);
      } finally {
        reloading = false;
      }
    })();
...
main().catch((error) => {
  process.stderr.write(`[hagency-services] ${error.message}\n`);
  process.exitCode = 1;
});
